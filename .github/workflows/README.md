...
| Assign issue contributors | `assign-issue.yml` | `issue_comment` (created comments starting with `/assign` on issues) | lets maintainers, committers, and reviewers listed in `docs/OWNER.md` use `/assign @username`; allows one assignee per issue and one assigned open issue per contributor, except that the users listed in `docs/OWNER.md` may hold several open issues; comments from users outside the allowlist are ignored without a reply |
