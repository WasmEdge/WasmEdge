...
WasmEdge_ValType *parseValueTypes(JNIEnv *env, jintArray jValueTypes) {
  ...
  WasmEdge_ValType *valTypes = malloc(len * sizeof(WasmEdge_ValType));
  // FIXME:
...
void setJavaValueObject(JNIEnv *env, WasmEdge_Value value, jobject j_val) {
  char *str_val;
  WasmEdge_ValType Ty = value.Type;
  if (WasmEdge_ValTypeIsI32(Ty)) {
    setJavaIntValue(env, value, j_val);
  } else if (WasmEdge_ValTypeIsI64(Ty) || WasmEdge_ValTypeIsFuncRef(Ty)) {
    setJavaLongValue(env, value, j_val);
  } else if (WasmEdge_ValTypeIsV128(Ty)) {
    str_val = u128toa(value.Value);
    setJavaStringValue(env, str_val, j_val);
  } else if (WasmEdge_ValTypeIsF32(Ty)) {
    setJavaFloatValue(env, value, j_val);
  } else if (WasmEdge_ValTypeIsF64(Ty)) {
    setJavaDoubleValue(env, value, j_val);
  } else if (WasmEdge_ValTypeIsExternRef(Ty)) {
