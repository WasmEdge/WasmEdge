...
impl<'a> Vm {
    ...
    pub fn run(self, func_name: &str, params: &[wasmedge::Value]) -> Result<Vec<wasmedge::Value>, anyhow::Error> {
        match self.inner {
            Some(mut vm) => {
...
impl<'a> VmBuilder<'a> {
    ...
    pub fn build(self) -> Result<Vm, anyhow::Error> {
        ...
        if vm.module.is_some() && vm.config.is_some() {
            ...
            let vm_instance = wasmedge::Vm::create(&module.inner, &config).map_err(VmError::Create)?;
