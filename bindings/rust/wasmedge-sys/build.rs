...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let header_path = incdir_path.join("wasmedge").join(WASMEDGE_H);
    let header_path_str = header_path
        .to_str()
        .unwrap_or_else(|| panic!("`{}` must be a utf-8 path", header_path.display()));
    println!(
    ...
    bindgen::builder()
        ...
        // .clang_arg(r"-L/root/workspace/me/WasmEdge/build/lib/api")
        .prepend_enum_name(false) // The API already prepends the name.
        .parse_callbacks(Box::new(bindgen::CargoCallbacks))
        .generate()
        .expect("failed to generate bindings")
