...
mod tests {
    ...
    use crate::{
        ...
        Config, Loader, Store, Value, WasmEdgeError,
    };
    ...
    fn test_vm_invoke_wasm_function_step_by_step() {
        ...
        assert!(result.is_ok());
        let values = result.unwrap().collect::<Vec<_>>();
    ...
    fn test_vm_register_wasm_from_file() {
        ...
        // register module from ImportObj module from a non-existed file
        ...
        // // register module from ImportObj module from a wasm file
        // let path =
        //     std::path::PathBuf::from(env!("WASMEDGE_DIR")).join("test/api/apiTestData/test.wasm");
    ...
    fn test_vm_register_wasm_from_module() {
        ...
        let result = Config::create();
        assert!(result.is_ok());
        let config = result.unwrap().enable_wasi(true);
