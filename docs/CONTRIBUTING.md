...
| Performance Issue | Something works, but is slower or uses more resources than expected. | `perf:` | `performance` |
...
- **Type labels** (`bug`, `enhancement`, `performance`, `question`) are applied automatically based on the template you choose.
...
- **For a performance improvement**, open a [Performance Issue](https://github.com/WasmEdge/WasmEdge/issues/new?template=performance_issue.yml) first, including the workload and the baseline measurements. The pull request should then provide before/after numbers for the same workload, and link back to the issue.
...
In addition to the checklist above, each type of pull request has its own requirements:

- **Bug fix (`fix:`)** — link the [Bug Report](https://github.com/WasmEdge/WasmEdge/issues/new?template=bug_report.yml) it resolves. Add a regression test that fails without the fix and passes with it, and describe the root cause in the description.
- **Feature (`feat:`)** — link the agreed [Feature Request](https://github.com/WasmEdge/WasmEdge/issues/new?template=feature_request.yml). Add tests that cover the new behavior, and update the relevant documentation. A breaking change must also follow the timing rules in the [Release Policy](./RELEASE_POLICY.md).
- **Performance (`perf:`)** — link the [Performance Issue](https://github.com/WasmEdge/WasmEdge/issues/new?template=performance_issue.yml). Provide reproducible before/after benchmarks for the same workload and environment, and confirm that the existing tests still pass so that behavior is unchanged.
...
- A maintainer responds to a new pull request within **2 weeks** of submission. During that review, the maintainer decides whether to keep the pull request open (for example, to continue the review or request changes) or to close it (for example, when it is out of scope, not part of the project's plan, not responded, or does not meet the [requirements](#requirements)).
