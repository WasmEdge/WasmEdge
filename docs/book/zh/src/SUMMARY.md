# Summary

- [快速启动](index.md)
    - [安装](start/install.md)
    - [用Docker镜像启动](start/docker.md)
    - [WasmEdge命令行](start/cli.md)
    - [通用二进制](start/universal.md)

- [WasmEdge介绍](intro.md)
    - [应用场景](intro/use.md)
        - [云原生的runtime](intro/use/runtime.md)
        - [JavaScript or DSL runtime](intro/use/js.md)
        - [Serverless函数服务](intro/use/serverless.md)
        - [边缘计算](intro/use/edge.md)
        - [SaaS里的嵌入式](intro/use/saas.md)
    - [WasmEdge的优势与特点](intro/features.md)
    - [WebAssembly的标准扩展](intro/standard.md)
    - [WasmEdge特有的扩展](intro/proprietary.md)
    - [与外部框架的集成](intro/integrations.md)
    - [FAQ](intro/faq.md)

- [开发WasmEdge应用](dev.md)
    - [Rust](dev/rust.md)
        - [使用操作系统服务](dev/rust/wasi.md)
        - [Tensorflow推理](dev/rust/tensorflow.md)
        - [网络](dev/rust/networking.md)
        - [使用命令行程序](dev/rust/command.md)
        - [rustwasmc](dev/rust/bindgen.md)
    - [JavaScript](dev/js.md)
        - [快速启动](dev/js/quickstart.md)
        - [ES6模块](dev/js/es6.md)
        - [CommonJS模块](dev/js/cjs.md)
        - [NodeJS &amp; NPM模块](dev/js/npm.md)
        - [示例：React SSR](dev/js/ssr.md)
        - [Tensorflow推理](dev/js/tensorflow.md)
        - [网络](dev/js/networking.md)
        - [异步网络服务](dev/js/async.md)
        - [用 Rust 实现 JS API](dev/js/rust.md)
    - [Go](dev/go.md)
    - [Swift](dev/swift.md)
    - [AssemblyScript](dev/as.md)
    - [Kotlin](dev/kotlin.md)
    - [Grain](dev/grain.md)

- [嵌入WasmEdge函数](embed.md)
    - [C SDK](embed/c.md)
       - [API references](embed/c/ref.md)
    - [Go SDK](embed/go.md)
       - [Embed WASM apps](embed/go/app.md)
       - [Embed WASM funcs](embed/go/function.md)
       - [Pass complex data](embed/go/memory.md)
       - [Tensorflow](embed/go/tensorflow.md)
       - [Embed bindgen funcs](embed/go/bindgen.md)
       - [API references](embed/go/ref.md)
    - [Node.js SDK](embed/node.md)
    - [Rust SDK](embed/rust.md)
    - [Python SDK](embed/python.md)

- [用Kubernetes管理WasmEdge应用](kubernetes.md)
    - [快速启动](kubernetes/quickstart.md)
    - [应用示例](kubernetes/demo.md)
        - [简单的WASI例子](kubernetes/demo/wasi.md)
        - [HTTP微服务](kubernetes/demo/server.md)
    - [容器runtimes](kubernetes/container.md)
        - [crun](kubernetes/container/crun.md)
        - [runc](kubernetes/container/runc.md)
        - [youki](kubernetes/container/youki.md)
    - [CRI runtimes](kubernetes/cri.md)
        - [CRI-O](kubernetes/cri/crio.md)
        - [containerd](kubernetes/cri/containerd.md)
    - [Kubernetes](kubernetes/kubernetes.md)
        - [Kubernetes + CRI-O](kubernetes/kubernetes/kubernetes-crio.md)
        - [Kubernetes + containerd](kubernetes/kubernetes/kubernetes-containerd.md)
        - [KubeEdge](kubernetes/kubernetes/kubeedge.md)
        - [SuperEdge](kubernetes/kubernetes/superedge.md)
        - [OpenYurt](kubernetes/kubernetes/openyurt.md)

- [应用框架与平台](frameworks.md)
    - [Service mesh与分布式框架](frameworks/mesh.md)
        - [Dapr](frameworks/mesh/dapr.md)
        - [MOSN](frameworks/mesh/mosn.md)
        - [Apache EventMesh](frameworks/mesh/eventmesh.md)
    - [应用框架](frameworks/app.md)
        - [YoMo](frameworks/app/yomo.md)
        - [Reactr](frameworks/app/reactr.md)
    - [Serverless平台](frameworks/serverless.md)
        - [Vercel](frameworks/serverless/vercel.md)
        - [Netlify](frameworks/serverless/netlify.md)
        - [AWS](frameworks/serverless/aws.md)
        - [Tencent](frameworks/serverless/tencent.md)
        - [Second State](frameworks/serverless/secondstate.md)

- [操作系统](os.md)
    - [Linux](os/linux.md)
    - [Windows](os/windows.md)
    - [Mac](os/mac.md)
    - [seL4](os/sel4.md)
    - [Open Harmony](os/openharmony.md)

- [为WasmEdge贡献代码](extend.md)
    - [编译WasmEdge](extend/build.md)
        - [用Mac编译](extend/build_on_mac.md)
        - [用Windows编译](extend/build_on_windows.md)
    - [WasmEdge plug-in API](extend/plugin.md)
        - [Host functions](extend/plugin/hostfunction.md)
        - [External references](extend/plugin/externref.md)

