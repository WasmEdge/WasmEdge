...
///
/// \returns WasmEdge_Async. Call `WasmEdge_AsyncGet` for the result, and call
/// `WasmEdge_AsyncDelete` to destroy this object. The handle must be deleted
/// BEFORE the producing VM (WasmEdge_VMDelete rejects otherwise).
WASMEDGE_CAPI_EXPORT extern WasmEdge_Async *
...
/// VM (WasmEdge_VMAsyncExecute / *AsyncRunWasm* / *AsyncExecuteRegistered*)
/// must be released with WasmEdge_AsyncDelete BEFORE deleting the VM. A live
/// async handle co-owns the invocation's result lease; deleting the VM first
/// is a detected usage error and the deletion is REJECTED (the VM stays alive,
/// an error is logged) rather than deadlocking teardown.
///
/// Calling this from within one of this VM's own running host callbacks
/// (self-teardown) is a no-op rejection: the deletion is refused, an error
/// is logged, and the VM remains ALIVE and owned by the caller. Delete the
/// VM only after all of its invocations have returned.
///
