...
struct InfoMismatch {
  ...
  /// Case 12: unexpected number of values
  InfoMismatch(const size_t ExpC, const size_t GotC) noexcept
      : Category(MismatchCategory::Count), ExpCount(ExpC), GotCount(GotC) {}
...
struct fmt::formatter<WasmEdge::ErrInfo::InfoMismatch>
  ...
  auto format(const WasmEdge::ErrInfo::InfoMismatch &Info,
    ...
    case WasmEdge::ErrInfo::MismatchCategory::CompFunctionType:
      ...
      break;
    case WasmEdge::ErrInfo::MismatchCategory::Count:
      fmt::format_to(Iter, "Expected: {} , Got: {}"sv, Info.ExpCount,
                     Info.GotCount);
      break;
    default:
      break;
