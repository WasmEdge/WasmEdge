...
public:
  ...
  constexpr EndianValue(T Val) noexcept
      : Native(Val), LittleEndian(switchEndian(Val)) {}
  constexpr EndianValue(T Val, Endian Endian) noexcept {
    if (Endian != Endian::little) {
      Native = Val;
      LittleEndian = switchEndian(Native);
    } else {
  ...
  constexpr const T le() const noexcept { return LittleEndian; }
  constexpr const T newLe() noexcept {
    LittleEndian = switchEndian(Native);
    return LittleEndian;
  }
