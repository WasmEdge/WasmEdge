...
/// __attribute__((vector_size(16)), we use this type to fill the gap. The
/// vector type used everywhere else is TotalSize-aligned, so alignas restores
/// that: a std::array of 8-byte elements is only 8-aligned, which would leave
/// ValVariant and RefVariant under-aligned for the whole-slot 128-bit atomic
/// the GC coherent-slot accessors use (gc/coherent_slot.h).
template <typename Ty, size_t TotalSize,
          std::enable_if_t<(TotalSize % sizeof(Ty) == 0), int> = 0>
struct alignas(TotalSize) SIMDArray : std::array<Ty, (TotalSize / sizeof(Ty))> {
};
#else
template <typename Ty, size_t TotalSize,
          std::enable_if_t<(TotalSize % sizeof(Ty) == 0), int> = 0>
