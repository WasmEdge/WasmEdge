...
TypeU<T> Executor::runClzOp(Runtime::StackManager &StackMgr) const noexcept {
  ...
  if (I != 0U) {
    ...
    StackMgr.emplaceTop<T>(static_cast<T>(sizeof(T) * 8));
  }
  return {};
}
...
TypeF<T> Executor::runAbsOp(Runtime::StackManager &StackMgr) const noexcept {
  ...
  // Clear the sign bit via memcpy bit-cast, not reinterpret_cast (strict
  // aliasing UB between float and its integer representation).
