...
Expect<void> Executor::runExtractLaneOp(Runtime::StackManager &StackMgr,
  ...
  TOut Result = StackMgr.peekTop<VTIn>()[Index];
  StackMgr.emplaceTop<TOut>(std::move(Result));
  return {};
}
...
Executor::runSplatOp(Runtime::StackManager &StackMgr) const noexcept {
  ...
  if constexpr (sizeof(TOut) == 1) {
    StackMgr.emplaceTop<VTOut>(VTOut{Part, Part, Part, Part, Part, Part, Part,
                                     Part, Part, Part, Part, Part, Part, Part,
                                     Part, Part});
  } else if constexpr (sizeof(TOut) == 2) {
    StackMgr.emplaceTop<VTOut>(
        VTOut{Part, Part, Part, Part, Part, Part, Part, Part});
  } else if constexpr (sizeof(TOut) == 4) {
    StackMgr.emplaceTop<VTOut>(VTOut{Part, Part, Part, Part});
  } else if constexpr (sizeof(TOut) == 8) {
    StackMgr.emplaceTop<VTOut>(VTOut{Part, Part});
  }
  return {};
}
...
Executor::runVectorConvertOp(Runtime::StackManager &StackMgr) const noexcept {
  ...
  if constexpr (sizeof(TIn) == sizeof(TOut)) {
    StackMgr.emplaceTop(VTOut{static_cast<TOut>(V[0]), static_cast<TOut>(V[1]),
                              static_cast<TOut>(V[2]),
                              static_cast<TOut>(V[3])});
  } else { // int64/uint64 to double
    StackMgr.emplaceTop(
        VTOut{static_cast<TOut>(V[0]), static_cast<TOut>(V[1])});
  }
  return {};
}
