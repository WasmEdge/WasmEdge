...
public:
  ...
  // beginCycle() AFTER the collection has won the exclusive token but BEFORE
  // the Idle->MarkingRoot CAS -- so the token is held while the heap is still
  // Idle and no allocator lock is held. Lets a test park a grower behind the
  // held token and have it pop its table.grow initializer while the write
  // barrier is still quiet (Idle), so the grower's scoped-root pin -- not the
  // pop-time shade -- is what must carry that ref into this cycle's root
  // snapshot (GCThread.InterpreterGrowInitializerSurvivesCollect). Set while
  // Idle, like setTracerPauseHook. Empty in production.
...
private:
  ...
  // Test seam: GC.RootShadeQuietDuringSweeping drives the root-scan shading
  // path by hand to prove markGray() is quiet outside a marking phase. There is
  // no public route to it -- writeBarrier() gates itself, so it cannot reach
  // the gate that now lives inside markGray() -- and widening the shading API
  // for a test would defeat the point of the friendship above. Defined only in
  // test/gc/GCTest.cpp; nothing in the library declares it.
  ...
  // next stop-the-world root scan. Guarded by HeapMutex.
