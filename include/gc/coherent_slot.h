...
///
/// \file
/// A managed struct/array field is a 128-bit RefVariant: the low word (Raw[0])
/// is the type tag, the high word (Raw[1]) the object pointer. During
/// concurrent marking a collector worker reads such a field while a mutator may
/// struct.set/array.set the SAME slot, so two independent word accesses can
/// fabricate a torn (type, pointer) pair that the runtime would dereference.
///
/// The accessors here close that gap:
///   * loadPointerWordRelaxed -- the marker's fast path: a single relaxed
///     atomic load of the pointer word only.
...
///
/// The primary path is a single 16-byte atomic on the whole slot: coherent by
/// construction and TSan-clean, since TSan never reports a race between two
/// atomic accesses even at differing widths. It is selected only when the
/// toolchain offers a genuinely inline-lock-free double-width CAS (x86-64
/// cmpxchg16b, aarch64 casp/ldxp-stxp, or the MSVC 128-bit interlocked
/// intrinsic), never degrading to an __atomic_*_16 libcall.
///
/// The fallback path, for platforms with neither, is a striped SPINLOCK rather
/// than a seqlock: both reader and writer take the per-stripe lock, so
/// same-stripe writers serialize and a reader always sees a whole pair. (A bare
/// seqlock with a shared stripe counter races itself: one writer can drive the
/// sequence while another is mid-write, letting a reader accept a torn value.)
/// Every word is still accessed with a relaxed atomic so the lockless marker
/// read never races a locked writer's store.
///
//===----------------------------------------------------------------------===//
#pragma once
...
//     -mcx16, or aarch64): inline lock-free 16-byte __sync CAS; under
...
// Both accessors are built from a 16-byte compare-and-swap, exactly as the
// MSVC path is. __atomic_load_n / __atomic_store_n at 16 bytes are libcalls
// under GCC even where __GCC_HAVE_SYNC_COMPARE_AND_SWAP_16 is defined (checked
// on x86-64 -mcx16; on aarch64 it surfaced as an undefined __atomic_load_16 at
// link time). That macro promises one thing only: an inline 16-byte __sync
// compare-and-swap. Clang inlines either form, so this costs it nothing.
inline SlotWords loadWords(const void *Addr) noexcept {
  using U128 = unsigned __int128;
  auto *P = const_cast<U128 *>(reinterpret_cast<const U128 *>(Addr));
  // Comparand and exchange value are both zero: a match rewrites the same bits,
  // a miss leaves the slot untouched. Non-destructive either way.
  const U128 V = __sync_val_compare_and_swap(P, U128{0}, U128{0});
  SlotWords R;
...
inline void storeWords(void *Addr, SlotWords W) noexcept {
  using U128 = unsigned __int128;
  auto *P = reinterpret_cast<U128 *>(Addr);
  U128 Desired;
  std::memcpy(&Desired, W.data(), sizeof(Desired));
  // Each failed swap returns the value observed, so the retry converges to a
  // single coherent 128-bit store.
  U128 Expected{0};
  for (;;) {
    const U128 Seen = __sync_val_compare_and_swap(P, Expected, Desired);
    if (Seen == Expected) {
      return;
    }
    Expected = Seen;
