...
public:
  ...
  // generated code polls it inline at loop back-edges (see stopRequested()).
  // The controller outlives every compiled entry that reads it.
  ...
  // has no entry. Read under RegistryMtx (State is coordinator-visible), so
  // call it only from a normal (non-signal) context such as the enterFunction
  ...
  // strand the entry in NativeRunning (remotely scannable) when the boundary
  // was Running. Runs in the normal (non-signal) recovery context in
  // helper.cpp, so taking RegistryMtx and parking are legal here (unlike the
  // signal-safe head truncation in emitFault). Restoring TO a scannable state
  ...
  // coordinator (the TOCTOU window between waitForAcks and
  // scanNonRunningRoots).
  ...
  // A per-controller single-owner arbiter. A stop-the-world operation
  // (collection or table growth) must own the token before it may drive a
  // handshake, so at most one such operation is ever in flight -- growth and
  // collection can never both stop the world at once. collect() takes it
  ...
  // under RegistryMtx (so a handshake the current owner raises counts it
  // without waiting), enqueues a FIFO ticket, and parks on TokenCV until the
  // reserved handoff grants it the token (returns true, OutGen set) or the
  // controller starts Closing (returns false). Exactly-once release is via
  // endExclusiveOp with the OutGen this returned. noexcept: on the mutator hot
  // path.
  ...
  // token is Free and the controller is not Closing, take it immediately (mint
  // a fresh owner generation into OutGen, return true); otherwise return false
  // at once WITHOUT queuing or parking. Used by the collector
  // (Controller::collect): a losing collection is skippable, so it must NOT
  // enqueue as a FIFO grow-ticket -- the reserved handoff relabels every
  // granted ticket OwnedGrowing (tickets carry no per-ticket Want), which would
  // trip endExclusiveOp's assuming(ExclState == Have) when the collector later
  // released OwnedCollecting. Grows still use the blocking beginExclusiveOp.
  // Release via endExclusiveOp with the OutGen this returned.
  ...
  // -> HandshakeMtx -> HeapMutex. Reuses the exact primitives collect() uses;
  // it does NOT reimplement the handshake.
  ...
  void growStopTheWorld(Allocator &A, ResizeFn &&Resize) noexcept {
    ...
    waitForAcks(Gen);
    {
      ...
      // vector under it, so holding it across the swap stops the scan iterating
      // a freed buffer (a collection cannot actually run here -- the grower
      // holds the exclusive token -- but keep the lock discipline the plain
      // path uses).
  ...
  // TokenCV) awaiting the reserved FIFO handoff. Read under ExclusiveMtx. Lets
  // a test prove a grower parked behind an in-flight collection without a
  // timing window (GCThread.GrowBlocksDuringCollectAndViceVersa).
...
private:
  ...
  uint64_t ExclGrantedTicket =
      0; // ticket the reserved handoff granted (0=none)
