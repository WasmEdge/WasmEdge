...
/// WASI component instances, which borrow them for their lifetime. It is an
/// instance itself so that the VM keeps it by the base type, as the core VM
/// keeps `WasiModule`; it exports nothing and is never registered by name.
class WasiHosts : public Runtime::Instance::ComponentInstance {
public:
  WasiHosts() noexcept
      : Runtime::Instance::ComponentInstance("wasi:hosts"), Io(Environment),
        Sockets(Io), Http(Io) {}
