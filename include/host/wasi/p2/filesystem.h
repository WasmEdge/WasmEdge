...
///
/// \file
/// This file contains the host component instances of `wasi:filesystem@0.2.12`.
///
//===----------------------------------------------------------------------===//
#pragma once
...
using FsStatus = Expected<Runtime::Component::WitUnit, FsErrorCode>;
...
protected:
  ...
  FsStatus status(Descriptor Self, Op &&Run) const noexcept {
    auto Node = node(Self);
    if (!Node) {
      return Unexpected<FsErrorCode>(WasiComponent::getFsError(Node.error()));
    }
    if (auto Res = Run(**Node); !Res) {
      return Unexpected<FsErrorCode>(WasiComponent::getFsError(Res.error()));
    }
...
/// `[method]descriptor.write-via-stream` and `append-via-stream`
...
/// `[method]descriptor.create-directory-at` and its one-path siblings
