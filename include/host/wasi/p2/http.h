...
using HttpStatus = Expected<Runtime::Component::WitUnit, HttpError>;
using HeaderStatus = Expected<Runtime::Component::WitUnit, HeaderError>;

/// `fields`: ordered name and value pairs, immutable once handed out.
struct Fields {
  ...
  static std::unique_ptr<Fields> immutableCopy(const Fields *From);
...
/// A body the guest writes; its table entry lives as long as its parent.
struct OutgoingBody {
  std::vector<uint8_t> Data;
  bool StreamTaken = false;
  bool Finished = false;
  /// The guest dropped the handle; the entry stays for its parent.
  bool Dropped = false;
  std::optional<Fields> Trailers;
};
...
/// BodyRep names the entry of the outgoing body table, 0 for none.
...
struct IncomingRequest {
  ...
  std::optional<std::string> Authority;
  Fields Headers;
  std::unique_ptr<IncomingBody> Body;
};
...
/// A set outparam stays in its table until the embedder removes it.
struct ResponseOutparam {
  std::optional<HttpResult<OutgoingResponse>> Response;
};
...
/// The response of `outgoing-handler.handle`, set once the exchange ends.
struct FutureIncomingResponse {
  OutgoingRequest Request;
  RequestOptions Options;
  std::optional<HttpResult<std::unique_ptr<IncomingResponse>>> Result;
  bool Taken = false;
};

struct HttpHost;

/// An output stream that appends to the outgoing body BodyRep names.
class BodyOutputStream : public OutputStream {
public:
  BodyOutputStream(HttpHost &H, uint64_t Rep) noexcept
      : Http(H), BodyRep(Rep) {}
...
/// An input stream over the incoming body BodyRep names, closed at its end.
class BodyInputStream : public InputStream {
public:
  BodyInputStream(HttpHost &H, uint64_t Rep) noexcept : Http(H), BodyRep(Rep) {}
  bool ready() noexcept override { return true; }
  void addInterest(std::optional<std::chrono::steady_clock::time_point> &,
                   std::vector<PollEntry> &) noexcept override {}
...
struct HttpHost {
  ...
  /// Run the exchange of the future at Rep once its body is finished.
  Expect<void> runExchange(Runtime::Component::CallingFrame &Frame,
                           uint64_t Rep);
  /// Remove a parent of an outgoing body together with the body it names.
  void removeOutgoingRequest(uint64_t Rep) noexcept;
  void removeOutgoingResponse(uint64_t Rep) noexcept;
  void removeFutureResponse(uint64_t Rep) noexcept;
};

/// The addresses of Host at Port, resolved now.
HttpResult<std::vector<WasiP3::RawAddress>>
resolveHttpAddresses(const std::string &Host, uint16_t Port);

/// The error of a failed connection or a failed name resolution.
HttpError getConnectError(__wasi_errno_t Errno) noexcept;
HttpError getResolveError(__wasi_errno_t Errno) noexcept;

/// The HTTP/1.1 bytes of Request, and the host and port it goes to.
/// Body is null when the request has none.
HttpResult<std::vector<uint8_t>> encodeHttp11(const OutgoingRequest &Request,
                                              const OutgoingBody *Body,
                                              std::string &Host,
                                              uint16_t &Port);

/// The response out of the whole HTTP/1.1 reply.
HttpResult<std::unique_ptr<IncomingResponse>>
decodeHttp11(const std::vector<uint8_t> &Reply, bool HeadRequest);

/// Send Request in HTTP/1.1 over a new connection and read the whole response,
/// parking Frame while the socket is not ready.
...
public:
  static constexpr const char *ParamNames[] = {"self"};
  using HttpFunction::HttpFunction;
  Expect<Expected<Runtime::Component::Own<InputStream>,
                  Runtime::Component::WitUnit>>
