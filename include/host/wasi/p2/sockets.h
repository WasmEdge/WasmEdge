...
/// The datagram streams of a socket name it by its representation.
struct IncomingDatagramStream {
  uint64_t SocketRep = 0;
};
...
using NetStatus = Expected<Runtime::Component::WitUnit, NetworkError>;
...
struct SocketsHost {
  ...
  /// The socket behind a borrow, null when the handle is not live.
  TcpSocket *getSocket(TcpBorrow Self) noexcept {
    return TcpSockets.get(Self.Rep);
  }
  UdpSocket *getSocket(UdpBorrow Self) noexcept {
    return UdpSockets.get(Self.Rep);
  }
  /// The socket behind a datagram stream, null once either is gone.
  UdpSocket *getSocket(IncomingBorrow Self) noexcept {
    const IncomingDatagramStream *Stream = Incoming.get(Self.Rep);
    return Stream ? UdpSockets.get(Stream->SocketRep) : nullptr;
  }
  UdpSocket *getSocket(OutgoingBorrow Self) noexcept {
    const OutgoingDatagramStream *Stream = Outgoing.get(Self.Rep);
    return Stream ? UdpSockets.get(Stream->SocketRep) : nullptr;
  }
};
...
/// `[method]tcp-socket.start-bind` and `start-connect`: complete at once.
...
/// The seven instances of `wasi:sockets@0.2.12`, in order.
