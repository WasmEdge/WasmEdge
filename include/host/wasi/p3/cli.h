...
using StdioResult = Expected<Runtime::Component::WitUnit, CliErrorCode>;
...
/// `stdin.read-via-stream`: a host task feeds the stream from stdin.
...
/// `stdout.write-via-stream` and `stderr.write-via-stream`
...
/// `terminal-stdout.get-terminal-stdout` (and the same for `terminal-stderr`)
