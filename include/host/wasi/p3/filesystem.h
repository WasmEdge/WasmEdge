...
///
/// \file
/// This file contains the host component instances of `wasi:filesystem@0.3.1`.
///
//===----------------------------------------------------------------------===//
#pragma once
...
inline __wasi_lookupflags_t getLookupFlags(PathFlags Flags) noexcept {
...
struct NewTimestamp {
  ...
  /// The preview-1 flag bits of this timestamp as the access or modify time.
...
/// The descriptors behind the `descriptor` resource.
using DescriptorTable =
    Runtime::Component::ResourceTable<WASI::VINode,
                                      std::shared_ptr<WASI::VINode>>;
using Descriptor = Runtime::Component::Borrow<WASI::VINode>;

template <typename T> using FsResult = Expected<T, FsError>;
using FsStatus = Expected<Runtime::Component::WitUnit, FsError>;
...
template <> struct Wit<Host::WasiP3::FsError> {
  ...
  static uint32_t getCase(Host::WasiComponent::FsErrorCode Code) noexcept {
  ...
  static ComponentValVariant into(Host::WasiP3::FsError &&E) noexcept {
    const uint32_t Case = getCase(E.Code);
    if (Case == 36) {
      return WitVariant::into(
...
protected:
  ...
  node(Descriptor Self) const noexcept {
    const std::shared_ptr<WASI::VINode> *Node = Table.getHolder(Self.Rep);
    if (!Node) {
  ...
  FsStatus status(Descriptor Self, Op &&Run) const noexcept {
    ...
    if (!Node) {
      return Unexpected<FsError>(
          FsError(WasiComponent::getFsError(Node.error())));
    }
    if (auto Res = Run(**Node); !Res) {
      return Unexpected<FsError>(
          FsError(WasiComponent::getFsError(Res.error())));
    }
