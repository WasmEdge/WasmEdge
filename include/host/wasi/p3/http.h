...
using OptionsStatus =
    Expected<Runtime::Component::WitUnit, RequestOptionsError>;
...
/// A message body; streams are indices in the stream table, 0 for none.
struct Body {
  uint32_t Contents = 0;
  uint32_t TrailersEnd = 0;
...
struct Request {
  ...
  std::optional<Scheme> ReqScheme;
  std::optional<std::string> Authority;
  std::optional<RequestOptions> Options;
  Fields Headers;
  Body Content;
};
...
/// The tables and the element types of the http resources.
struct HttpHost {
  ...
  const Runtime::Instance::Component::ResourceTypeInstance *ResponseType =
      nullptr;
  /// The element types of the futures the types instance hands out.
  ...
  Body newBody(Runtime::Component::CallingFrame &Frame,
  ...
  /// The streams a consumer reads a body through; Content may be null.
  /// The bytes and the trailers of a host body move out of Content.
