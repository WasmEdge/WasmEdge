...
/// A socket address as the preview-1 layer takes it.
struct RawAddress {
...
/// The state behind a `udp-socket` handle.
struct UdpSocket {
  std::shared_ptr<WASI::VINode> Node;
  IpAddressFamily Family = IpAddressFamily::Ipv4;
...
using SockStatus = Expected<Runtime::Component::WitUnit, SocketError>;
...
/// The conversions between the WIT addresses and the preview-1 forms.
...
/// Socket options through the preview-1 layer or the native socket.
...
/// `[method]tcp-socket.listen`: a host task accepts into the stream.
...
/// `[method]tcp-socket.send`: the stream sinks into the connection.
...
/// `[method]tcp-socket.receive`: a host task feeds the stream.
