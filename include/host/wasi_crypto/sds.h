...
protected:
  // this should be a strict_lock, but unique_lock is needed to be able to
  // return it.
  std::unique_lock<mutex_type> lk_;
  T const &value_;
...
public:
   ...
   * @effects locks the mutex @c mtx, stores a reference to it and to the value
   * type @c value.
   */
  const_strict_lock_ptr(T const &val, Lockable &mtx) : lk_(mtx), value_(val) {}
  const_strict_lock_ptr(T const &val, Lockable &mtx,
                        adopt_lock_t tag) BOOST_NOEXCEPT : lk_(mtx, tag),
                                                           value_(val) {}
  /**
   * Move constructor.
   * @effects takes ownership of the mutex owned by @c other, stores a reference
   * to the mutex and the value type of @c other.
   */
...
public:
   ...
   * type @c value.
   */
  strict_lock_ptr(T &val, Lockable &mtx) : base_type(val, mtx) {}
...
template <typename SV> struct synchronized_value_strict_lock_ptr {
  typedef strict_lock_ptr<typename SV::value_type, typename SV::mutex_type>
      type;
};
template <typename SV> struct synchronized_value_strict_lock_ptr<const SV> {
  typedef const_strict_lock_ptr<typename SV::value_type,
 ...
 *
 * An object of type const_unique_lock_ptr is a unique_lock that provides a
 * const pointer access to the synchronized value type. As unique_lock controls
 * the ownership of a lockable object within a scope. Ownership of the lockable
 * object may be acquired at construction or after construction, and may be
 * transferred, after acquisition, to another const_unique_lock_ptr object.
 ...
 * pointed do not exist for the entire remaining lifetime of the
 * const_unique_lock_ptr object. The supplied Mutex type shall meet the
 * BasicLockable requirements.
 *
 * @note const_unique_lock_ptr<T, Lockable> meets the Lockable requirements.
 * If Lockable meets the TimedLockable requirements,
 * const_unique_lock_ptr<T,Lockable> also meets the TimedLockable requirements.
 *
 * @param T the value type.
 * @param Lockable the mutex type protecting the value type.
 */
template <typename T, typename Lockable = std::mutex>
class const_unique_lock_ptr : public std::unique_lock<Lockable> {
  typedef std::unique_lock<Lockable> base_type;
...
public:
   ...
   * @param mtx reference to the mutex used to protect the value.
   *
   * @requires If mutex_type is not a recursive mutex the calling thread does
   * not own the mutex.
   *
   * @effects locks the mutex @c mtx, stores a reference to it and to the value
   * type @c value.
   ...
   * @effects stores a reference to it and to the value type @c value taking
   * ownership.
   */
  const_unique_lock_ptr(T const &val, Lockable &mtx,
                        adopt_lock_t) BOOST_NOEXCEPT
   ...
   * @requires If mutex_type is not a recursive mutex the calling thread does
   * not own the mutex.
   * @effects try to lock the mutex @c mtx, stores a reference to it and to the
   * value type @c value.
   */
  const_unique_lock_ptr(T const &val, Lockable &mtx,
                        try_to_lock_t) BOOST_NOEXCEPT
...
public:
   ...
   * type @c value.
   */
  unique_lock_ptr(T &val, Lockable &mtx) : base_type(val, mtx) {}
  /**
   * @param value reference of the value to protect.
   * @param mtx reference to the mutex used to protect the value.
   * @param tag of type adopt_lock_t used to differentiate the constructor.
...
template <typename SV> struct synchronized_value_unique_lock_ptr {
  typedef unique_lock_ptr<typename SV::value_type, typename SV::mutex_type>
      type;
};
template <typename SV> struct synchronized_value_unique_lock_ptr<const SV> {
  typedef const_unique_lock_ptr<typename SV::value_type,
                                typename SV::mutex_type>
      type;
};
/**
 ...
 * @param Lockable the mutex type protecting the value type.
 */
template <typename T, typename Lockable = std::mutex> class synchronized_value {
...
public:
   ...
   * Effects: Assigns the value on a scope protected by the mutex of the rhs.
   ...
   * Assignment operator.
   *
   * Effects: Copies the underlying value on a scope protected by the two
   * mutexes. The mutex is not copied. The locks are acquired using lock, so
   * deadlock is avoided. For example, there is no problem if one thread assigns
   * a = b and the other assigns b = a.
   *
   * Return: *this
   */
   ...
   *
   * Requires: T is CopyConstructible
   * Return: A copy of the protected value obtained on a scope protected by the
   * mutex.
   *
   */
  T get() const {
   ...
   * Swap
   *
   * Effects: Swaps the data. Again, locks are acquired using lock(). The
   * mutexes are not swapped. A swap method accepts a T& and swaps the data
   * inside a critical section. This is by far the preferred method of changing
   * the guarded datum wholesale because it keeps the lock only for a short
   * time, thus lowering the pressure on the mutex.
   */
  void swap(synchronized_value &rhs) {
    ...
    // auto _ = make_unique_locks(mtx_, rhs.mtx_);
    std::unique_lock<mutex_type> lk1(mtx_, defer_lock);
    std::unique_lock<mutex_type> lk2(rhs.mtx_, defer_lock);
    lock(lk1, lk2);
  ...
  /**
   * Essentially calling a method obj->foo(x, y, z) calls the method foo(x, y,
   * z) inside a critical section as long-lived as the call itself.
   */
  strict_lock_ptr<T, Lockable> operator->() {
    return BOOST_THREAD_MAKE_RV_REF(
        (strict_lock_ptr<T, Lockable>(value_, mtx_)));
  }
  /**
   * If the synchronized_value object involved is const-qualified, then you'll
   * only be able to call const methods through operator->. So, for example,
   * vec->push_back("xyz") won't work if vec were const-qualified. The locking
   * mechanism capitalizes on the assumption that const methods don't modify
   * their underlying data.
   */
  const_strict_lock_ptr<T, Lockable> operator->() const {
    return BOOST_THREAD_MAKE_RV_REF(
        (const_strict_lock_ptr<T, Lockable>(value_, mtx_)));
  }
  /**
  ...
  template <typename R> inline R operator()(R (*fct)(value_type &)) {
    strict_lock<mutex_type> lk(mtx_);
    return fct(value_);
  }
  template <typename R>
   ...
   * As discussed, operator-> can only lock over the duration of a call, so it
   * is insufficient for complex operations. With synchronize() you get to lock
   * the object in a scoped and to directly access the object inside that scope.
   *
   * Example
   *   void fun(synchronized_value<vector<int>> & v) {
...
public:
   ...
   */
  template <typename OStream> void save(OStream &os) const {
    strict_lock<mutex_type> lk(mtx_);
    os << value_;
  }
  /**
   ...
   */
  template <typename IStream> void load(IStream &is) const {
    strict_lock<mutex_type> lk(mtx_);
    is >> value_;
  }
...
//  Comparison with T
template <typename T, typename L>
bool operator!=(synchronized_value<T, L> const &lhs,
...
template <typename... SV>
std::tuple<typename synchronized_value_strict_lock_ptr<SV>::type...>
synchronize(SV &...sv) {
  boost::lock(sv.mtx_...);
  typedef std::tuple<typename synchronized_value_strict_lock_ptr<SV>::type...>
      t_type;
  return t_type(typename synchronized_value_strict_lock_ptr<SV>::type(
      sv.value_, sv.mtx_, adopt_lock)...);
}
#else
template <typename SV1, typename SV2>
std::tuple<typename synchronized_value_strict_lock_ptr<SV1>::type,
           typename synchronized_value_strict_lock_ptr<SV2>::type>
synchronize(SV1 &sv1, SV2 &sv2) {
  boost::lock(sv1.mtx_, sv2.mtx_);
  typedef std::tuple<typename synchronized_value_strict_lock_ptr<SV1>::type,
                     typename synchronized_value_strict_lock_ptr<SV2>::type>
      t_type;
  return t_type(typename synchronized_value_strict_lock_ptr<SV1>::type(
                    sv1.value_, sv1.mtx_, adopt_lock),
                typename synchronized_value_strict_lock_ptr<SV2>::type(
                    sv2.value_, sv2.mtx_, adopt_lock));
}
...
synchronize(SV1 &sv1, SV2 &sv2, SV3 &sv3) {
  ...
  return t_type(typename synchronized_value_strict_lock_ptr<SV1>::type(
                ...
                typename synchronized_value_strict_lock_ptr<SV2>::type(
                    sv2.value_, sv2.mtx_, adopt_lock),
                typename synchronized_value_strict_lock_ptr<SV3>::type(
                    sv3.value_, sv3.mtx_, adopt_lock));
}
#endif
#endif
} // namespace boost
