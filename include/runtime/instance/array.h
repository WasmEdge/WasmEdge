...
public:
  ...
  ArrayInstance(GC::Allocator &Allocator, const ModuleInstance *ModInst,
    ...
    // on overflow leave Data == nullptr so arrayNew() traps (AccessNullArray)
    // rather than truncate the byte size into an undersized allocation that
    // std::fill then overflows.
