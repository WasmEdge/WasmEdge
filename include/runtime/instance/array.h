...
public:
  ...
  /// Get full array as a read-only span. Length-only / internal read use that
  /// does not race the marker; the mutable span accessor was removed so callers
  /// cannot bypass the coherent per-element accessors (GC::loadCoherent /
  /// GC::storeCoherent) that struct/array field mutation must go through.
