...
public:
  ...
  // managed GC object" bit (spec 4.2), mirroring TableInstance. It cannot be
  ...
  // resolve it via AST::TypeMatcher::refTypeCanHoldGCObject; callers without one
  // (e.g. the standalone C API) resolve conservatively -- see those call sites.
  ...
  /// single, fallible, centralized owner-attach check (spec 4.2), mirroring
  /// TableInstance::setAllocator. A global is never growable, so only the
  /// managed-capable hazard (a) applies: a SECOND, FOREIGN controller is
  /// rejected when this global can hold a managed GC object (a ref it stores
  /// here barriers against the wrong allocator and can be swept while still
  /// reachable). A non-managed global (e.g. a numeric or funcref global) is
  /// freely shareable: a foreign attach keeps the FIRST owner and succeeds. A
  /// same-owner re-attach is an idempotent success. The claim of an unattached
  /// global runs under a per-instance mutex so two controllers can never race
  /// it into a torn owner. The instance must stay valid until the matching
  /// removeGlobal().
  ...
  /// reporting whether THIS call NEWLY claimed ownership so a preflight-then-
  /// commit registration can reverse (detachAllocator) exactly the attaches it
  ...
  /// Resolved once at construction (spec 4.2); see the constructor.
  ...
  ValVariant getValue() const noexcept {
    ...
    // slot as one coherent transaction so a concurrent setValue on another
    // mutator can never hand back a torn pair. Numeric globals cannot form an
    // invalid pointer and keep the plain read.
  ...
  void setValue(const ValVariant &Val) noexcept {
    ...
    // Reference-typed global (round-2 A2): publish the new (type, pointer) pair
    // atomically so the marker's relaxed pointer-word load and a concurrent
    // coherent reader never observe a torn slot. Numeric globals keep the plain
    // store (no pointer word for the collector to misread).
    if (GlobType.getValType().isRefType()) {
...
private:
  ...
  /// ownership; false for an idempotent same-owner or foreign-but-shareable
  /// attach; and the managed-capable error for a rejected foreign attach. A
  /// global is never growable, so only hazard (a) (managed-capable) applies.
  /// Single source of the attach RULE shared by setAllocator/claimForRegister.
