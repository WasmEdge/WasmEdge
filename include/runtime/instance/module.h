...
public:
  ...
  /// \name R7-M3 GC-capability flag, carried on the *instance*.
  ///
...
protected:
  ...
  Expect<void> addTable(GC::Allocator &A, const AST::TableType &TType,
                        Args &&...Values) {
    std::unique_lock Lock(Mutex);
    // Resolve the table's immutable can-hold-managed bit here, at the one
    // call site that has both the table type and this module's type list
    // (spec 4.2): the heap-type index in TType is defining-module-relative,
    // so TableInstance itself cannot compute it.
    ...
    // Fallible attach (spec 4.2): a freshly created in-module table is
    // unattached, so its first attach to this module's allocator always
    // succeeds; propagate the result so every caller honors the contract.
  ...
  Expect<void> addGlobal(GC::Allocator &A, const AST::GlobalType &GType,
                         Args &&...Values) {
    std::unique_lock Lock(Mutex);
    // Resolve the global's immutable can-hold-managed bit here (spec 4.2),
    // mirroring addTable: this is the one site with both the global type and
    // this module's type list, needed to resolve a concrete heap-type index.
    const bool CanHoldManaged = AST::TypeMatcher::refTypeCanHoldGCObject(
