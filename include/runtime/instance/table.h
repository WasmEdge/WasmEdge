...
public:
  ...
  // managed GC objects" bit (spec 4.2). It cannot be computed here: a
  ...
  // barriers. This is the single, fallible, centralized owner-attach check
  // (spec 4.2): a SECOND, FOREIGN controller is rejected when the table is
  // unsafe to share across allocators, namely when it is EITHER
  ...
  Expect<void> setAllocator(GC::Allocator &A) noexcept {
    ...
    // the E2.2 registerModule walk needs it -- see claimForRegister().
  ...
  /// E2.2 registerModule ownership walk: attach to \p A exactly as setAllocator,
  /// but report whether THIS call NEWLY claimed ownership. A preflight-then-
  /// commit registration records only the newly-claimed tables so that, if a
  /// later table in the same module is rejected, it can reverse (detachAllocator)
  /// exactly the attaches it made and nothing else. Returns true when the table
  /// transitioned from unattached to \p A-owned; false for an idempotent
  /// same-owner re-attach or a foreign-but-freely-shareable attach (both leave
  /// the prior owner in place, so there is nothing to roll back); and the
  /// unchanged E2.1 two-hazard error for a rejected foreign attach.
  Expect<bool> claimForRegister(GC::Allocator &A) noexcept {
    std::lock_guard<std::mutex> Lock(AttachMutex);
    return setAllocatorLocked(A);
  }

  /// E2.2: reverse a claimForRegister()/setAllocator() ownership claim made by
  /// \p A, exactly undoing the `Allocator = &A; addTable(*this)` in
  /// setAllocatorLocked -- un-register from A's root set and clear ownership
  /// under AttachMutex. A call naming a NON-owner (a foreign, freely-shareable
  /// attach that kept the FIRST owner and registered nothing) is a no-op, so a
  /// registerModule rollback never disturbs a table it did not newly claim.
  /// Distinct from the private clearAllocator() teardown hook, which runs under
  /// A's HeapMutex and must NOT re-enter removeTable().
  ...
  /// E2.2: true iff a GC controller owns this table AND its element type holds
  /// INTERNAL GC-managed objects (struct/array/any/eq/i31/exn or a concrete
  /// struct/array type index) -- i.e. a raw mutation would place/observe a
  ...
  /// it (extern.convert_any can wrap a GC object): an externref table is the
  /// host-facing reference primitive, and storing a host reference through it is
  /// the standard C-API workflow, which must keep working. A standalone
  ...
  const uint64_t *getSizePtr() const noexcept {
    // atomic<uint64_t> is lock-free and layout-compatible with uint64_t; the AOT
    // reader loads this cell atomically (context.h getTableSize).
  ...
  /// heap object. Resolved once at construction (spec 4.2) -- not
  /// recomputable from TabType alone, since a concrete heap-type index is
  ...
  /// second cross-controller sharing hazard (spec 4.2 (b)).
  ...
  /// mutator-parking stop-the-world operation (E1.2): the grower acquires the
  ...
  /// swapped. That is what closes the interpreter concurrent-grow-vs-reader race
  /// (getRefAddr/setRefAddr read the Refs vector object a reallocating grow
  /// mutates). A standalone table (no controller) keeps the plain path -- there
  /// is no mutator registry to stop.
  ///
  /// On success, if \p OldSize is non-null, the pre-grow size is written to it
  /// from INSIDE the exclusive window (H-5): both callers push that size as the
  ...
  bool growTable(const uint64_t Count, const RefVariant &Val,
    ...
    // global stop-the-world every iteration (DoS amplification). MaxSizeCaped is
    // constant (the address-type ceiling, further capped by an explicit max) and
    // LiveSize is monotonically non-decreasing, so a grow observed over the cap
    // at the current size stays over it -- this NEVER false-rejects. Only the
    // pass->fail direction (a concurrent winner raised the min) still needs the
    // in-window re-validation, which growRefsLocked does. Read the size from the
    // atomic mirror (race-free against a grower's setLiveSize store); the max /
    // addr-type fields are immutable after construction.
    {
    ...
    // re-validates the limit a prior winner raised (spec 4.1). No root scan and
  ...
  Expect<void> setRefs(Span<const RefVariant> Slice, const uint64_t Dst,
    ...
    if (likely(Length > 0)) {
      ...
      // Per-element coherent copy (round-2 A2): each slot may be read by the
      // marker or a concurrent coherent reader, so a bulk memmove would tear.
      // Pick the direction from the actual memory positions so an overlapping
      // same-table copy reads each source slot before it can be overwritten
      // (memmove semantics); for distinct tables there is no overlap.
  ...
  Expect<void> fillRefs(const RefVariant &Val, const uint64_t Offset,
    ...
    // Per-element coherent store (round-2 A2): each slot may be read by the
    // marker or a concurrent coherent reader, so a bulk fill_n (two independent
    // word stores per slot) would let them observe a torn pair.
    for (uint64_t I = 0; I < Length; ++I) {
  ...
  Expect<RefVariant> getRefAddr(const uint64_t Idx) const noexcept {
    ...
    // Coherent read of the 128-bit ref element (round-2 A2): a concurrent
    // setRefAddr / bulk store on another mutator can never hand back a torn
    // (type, pointer) pair.
  ...
  Expect<void> setRefAddr(const uint64_t Idx, const RefVariant &Val) noexcept {
    ...
    // Publish the new (type, pointer) pair atomically (round-2 A2) so the
    // marker's relaxed pointer-word load and a concurrent coherent reader never
    // observe a torn slot.
...
private:
  ...
  /// Core of the E2.1 fallible owner-attach check (spec 4.2), assuming the
  /// caller holds AttachMutex. Returns true iff this call NEWLY claimed
  /// ownership (was unattached, now owned by \p A); false for an idempotent
  /// same-owner re-attach or a foreign-but-freely-shareable attach; and the
  /// two-hazard error for a rejected foreign attach. Kept as the single source
  /// of the attach RULE so setAllocator() and claimForRegister() cannot diverge.
  ...
  /// Reads the CURRENT min, so a grow-loser that woke owning the exclusive token
  /// re-validates the limit a prior winning grow may have raised. On success
  /// writes the pre-grow size to \p OldSize (if non-null) from inside the window
  /// (H-5). Returns false on an over-limit or allocation-failed grow, leaving
  /// Refs untouched.
  bool growRefsLocked(const uint64_t Count, const RefVariant &Val,
    ...
    const uint64_t NewSize = Refs.size() + Count;
#if defined(__EXCEPTIONS) || defined(__cpp_exceptions) || defined(_CPPUNWIND)
    try {
#endif
      if (Allocator && NewSize > Refs.capacity()) {
        // A3: a reallocating resize would free the old Refs buffer while a
        ...
        // buffers at the next stop-the-world root scan, when no mutator holds an
        // in-flight pointer into it. Reserve geometrically so repeated grows do
        // not retire a buffer on every call. All throwing work happens before
        // the old buffer is handed over, so a failure leaves Refs intact.
...
#endif
    // Report the pre-grow size from inside the exclusive window (H-5) before
  ...
  // Atomic so a concurrent AOT reader's load (context.h getTable) does not data-
  // race a reallocating growTable's republish of the base pointer (round-2 A3).
  // The pointer read is a whole value either way; atomicity satisfies the memory
  ...
  // getTableSize) does not data-race growTable's setLiveSize (round-2 A3).
  // Lock-free and layout-compatible with a plain uint64_t.
  std::atomic<uint64_t> LiveSize;
  // Resolved once at construction (spec 4.2); see canHoldManaged().
