...
public:
  ExeResult(Executor::ErrCode Status, std::string Information,
            uint64_t ExecTime = 0, uint64_t HostFuncTime = 0,
            uint64_t ExecInstrCnt = 0, uint64_t Gas = 0)
      : Status(Status), Information(Information), ExecTime(ExecTime),
        HostFuncTime(HostFuncTime), ExecInstrCnt(ExecInstrCnt), Gas(Gas) {}
  virtual ~ExeResult() noexcept = default;
  virtual void show() {
    switch (Status) {
    ...
    LOG(INFO) << std::endl
              ...
              << static_cast<uint64_t>((double)ExecInstrCnt * 1000000 /
                                       ((ExecTime) ? ExecTime : 1))
              << std::endl;
  }
