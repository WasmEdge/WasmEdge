...
private:
  ...
  Expect<int64_t> readSLEB(uint32_t MaxBits) noexcept {
    ...
    while (true) {
      ...
      if ((B & 0x80U) == 0) {
        ...
        if (Rest <= 7) {
          const uint8_t Mask =
              static_cast<uint8_t>((0x7FU >> (Rest - 1)) << (Rest - 1));
  ...
  Expect<ComponentValVariant> decodePrim(PrimValType P) noexcept {
    ...
    case PrimValType::String: {
      ...
      // `val(string)` is a `core:name`, so the payload must be valid UTF-8.
      const size_t Begin = Off;
      const size_t End = Off + static_cast<size_t>(Len);
      while (Off < End) {
        EXPECTED_TRY(readUtf8Scalar());
      }
      if (Off != End) {
        return Unexpect(ErrCode::Value::ComponentMalformedValue);
      }
  ...
  Expect<uint32_t> readUtf8Scalar() noexcept {
    ...
    uint32_t CP = 0;
    uint32_t Tail = 0;
    // The smallest scalar the sequence length is allowed to encode.
    uint32_t Least = 0;
    if (B0 < 0x80U) {
    ...
    if (CP < Least || CP >= 0x110000U || (CP >= 0xD800U && CP <= 0xDFFFU)) {
      return Unexpect(ErrCode::Value::ComponentMalformedValue);
    }
    return CP;
  }
  ...
  Expect<ComponentValVariant> decodeDef(const DefValType &D) noexcept {
    ...
    if (D.isMapTy()) {
      // (map k v) decodes like (list (tuple k v)).
      ListVal LV;
      EXPECTED_TRY(auto Count, readULEB(32));
      for (uint64_t I = 0; I < Count; ++I) {
        TupleVal Entry;
        EXPECTED_TRY(auto K, decodeVal(D.getMap().KeyTy));
        Entry.Values.push_back(std::move(K));
        EXPECTED_TRY(auto V, decodeVal(D.getMap().ValTy));
        Entry.Values.push_back(std::move(V));
        LV.Elements.push_back(makeComponentVal(std::move(Entry)));
      }
      return makeComponentVal(std::move(LV));
    }
    if (D.isTupleTy()) {
