      ...
      if (IT != nullptr) {
        ...
        for (auto &InstanceDecl : InstanceDecls) {
          if (std::holds_alternative<CoreType>(InstanceDecl)) {
            ...
            Ctx.addComponentInstanceExport(
...
struct CoreInstanceExprVisitor {
  ...
  Expect<void> operator()(const CoreInstantiate &Inst) {
    ...
    const auto &InstanceIdx = Ctx.getCoreIndexSize(CoreSort::Instance) - 1;
    for (const auto &ExportDesc : ExportDescs) {
      Ctx.addCoreInstanceExport(static_cast<uint32_t>(InstanceIdx), ExportDesc);
    }
    return {};
  }
...
struct AliasTargetVisitor {
  ...
  Expect<void> operator()(const AliasTargetExport &ATE) {
    ...
    if (std::holds_alternative<CoreSort>(S)) {
      ...
      for (const auto &Export : CoreExports) {
        if (Export.getExternalName() != Name) {
          continue;
        }

        auto ExtType = Export.getExternalType();
        std::optional<CoreSort> MappedSort = mapExternalTypeToCoreSort(ExtType);
        ...
        if (MappedSort && *MappedSort == CoreS) {
          ...
          spdlog::error(ErrCode::Value::InvalidTypeReference);
          spdlog::error(
              "AliasTargetExport: External type mapping mismatch for export '{}': expected {}, got {}"sv,
              Name, toString(CoreS),
      ...
      for (const auto &Export : ComponentExports) {
        if (Export.getExportName() != Name) {
          continue;
        }

        auto ExternDesc = Export.getExternDesc();
        std::optional<SortCase> MappedSort =
            mapExternDescToSortCase(ExternDesc);
