...
inline Expect<std::string> parseString(std::string_view Text,
  ...
  while (!Text.empty()) {
    ...
    case '\'':
      ...
      break;
    case 'u': {
      // \u{ hexdigit+ } encodes a single Unicode scalar value. Require the
      // opening '{', at least one hex digit, a closing '}', and a value that
      // is a legal scalar (<= U+10FFFF and not a surrogate U+D800..U+DFFF).
      if (Text.empty() || Text.front() != '{') {
      ...
      while (!Text.empty() && Text.front() != '}') {
        ...
        // Bound the accumulator: also prevents overflow on long inputs.
        if (CodePoint > 0x10FFFF) {
          return Unexpect(ErrCode::Value::WatMalformedString);
        }
        ++Digits;
