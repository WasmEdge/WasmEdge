...
inline Expect<int64_t> parseInt(std::string_view Text) {
  ...
  if (errno == ERANGE && *Ptr != '-') {
    ...
    // Bit-reinterpret the unsigned magnitude back into a signed value;
    // matches parseU32/parseU64 below. A direct static_cast would be
    // implementation-defined under C++17 when UVal > INT64_MAX.
    int64_t Signed;
    std::memcpy(&Signed, &UVal, sizeof(Signed));
    return Signed;
...
inline Expect<T> parseFloat(std::string_view Text, UintT ExponentMask,
  ...
  if (View.size() > 6 && View.substr(0, 6) == "nan:0x") {
    ...
    char *End = nullptr;
    errno = 0;
    // Parse into the widest unsigned integer first so payloads wider than
    // UintT (e.g. 0x100000001 for an f32 whose mantissa is only 23 bits)
    // are caught by the range check below instead of being silently
    // truncated by the cast to UintT.
    uint64_t RawPayload = std::strtoull(PayloadStr.c_str(), &End, 16);
    ...
    const uint64_t MantissaMask =
        (static_cast<uint64_t>(1) << MantissaBits) - 1;
    if (RawPayload & ~MantissaMask) {
      return Unexpect(ErrCode::Value::WatConstantOutOfRange);
    }
    UintT Payload = static_cast<UintT>(RawPayload);
