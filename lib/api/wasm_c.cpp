...
wasm::vec<byte_t> wasm::Module::serialize() const {
  return wasm::vec<byte_t>::make_uninitialized();
}
...
wasm::own<wasm::Global> wasm::Global::copy() const {
  return own<wasm::Global>();
}
wasm::own<wasm::GlobalType> wasm::Global::type() const {
  return own<wasm::GlobalType>();
}
...
wasm::own<wasm::Table> wasm::Table::copy() const { return own<wasm::Table>(); }
...
bool wasm::Table::set(Table::size_t index, const wasm::Ref *) {
  UNUSED(index);
  return false;
}
wasm::Table::size_t wasm::Table::size() const { return 0; }
bool wasm::Table::grow(wasm::Table::size_t delta, const wasm::Ref *init) {
  UNUSED(delta);
  UNUSED(init);
  return false;
}
...
wasm::own<wasm::Memory> wasm::Memory::copy() const {
  return own<wasm::Memory>();
}
wasm::own<wasm::MemoryType> wasm::Memory::type() const {
  return own<wasm::MemoryType>();
}
