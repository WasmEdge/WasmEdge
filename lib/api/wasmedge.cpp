...
// E4.0 (spec 4.4): the legacy managed-ref getter restriction predicate,
...
// exact function called below, not a hand-rederived copy of the boolean) --
// see that header for the full rationale, including the deliberate
// externref exclusion.

// E4.0: the documented canonical sentinel a restricted managed-ref getter
// returns -- a null reference of the slot's declared type \p RT (correct
// type tag, null/zeroed value bits), built the same way a defaultable
// reference-typed slot is default-initialized elsewhere (RefVariant(ValType)).
// Logs an spdlog warning: a defined-error return isn't representable in the
// bare WasmEdge_Value / WasmEdge_TableInstanceGetData signatures, so the
// warning is the only way to surface that the caller received a sentinel, not
// a live borrowed reference. Note this is also the only representable
// sentinel for a NON-nullable managed-ref slot: there is no valid non-null
// value we could synthesize (a live managed object cannot be fabricated), so
// the typed-null return here is a non-round-trippable ERROR INDICATOR, not a
// usable value -- feeding it back into WasmEdge_GlobalInstanceCreate /
// WasmEdge_TableInstanceCreateWithInit for a non-nullable slot type is caller
// misuse that will trip the construction invariant
// (isNullableRefType() || !Val.isNull()).
...
// E4.1 (spec 4.4): shared body of the producer-bearing RETAINED managed-ref
...
//
//   - For a live GC-managed (struct/array) reference, pin it as a host
//     boundary root through Allocator::retainResult -- the EXACT call
//     Executor::invoke makes for a returned GC ref (executor.cpp) -- so a
//     concurrent collection cannot reclaim it until the host releases it via
//     WasmEdge_ExecutorReleaseRef. The reference is handed back typed with its
//     resolved ABSTRACT heap type (structref/arrayref) so the by-value
//     release path (genRetainedRef -> ValType::isGCRefType) matches it by
//     type, exactly as an Invoke-returned ref is matched.
//   - A null reference, a non-managed reference (funcref), and externref are
//     returned as-is with no retention. externref is deliberately excluded
//     (mirroring the E4.0/E2.2 externref exclusion): it is the host-facing
//     reference primitive whose host-pointer round trip must keep working, and
//     an externalized GC object handed over as externref is released only via
//     releaseAllRefs, never by-value -- so retaining it here would leak a root
...
WASMEDGE_CAPI_EXPORT WasmEdge_Result WasmEdge_TableInstanceGetData(
  ...
  return wrap([&]() { return fromTabCxt(Cxt)->getRefAddr(Offset); },
              [&Data, &Cxt](auto &&Res) {
                ...
                // E4.0 (spec 4.4): a managed-ref slot -- do not hand out the
                // unretained borrowed ref just read into Res; it is not a
                // scanned host root, so return the documented sentinel
                // instead. See isRestrictedManagedRefGetterType.
...
WASMEDGE_CAPI_EXPORT WasmEdge_Result WasmEdge_TableInstanceGetDataRetained(
  ...
  return wrap(
      [&]() -> WasmEdge::Expect<WasmEdge_Value> {
        ...
        // E4.1 (spec 4.4): producer-scoped. Reject an instance owned by a
        // DIFFERENT controller up front -- before reading the slot, so a
        // foreign query never reads or roots an object this executor does not
        // own. An unattached table (Allocator == nullptr) is NOT foreign and
        // proceeds normally.
...
WASMEDGE_CAPI_EXPORT WasmEdge_Result WasmEdge_TableInstanceSetData(
  ...
  return wrap(
      [&]() -> WasmEdge::Expect<void> {
        // E2.2: refuse a raw mutation on a table a GC controller manages. The
        // C-API cannot prove the caller's controller matches the owner, so a
        // managed store here could barrier against the wrong allocator (a
        // reachable object swept). A standalone (unattached) table, or an
        // attached but non-managed table, is unaffected -- see
...
WASMEDGE_CAPI_EXPORT WasmEdge_Result WasmEdge_TableInstanceGrow(
  ...
  return wrap(
      [&]() -> WasmEdge::Expect<void> {
        // E2.2: refuse a raw grow on a table a GC controller manages. The raw
        // path cannot prove the caller's controller matches the owner, so a
        // reallocating grow could stop-the-world on / retire buffers against the
        // wrong allocator. A standalone or non-managed table is unaffected.
        if (fromTabCxt(Cxt)->isManagedByController()) {
          spdlog::error(WasmEdge::ErrCode::Value::IncompatibleImportType);
          return Unexpect(WasmEdge::ErrCode::Value::IncompatibleImportType);
        }
...
WASMEDGE_CAPI_EXPORT WasmEdge_Value WasmEdge_GlobalInstanceGetValue(
  ...
  if (Cxt) {
    ...
    // E4.0 (spec 4.4): a managed-ref slot -- do not hand out the unretained
    // borrowed ref getValue() would return; it is not added to HostRoots (see
    // getValue's doc-comment), so return the documented sentinel instead. See
...
WASMEDGE_CAPI_EXPORT WasmEdge_Result WasmEdge_GlobalInstanceGetValueRetained(
  ...
  return wrap(
      [&]() -> WasmEdge::Expect<WasmEdge_Value> {
        ...
        // DIFFERENT controller up front -- before reading the slot, so a
        // foreign query never reads or roots an object this executor does not
        // own. An unattached global (Allocator == nullptr) is NOT foreign and
        // proceeds normally.
