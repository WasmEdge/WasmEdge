...
void Statistics::dumpToLog(const Configure &Conf) const noexcept {
  ...
  if (StatConf.getStatsOutputFormat() ==
    ...
    if (!AnyStatEnabled) {
      spdlog::warn("--stats-format=json has no effect without "
                   "--enable-time-measuring, --enable-instruction-count, "
                   "or --enable-gas-measuring."sv);
      return;
    }
    std::string Json = "{\n";
    bool First = true;
    ...
    if (HasCount && HasTime) {
      ...
      const uint64_t IPSVal =
          likely(!std::isnan(IPS)) ? static_cast<uint64_t>(IPS)
