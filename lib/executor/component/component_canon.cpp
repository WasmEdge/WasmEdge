...
Expect<AST::FunctionType> ComponentExecutor::flattenFuncType(
  ...
  if (const auto &Result = FuncType.getResult(); Result.has_value()) {
    EXPECTED_TRY(flattenType(TypeInst, *Result, IsMem64, Results));
  }
...
ComponentExecutor::instantiate(Runtime::Instance::ComponentInstance &CompInst,
  ...
  for (const auto &Canon : CanonSec.getContent()) {
    ...
    case ComponentCanonOpCode::Task__return: {
      ...
      if (const auto &Result = Canon.getResult(); Result.has_value()) {
        EXPECTED_TRY(flattenType(CompInst, *Result, IsMem64, Params)
                         .map_error(ReportError));
...
Expect<void> ComponentExecutor::runCanon(const Component::CanonFunction &Canon,
  ...
  case ComponentCanonOpCode::Stream__forward:
    return runCanonStreamForward(Canon, Args, true);
  case ComponentCanonOpCode::Future__forward:
    return runCanonStreamForward(Canon, Args, false);
  case ComponentCanonOpCode::Error_context__new:
