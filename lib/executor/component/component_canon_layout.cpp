...
Expect<uint32_t> alignment(const CanonCtx &Cx,
  ...
  // range, so the code forwards directly.
...
// Maximum payload alignment across a variant's cases, 1 if no case has a
// payload.
...
Expect<uint32_t> elemSizeDef(const CanonCtx &Cx,
  ...
  // The aggregate alignment is the max of the field alignments, and the
  // trailing size aligns to it; track the max in-loop instead of re-walking.
  if (T.isRecordTy()) {
    uint32_t Off = 0u;
    uint32_t Max = 1u;
    for (const auto &F : T.getRecord().LabelTypes) {
  ...
  if (T.isVariantTy()) {
    // Variant alignment = max(disc, payload alignments); the max-align and
    // max-size come out of a single pass.
    const auto &V = T.getVariant();
    const uint32_t NumCases = static_cast<uint32_t>(V.Cases.size());
...
flattenTypeDef(const CanonCtx &Cx,
  ...
  if (T.isListTy()) {
    // no-len → [i32, i32] (ptr, len); with-len → the element flattening
    // repeated len times.
    const auto &L = T.getList();
    if (L.Len.has_value()) {
...
Expect<FlatFuncType> flattenFuncType(const CanonCtx &Cx,
  ...
  // An indirect param or result travels as one pointer into the memory the
  // canon options select, so it takes that memory's address type.
  const ValType Ptr = Cx.ptrType();

  if (Async) {
    if (IsLift) {
      // Async lift: the params spill beyond the cap, and the core function
      // returns the packed callback code, or nothing when stackful.
      if (F.Params.size() > MaxFlatParams) {
        F.Params.assign(1, Ptr);
      }
      F.Results.clear();
      if (Callback) {
      ...
      // Async lower: the params spill beyond the async cap, any result adds
      // a return-buffer pointer, and the result is the subtask state.
      if (F.Params.size() > MaxFlatAsyncParams) {
        F.Params.assign(1, Ptr);
      }
      if (!F.Results.empty()) {
        F.Params.push_back(Ptr);
      }
      F.Results.assign(1, I32T);
    }
    return F;
  }

  // Params over the cap take the indirect path in both directions,
  // collapsing to a single pointer.
  if (F.Params.size() > MaxFlatParams) {
    F.Params.assign(1, Ptr);
  }
  ...
  if (F.Results.size() > MaxFlatResults) {
    if (IsLift) {
      ...
      F.Results.assign(1, Ptr);
    } else {
      // The trailing pointer is the caller-provided out-pointer for the
      // lowered tuple; the thunk reads it from the end of the argument list.
      F.Params.push_back(Ptr);
      F.Results.clear();
    }
  }
