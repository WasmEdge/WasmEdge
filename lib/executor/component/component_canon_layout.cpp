...
uint32_t ComponentExecutor::getDiscriminantSize(uint64_t NumCases) noexcept {
  if (NumCases <= 256) {
    return 1;
  }
  if (NumCases <= 65536) {
...
// Round up to an alignment. See "include/executor/component/executor.h".
uint64_t ComponentExecutor::alignTo(uint64_t Ptr, uint32_t Align) noexcept {
...
std::vector<std::optional<ComponentValType>> ComponentExecutor::getVariantCases(
    const AST::Component::DefValType &Ty) noexcept {
