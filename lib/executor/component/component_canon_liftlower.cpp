...
// A flat slot holding a pointer or a length takes the address type of the
// memory the canon options select.
uint64_t flatPtr(const CanonCtx &Cx, const ValVariant &V) noexcept {
  return Cx.memory64() ? V.get<uint64_t>()
                       : static_cast<uint64_t>(V.get<uint32_t>());
}

ValVariant ptrSlot(const CanonCtx &Cx, uint64_t V) noexcept {
  return Cx.memory64() ? ValVariant(V) : ValVariant(static_cast<uint32_t>(V));
}

// Bit-pattern reinterpret helpers for a variant payload. C++17 has no
// std::bit_cast, so this goes through memcpy on a same-sized scratch slot.
...
// Read a `Have`-typed flat slot from VI and reinterpret it into the `Want`
// slot the variant's selected case expects.
...
// The inverse of coerceLiftSlot: widen a lowered slot from its native flat
// type `Have` into the variant's joined slot `Want`.
...
liftFlatPrim(const CanonCtx &Cx, FlatIter &VI,
  ...
  case P::String: {
    // Take (ptr, tagged_code_units) from the flat values, then decode per
    // the canon string-encoding option.
...
liftFlatDef(const CanonCtx &Cx, FlatIter &VI,
  ...
  if (T.isListTy()) {
    // with-len lifts len elements straight from the flat iterator, with no
    // ptr/len pair and no memory load.
  ...
  if (T.isVariantTy() || T.isOptionTy() || T.isResultTy()) {
    // Read the disc, coerce the prefix into the case's native flat shape,
    // then drain the unused suffix.
    size_t NumCases = 0;
...
// Two's-complement reinterpretation into the unsigned width, bucketed to
// i32 or i64.
...
lowerFlatPrim(const CanonCtx &Cx, const ComponentValVariant &V,
  ...
  case P::String: {
    // Encode per the canon string-encoding option, then push
    // (ptr, tagged_code_units).
...
lowerFlatDef(const CanonCtx &Cx, const ComponentValVariant &V,
  ...
  if (T.isListTy()) {
    // no-len reallocs, stores, and pushes (ptr, len); with-len concatenates
    // the per-element lowerings straight into the flat values.
    if (T.getList().Len.has_value()) {
      const auto &L = T.getList();
      const uint32_t Len = *L.Len;
      const auto &VC = std::get<std::shared_ptr<ValComp>>(V);
  ...
  if (T.isVariantTy() || T.isOptionTy() || T.isResultTy()) {
    // Lower the payload to its native flat shape, coerce each slot into the
    // joined shape, then tail-pad zeros.
    size_t NumCases = 0;
    uint32_t Case = 0;
    std::optional<ComponentValType> CasePayloadTy;
...
// The value an embedder hands in must hold the alternative its declared
// primitive type lowers from.
...
Expect<void> checkValueDef(const CanonCtx &Cx, const ComponentValVariant &V,
                           const AST::Component::DefValType &T) noexcept;

// Structural check of an embedder-supplied value: the lowering path reads it
// unchecked, so a mismatch would otherwise be undefined behaviour.
Expect<void> checkValue(const CanonCtx &Cx, const ComponentValVariant &V,
                        const ComponentValType &T) noexcept {
