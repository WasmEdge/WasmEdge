...
Expect<void> logSortMismatch(ErrCode::Value Code, std::string_view Sort,
...
// Whether the actual core module provides what the declared core module type
// requires. A declared import matches contravariantly, a declared export
// covariantly.
Expect<void>
matchCoreModuleType(const AST::Module &Actual,
  ...
  for (const auto &Imp : Actual.getImportSection().getContent()) {
    ...
    if (Iter == DeclImports.end()) {
      return logSortMismatch(ErrCode::Value::ComponentCoreModNotDefined,
    ...
    if (Iter->second.Kind != Imp.getExternalType()) {
      ...
      return logSortMismatch(ErrCode::Value::ComponentCoreModKindMismatch,
                             "core import"sv, Name, Provider);
    }
    if (!matchEntry(Iter->second, Idx, /*IsImport=*/true)) {
      return logSortMismatch(ErrCode::Value::ComponentCoreModWrongType,
                             "core import"sv, Name, Provider);
    }
  }
...
ComponentExecutor::instantiate(Runtime::Component::StoreManager &StoreMgr,
  ...
  for (const auto &Imp : ImportSec.getContent()) {
    ...
    case AST::Component::ExternDesc::DescType::InstanceType: {
      ...
      if (Inst == nullptr) {
        return logNotFound("instance"sv, Name, ""sv);
      }
    ...
    case AST::Component::ExternDesc::DescType::FuncType: {
      ...
      if (Func == nullptr) {
        return logNotFound("func"sv, Name, ""sv);
      }
    ...
    case AST::Component::ExternDesc::DescType::ComponentType: {
      ...
      if (Def == nullptr) {
        return logNotFound("component"sv, Name, ""sv);
      }
    ...
    case AST::Component::ExternDesc::DescType::ValueBound:
      return logNotFound("value"sv, Name, ""sv);
    case AST::Component::ExternDesc::DescType::CoreType:
      return logNotFound("core module"sv, Name, ""sv);
    default:
  ...
  for (const auto &Imp : ImportSec.getContent()) {
    ...
    case AST::Component::ExternDesc::DescType::TypeBound: {
      ...
      if (!Desc.isEqType() && TypeDef->Resource == nullptr) {
        return logSortMismatch(ErrCode::Value::ComponentImportExpectedResource,
                               "type"sv, Name, From);
      }
      CompInst.addTypeDefinition(*TypeDef);
      break;
...
Expect<void> ComponentExecutor::matchInstanceType(
  ...
  std::vector<Runtime::Instance::ComponentInstance::TypeDefinition> Local;
  ...
  for (const auto &Decl : Ty.getDecl()) {
    ...
    if (Decl.isAlias()) {
      ...
      Runtime::Instance::ComponentInstance::TypeDefinition Entry;
    ...
    case AST::Component::ExternDesc::DescType::FuncType:
      ...
      if (Inst.findComponentInstance(Name) != nullptr) {
        return logSortMismatch(ErrCode::Value::ComponentImportExpectedFunc,
                               "func"sv, Name, From);
      }
      return logNotFound("func"sv, Name, From);
    case AST::Component::ExternDesc::DescType::InstanceType:
      ...
      if (Inst.findFunction(Name) != nullptr) {
        return logSortMismatch(ErrCode::Value::ComponentImportExpectedInstance,
    ...
    case AST::Component::ExternDesc::DescType::TypeBound: {
      ...
      if (TypeDef == nullptr) {
        ...
        if (Desc.isEqType()) {
          ...
          Local.push_back(
              Idx < Local.size()
                  ? Local[Idx]
                  : Runtime::Instance::ComponentInstance::TypeDefinition{});
          break;
        }
        return logNotFound("type"sv, Name, From);
      }
      ...
      if (Desc.isEqType()) {
        ...
        if (Expected != nullptr && TypeDef->Resource != Expected) {
          return logSortMismatch(ErrCode::Value::ComponentResourceMismatched,
    ...
    case AST::Component::ExternDesc::DescType::CoreType: {
      ...
      if (Mod == nullptr) {
        if (Inst.findComponentInstance(Name) != nullptr) {
          return logSortMismatch(ErrCode::Value::ComponentImportExpectedModule,
