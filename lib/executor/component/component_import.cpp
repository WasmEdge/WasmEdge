...
// Runtime instance-shape check over export kinds, eq-bound resource
// identities, and declared core-module types.
...
matchInstanceShape(const Runtime::Instance::ComponentInstance &Provided,
  ...
  struct ShapeTy {
    ...
    const Runtime::Instance::Component::ResourceTypeRT *RT = nullptr;
  };
  ...
  for (const auto &Decl : Shape.getDecl()) {
    ...
    case DescType::TypeBound: {
      ...
      if (RT == nullptr && DT == nullptr) {
        if (Desc.isEqType()) {
          // An equality-constrained type export needs no supplier. The
    ...
    case DescType::InstanceType: {
      ...
      // The nested shape index space is component-level. When the view of
      // the importing component is available, resolve it there.
      break;
    }
...
// Pick the resolution side. The two sides differ in the sorts they provide
// and in their diagnostics, so they stay separate below.
Expect<void>
...
Expect<void> Component::Executor::instantiate(
  ...
  for (const auto &Import : ImportSec.getContent()) {
    ...
    case AST::Component::ExternDesc::DescType::TypeBound:
      // Type imports fill the type index space. An abstract resource import
      // mints a fresh opaque host identity, having no store provider.
      if (Desc.isEqType()) {
