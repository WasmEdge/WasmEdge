...
ComponentExecutor::instantiate(Runtime::Component::StoreManager &StoreMgr,
  ...
  // A taken name fails before the component is instantiated.
  if (Name.has_value() && StoreMgr.hasInstance(*Name)) {
    spdlog::error(ErrCode::Value::ComponentRegistrationConflict);
    spdlog::error(ErrInfo::InfoAST(ASTNodeAttr::Component));
  ...
  // The providers are linked before the walk, so the entry locks every store
  // it runs up front.
  for (const auto &Sec : Comp.getSections()) {
    const auto *ImportSec = std::get_if<AST::Component::ImportSection>(&Sec);
    if (ImportSec == nullptr) {
      continue;
    }
    for (const auto &Imp : ImportSec->getContent()) {
      const auto DescType = Imp.getDesc().getDescType();
      if (DescType == AST::Component::ExternDesc::DescType::InstanceType) {
        if (const auto *Inst = StoreMgr.findInstance(Imp.getName());
            Inst != nullptr) {
          CompInst->addDependency(
              *const_cast<Runtime::Instance::ComponentInstance *>(Inst));
        }
      } else if (DescType == AST::Component::ExternDesc::DescType::FuncType) {
        if (auto *Func = StoreMgr.findFunction(Imp.getName());
            Func != nullptr && Func->getComponentInstance() != nullptr) {
          CompInst->addDependency(*Func->getComponentInstance());
        }
      }
    }
  }
  ...
  EXPECTED_TRY(runEntry(
      *CompInst, Runtime::Instance::ComponentInstance::EntryKind::Instantiate,
      [this, &CompInst, &Comp, &StoreMgr]() {
        return instantiate(*CompInst, Comp, &StoreMgr, nullptr);
      }));

  if (Name.has_value()) {
    EXPECTED_TRY(registerComponent(StoreMgr, *CompInst));
  }
  return CompInst;
}
...
Expect<void> ComponentExecutor::instantiate(
  ...
  // A root instance walks its own copy of the component; a nested one walks
  // the copy of the tree it belongs to.
  ...
  // The sections instantiate in their binary order.
...
ComponentExecutor::instantiateModule(
  ...
  if (Mod.getStartSection().getContent().has_value()) {
    EXPECTED_TRY(runImplicitTask(
        &CompInst, getCurrentThread(CompInst),
        [this, &ModuleFinder, &ModInst, &Mod]() -> Expect<void> {
...
// "include/executor/component/executor.h".
Expect<std::unique_ptr<Runtime::Instance::ModuleInstance>>
ComponentExecutor::instantiateInlineExports(
    Runtime::Instance::ComponentInstance &CompInst,
