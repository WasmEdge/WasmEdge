...
ValType
repType(const Runtime::Instance::Component::ResourceTypeRT *RT) noexcept {
  return RT != nullptr && RT->RepI64 ? ValType(TypeCode::I64)
                                     : ValType(TypeCode::I32);
}

uint64_t repOf(const Runtime::Instance::Component::ResourceTypeRT *RT,
...
CanonResourceNewHostFunc::CanonResourceNewHostFunc(
    ...
    const Runtime::Instance::Component::ResourceTypeRT *RTIn) noexcept
...
Expect<void> CanonResourceDropHostFunc::run(const Runtime::CallingFrame &,
  ...
  if (Removed->Own) {
    if (RT != nullptr && RT->HostDtor) {
      ...
      // The destructor runs as an implicit synchronous task, so the built-ins
      // inside it act on the implementing instance.
      EXPECTED_TRY(Exec->getAsyncRuntime()
                       .resourceDtorCall(RT->Impl, RT->Dtor, Removed->Rep)
                       .map_error([](auto E) {
