...
ComponentExecutor::instantiate(Runtime::Instance::ComponentInstance &CompInst,
  ...
  // The start function runs as a task under the instantiation entry; the
  // embedder thread pumps until it ends.
  ...
  EXPECTED_TRY(pump(
                   [T, Root]() {
                     return T->isResolved() || Root == nullptr ||
                            Root->isEnded();
                   },
  ...
  if (Results.size() != Start.getResult()) {
    ...
    spdlog::error(ErrInfo::InfoMismatch(
        static_cast<size_t>(Start.getResult()), Results.size()));
    spdlog::error(ErrInfo::InfoAST(ASTNodeAttr::Comp_Start));
