...
AST::CustomSection createCorestack(
  ...
  for (size_t Idx = FramesSize; Idx > 0; Idx--) {
    ...
    // Entry/start frames (module start, WASI _start) have no caller instruction,
    // so From is default-constructed. Output 0/0 to satisfy format, but these
    // are not meaningful function indices.
    uint32_t Funcidx = 0;
