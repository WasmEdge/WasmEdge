...
Expect<void> Executor::execute(Runtime::StackManager &StackMgr,
  ...
  auto Dispatch = [this, &PC, &StackMgr]() -> Expect<void> {
    ...
    case OpCode::Select_t: {
      // Pop only the i32 condition; leave both candidate values on the
      // (GC-rooted) value stack and drop the unselected one in place. Popping
      // the candidates into native locals would detach a possibly-ref survivor
      // across the window, where a concurrent collection could reclaim it.
      // Stack layout (bottom..top): Val1, Val2, Cond.
    ...
    case OpCode::I8x16__shuffle: {
      ...
      uint8x16_t Out;
      std::memcpy(&Out, &Result[0], 16);
      StackMgr.emplaceTop(Out);
      return {};
    }
