...
void logArrayOOB(const ErrCode &Code, const uint32_t Idx, const uint32_t Cnt,
  ...
  if (Code == ErrCode::Value::ArrayOutOfBounds) {
    // The payload is a GCInstance::RawData, not an ArrayInstance handle; wrap
    // it (as runArrayLenOp does) instead of reinterpreting RawData as the
    // handle class, which would read RawData::ModInst as the handle's Data
    // pointer (type-confusion UB) and report a garbage length.
...
Expect<void> Executor::runStructNewOp(Runtime::StackManager &StackMgr,
  ...
  if (IsDefault) {
    ...
    // Keep the field initializers on the GC-rooted value stack across the
    // allocation: structNew copies them out before any stack mutation, but a
    // concurrent collection during the allocation must still see ref-typed
    // initializers as roots. Detaching them into a vector (via pops/popSpan)
    // would leave them unrooted and reclaimable mid-allocation.
  ...
  EXPECTED_TRY(
      ...
      arrayNewData(StackMgr, TypeIdx, DataIdx, Start, Length)
          .map_error([&](auto E) {
            ...
            // Compute the logged size in uint64_t so the diagnostic does not
            // wrap; this does not affect the 64-bit trap check in arrayNewData.
            return logError(E, Instr, [&]() {
      ...
      arrayInitData(StackMgr, Ref, TypeIdx, DataIdx, DstIdx, SrcIdx, Cnt)
          .map_error([&](auto E) {
            ...
            return logError(
                ...
                [&]() {
                  // Compute the logged size in uint64_t so the diagnostic does
                  // not wrap; the 64-bit trap check is in arrayInitData.
...
Expect<RefVariant> Executor::structNew(Runtime::StackManager &StackMgr,
  ...
  // The struct and array instances are owned by the GC allocator; each keeps a
  // pointer to its defining module instance to resolve the composite type used
  // during allocation and GC marking.
  const auto &CompType = getCompositeTypeByIdx(StackMgr, TypeIdx);
...
Expect<void> Executor::structSet(Runtime::StackManager &StackMgr,
  ...
  const auto &VType = getStructStorageTypeByIdx(StackMgr, TypeIdx, Off);
  // Only reference fields participate in the write barrier; for a numeric field
  // the barrier would read an arbitrary word as a candidate pointer (harmless
  // under the conservative design, but wasted work and a small false-retention
  // chance).
  if (VType.isRefType()) {
...
Executor::arrayInitElem(Runtime::StackManager &StackMgr, const RefVariant &Ref,
  ...
  for (uint32_t I = 0; I < Cnt; I++) {
    Allocator.writeBarrier(ElemSrc[SrcIdx + I]);
  }
  // The value has been packed.
