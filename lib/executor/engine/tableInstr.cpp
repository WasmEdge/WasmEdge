...
Expect<void> Executor::runTableSetOp(Runtime::StackManager &StackMgr,
                                     Runtime::Instance::TableInstance &TabInst,
                                     const AST::Instruction &Instr) {
  // Peek Ref and Idx; keep Ref on the (GC-rooted) value stack across
  // setRefAddr() so a concurrent collection cannot reclaim it from a native
  // local before the table write installs it. Pop the operands afterwards.
...
Executor::runTableGrowOp(Runtime::StackManager &StackMgr,
                         Runtime::Instance::TableInstance &TabInst) {
  // Peek N and the init Ref; keep Ref on the (GC-rooted) value stack across
  // growTable() so a concurrent collection cannot reclaim it from a native
  // local before the new slots are filled. Pop the operands after the grow.
  const auto AddrType = TabInst.getTableType().getLimit().getAddrType();
...
Expect<void> Executor::runTableFillOp(Runtime::StackManager &StackMgr,
                                      Runtime::Instance::TableInstance &TabInst,
                                      const AST::Instruction &Instr) {
  // Peek the length, ref_value, and offset; keep the fill Ref on the
  // (GC-rooted) value stack across fillRefs() so a concurrent collection cannot
  // reclaim it from a native local before the slots are written. Pop the
  // operands afterwards.
  const auto AddrType = TabInst.getTableType().getLimit().getAddrType();
