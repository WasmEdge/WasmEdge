...
Expect<Executor::RegisteredRootsClaim> Executor::attachRegisteredModuleRoots(
  ...
  // E1.2 caveat: only a GC-enabled executor takes ownership of a prebuilt
  // module's roots. A non-GC executor has no live collector to scan them and
  // registers exactly as before -- no walk, no attach. An empty (disarmed)
  // claim rolls back nothing and commits nothing.
  if (!GCEnabled) {
    return RegisteredRootsClaim{};
  }
  GC::Allocator &Alloc = getAllocator();

  // Preflight-all-then-commit with rollback (spec 4.2): claim every owned table
  // and global for THIS allocator, validating the two cross-controller sharing
  // hazards. Record only the instances this call NEWLY claimed; a table/global
  // that was already same-owner (idempotent) or foreign-but-freely-shareable is
  // NOT recorded, so a rollback never detaches something this call did not
  // claim. On the first rejection, reverse exactly the recorded attaches and
  // return the error WITHOUT publishing the module. The returned claim stays
  // ARMED so the caller also reverses these attaches if PUBLICATION fails.
  //
  // Coverage: the module's own owned tables and globals (OwnedTabInsts /
  // OwnedGlobInsts) -- the roots whose ownership this registration establishes.
  // Imported tables/globals belong to another module whose own registration
  // already attached them; they are not re-claimed here. A core ModuleInstance
  // has no nested-component table graph (components are separate
  // ComponentInstance objects registered via registerComponent, which this task
  // does not touch), so this walk is complete for the ModuleInstance path.
  // Armed for the whole walk: any early return below destructs it, reversing
  // exactly the attaches recorded so far.
...
Executor::registerModule(Runtime::StoreManager &StoreMgr,
  ...
  // E2.2: take ownership of the prebuilt module's GC roots (and reject an
  // unsafe cross-controller share) before publishing it.
...
Executor::registerModule(Runtime::StoreManager &StoreMgr,
                         ...
                         std::string_view Name) {
  // E2.2: take ownership of the prebuilt module's GC roots (and reject an
  // unsafe cross-controller share) before publishing it under the alias name.
  auto Claim = attachRegisteredModuleRoots(ModInst);
  if (!Claim) {
    spdlog::error(Claim.error());
    spdlog::error(ErrInfo::InfoAST(ASTNodeAttr::Module));
...
/// defining module. Both are released when the worker completes and the
/// keep-alive is destroyed -- on the worker thread, so this is correct even if
/// the caller drops the Async handle immediately (fire-and-forget).
