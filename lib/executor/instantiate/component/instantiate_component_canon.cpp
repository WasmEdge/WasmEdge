...
Expect<void> pushDefValType(Runtime::Instance::ComponentInstance &Comp,
  ...
  } else if (std::holds_alternative<FlagsTy>(DT)) {
  ...
  } else if (std::holds_alternative<OwnTy>(DT)) {
    auto O = std::get<OwnTy>(DT);
  ...
  } else if (std::holds_alternative<BorrowTy>(DT)) {
...
Expect<void> pushType(Runtime::Instance::ComponentInstance &Comp,
  ...
  } else if (std::holds_alternative<uint32_t>(VT)) {
    EXPECTED_TRY(auto const Ty, Comp.getType(std::get<uint32_t>(VT)));
