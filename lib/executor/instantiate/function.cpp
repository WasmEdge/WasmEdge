...
Expect<void> Executor::instantiate(Runtime::Instance::ModuleInstance &ModInst,
  ...
  // Capability gate: a GC-enabled executor must not run a module whose compiled
  // code lacks GC support (no cooperative safepoint poll, no shadow-root spill)
  // natively -- a concurrent collection would hang on it or miss its roots.
  // When the executor has the GC proposal enabled and the compiled code is not
  // GC-capable, deopt to interpreter execution for the whole module by taking
  // the `else` branch below, which builds function instances from the locals +
  // instructions. This is the single choke point all instantiations funnel
  // through, and the backstop for paths where the loader could not decide (its
  // Configure may differ from this executor's).
