...
Expect<void> Executor::instantiate(
  ...
  for (const auto &ImpDesc : ImportSec.getContent()) {
    ...
    case ExternalType::Table: {
      ...
      // setAllocator centralizes the cross-controller safety check (spec 4.2):
      // it is idempotent for a same-owner re-attach, keeps the first owner for a
    ...
    case ExternalType::Global: {
      ...
      // freely shareable non-managed global, and rejects sharing a
      // managed-capable global across executors (a ref this executor stores
      // there would be invisible to the owning allocator's GC and swept while
      // live). Interim guard until cross-allocator roots exist.
      if (auto Res = ImpInst->setAllocator(getAllocator()); !Res) {
        spdlog::error(ErrCode::Value::IncompatibleImportType);
        spdlog::error(
