...
Expect<void>
Executor::instantiateModule(const CoreModuleFinder &ModuleFinder,
  ...
  EXPECTED_TRY(instantiate(ModuleFinder, ModInst, ImportSec)
...
Executor::instantiate(Runtime::StoreManager &StoreMgr, const AST::Module &Mod,
  ...
  // A provider found in the store becomes a dependency of this instance.
  CoreModuleFinder ModuleFinder = [&StoreMgr,
                                   &ModInst](std::string_view ModName)
  ...
  // The instances of a failed instantiation remain referenceable.
  EXPECTED_TRY(instantiateModule(ModuleFinder, *ModInst, Mod)
