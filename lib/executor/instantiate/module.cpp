...
Executor::instantiate(Runtime::StoreManager &StoreMgr, const AST::Module &Mod,
  ...
  if (auto Res = instantiate(StoreMgr, *ModInst.get(), ImportSec); !Res) {
  ...
  if (auto Res = instantiate(StackMgr, *ModInst.get(), GlobSec); !Res) {
  ...
  // This function will always success.
  instantiate(*ModInst.get(), ExportSec);
  ...
  if (auto Res = instantiate(StackMgr, *ModInst.get(), ElemSec); !Res) {
  ...
  if (auto Res = instantiate(StackMgr, *ModInst.get(), DataSec); !Res) {
