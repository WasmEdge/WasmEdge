      ...
      if (Terminate) {
        ...
        bool Done = false;
        {
          ...
          if (GrayHead == nullptr &&
            ...
            // quiescence: quiescence proves marking finished, not that a sweep
            // for THIS phase has not already run. Two workers can both observe
            // Gray.empty() && MarkingGray and each win a fresh TerminationOwner
            // CAS in turn, so without a phase gate the second would sweep again
            // and free the live set the first just relabelled White.
            //
            // The CAS admits exactly one MarkingGray -> Sweeping transition per
            // marking phase; a failed CAS means another owner already drove it,
            // so this worker is NOT Done and must NOT sweep. ExpectedState is a
            // fresh local per attempt because compare_exchange_strong
            // overwrites it on failure.
            //
            // The CAS also closes the barrier before the sweep, while still
            // stopped: publishing Sweeping under GrayMutex BEFORE firing
            // SweepStart makes writeBarrier no-op, so a SweepStart observer
            // cannot re-lock the non-recursive GrayMutex held here.
        ...
        if (Done) {
          ...
          // published: once Idle is visible a fresh Controller::collect() can
          // win beginCycle and overwrite CollectionOwnerGen, so reading it
          // later could hand endExclusiveOp the next cycle's generation.
          ...
          // WHOLE cycle. The sweep-completing worker -- not the coordinator --
          // releases it, handing the token across the STW #2 boundary to any
          // queued grower (endExclusiveOp's reserved FIFO handoff). Only a
          // Controller-backed cycle takes the token; a standalone Allocator
          // (Ctrl == nullptr, legacyManualCollect) never did, so guard on Ctrl.
          // This is the sole worker path that releases: the stale-termination,
          // teardown-Closing, and not-quiescent branches all reset
          // TerminationOwner WITHOUT sweeping and must NOT release (either
          // another worker completes this cycle's sweep and releases, or
          // teardown intentionally leaves the token owned).
          //
...
void Allocator::markRoots(bool ScanNativeStack) noexcept {
  ...
  // with no mutator registry and no coordinator (the bare-Allocator unit
  // tests). Its root set is therefore whatever this one thread can see: the
  // native scan below covers the calling thread alone (markNativeStackRoots
  // uses current-thread APIs), and marking is not stop-the-world. A second
  // mutator running AOT code with a ref only in a register/stack slot would be
  // missed -- which is sound here precisely because a standalone allocator has
  ...
  // registers or native frames. NativeRunning/Blocked entries do not ack and
  // are scanned remotely instead (scanNonRunningRoots), which is why they must
  // keep their refs on the value stack or in a published shadow frame.
  if (ScanNativeStack) {
    markNativeStackRoots();
  }
...
void Allocator::scanSharedRoots() noexcept {
  {
    std::unique_lock<std::mutex> Locker(HeapMutex);
    // Read each table element (and InitValue) with a single relaxed
    // atomic pointer-word load, which pairs race-free with a concurrent
    // coherent setRefAddr / fillRefs / setRefs -- so no TSAN_IGNORE_READS is
    // needed here.
    for (auto T : Heaps) {
    ...
    // Free the table buffers a reallocating growTable retired since
    // the last scan. This runs under the stop-the-world root snapshot with
    // every mutator parked at a safepoint or NativeRunning, so no mutator holds
    // an in-flight pointer into a retired buffer (a table access is
    // safepoint-free). Their live refs already migrated into the current Refs
    // buffers scanned above, so dropping them loses no root.
    RetiredTableBuffers.clear();
  }
  {
    std::unique_lock<std::mutex> Locker(GlobalMutex);
    // Read each ref global's pointer word with a single relaxed
    ...
    // reference and are skipped (their plain store must not race an atomic
    // read).
    for (auto G : Globals) {
...
void Allocator::markGray(uint8_t *Pointer) noexcept {
  ...
  // and leaving dangling pointers when H is later traced (use-after-free). The
  // same lock is also what keeps a sweep from interposing -- not because the
  // sweep holds it (it does not), but because the phase gate below is read
  // under it, against a MarkingGray -> Sweeping CAS that is published under it.
  // Relying instead on "the STW #2 barrier-close quiets writeBarrier while
  // every mutator is parked" is what this used to do, and it does NOT hold: the
  // root-scan callers are not barriers, and STW #2 does not park a
  // NativeRunning mutator. See the gate.
  std::unique_lock<std::mutex> GrayLocker(GrayMutex);
  // Shade only inside a marking phase, decided HERE under GrayMutex rather than
  // trusted from the caller. writeBarrier() checks the phase before calling,
  // but that check is a fast path, not the invariant: the root-scan callers
  // (Controller::selfScanInto -> markGrayRoot / markNativeStackRoots) do not
  // check at all, and every caller's check is a TOCTOU -- the phase can advance
  // between it and this call.
  //
  // That gap is reachable. A mutator returning from a host call is
  // NativeRunning, and STW #2 deliberately never waits for that state
  // (waitForAcks skips it, because scanNonRunningRoots already scanned its
  // stacks in place). Such a mutator can observe a stop flag that is still up,
  // then run its conservative self-scan only after STW #2 closed and the sweep
  // began. Since the sweep partitions the all-object list WITHOUT AllMutex, a
  // shade landing between that unlocked colour read and the Index.erase() that
  // follows wins the White -> Gray CAS on an object the sweep has already
  // condemned and pushes it onto the gray work list; the sweep then frees it,
  // leaving a dangling pointer for the next cycle's tracer to pop.
  //
  // GrayMutex is what makes this exact rather than merely narrower: the
  // MarkingGray -> Sweeping CAS is performed under GrayMutex too, so the two
  // orderings are the only ones. Shade first and the terminating owner then
  // observes a non-empty GrayHead, declares the cycle not quiescent and does
  // NOT sweep; CAS first and this reads Sweeping and declines. Either way no
  // shade is ever concurrent with a sweep -- the precondition runSweepAndSwap()
  // documents ("nothing calls markGray concurrently here") is now an invariant
  // instead of an assumption.
  //
  // Declining costs nothing: outside a marking phase there is no mark to
  // contribute to. A condemned object is unreachable, so refusing to resurrect
  // it is the correct answer, and a genuinely live object cannot be White here
  // -- STW #1 already shaded every root, including the shadow chain of a
  // NativeRunning mutator, and anything allocated since is born gray.
  if (const GCState Phase = CurrentGCState.load(std::memory_order_acquire);
      Phase != GCState::MarkingRoot && Phase != GCState::MarkingGray) {
    return;
  }
  bool Shaded = false;
  {
