...
Allocator::Allocator()
  ...
  for (auto &Collector : Collectors) {
    Collector = std::thread([this] {
      ...
      spdlog::debug("worker {} start"sv, std::this_thread::get_id());
      ...
      while (Stop.load(std::memory_order_acquire) == false) {
        Header *H = nullptr;
        {
          ...
          if (Gray.empty()) {
            ...
            if (WorkerCount.fetch_sub(1, std::memory_order_acq_rel) > 1) {
              spdlog::debug("worker {} wait"sv, std::this_thread::get_id());
              GrayNotEmptyCV.wait(Locker);
              spdlog::debug("worker {} wake"sv, std::this_thread::get_id());
              ...
              GCCV.notify_all();
              spdlog::debug("worker {} sweep wait"sv,
                            std::this_thread::get_id());
              GCCV.wait(Locker2);
              spdlog::debug("worker {} sweep wake"sv,
                            std::this_thread::get_id());
        ...
        spdlog::debug("pop {} from gray"sv, fmt::ptr(H));
        ...
        H->IsGray.store(false, std::memory_order_relaxed);
        {
          ...
          B.emplace(H);
          spdlog::debug("move {} to black {} size:{} g:{}"sv, fmt::ptr(H),
                        fmt::ptr(&B), H->Size, H->IsGray.load());
        }
      }
      spdlog::debug("worker {} stop"sv, std::this_thread::get_id());
    });
  }
}
...
void Allocator::markGray(uint8_t *Pointer) noexcept {
  ...
  if (IsWhite) {
    spdlog::debug("mark gray {}"sv, fmt::ptr(H));
