...
bool Controller::tryBeginExclusiveOp(ExclusiveOwner::State Want,
  ...
  // the collector's acquire: a losing collection is skippable (it simply
  // returns false, matching beginCycle's "already collecting" loser), so it
  // must not queue -- see the header for why queuing a collector trips
  // endExclusiveOp.
  if (Closing.load(std::memory_order_acquire)) {
    return false;
  }
...
void Controller::restoreStateAfterFault(MutatorState BoundaryState) noexcept {
  ...
  // waitForAcks (saw NativeRunning) and scanNonRunningRoots (now Running) for
  // the same generation. gcSafepoint self-scans + acks + parks while a
  // handshake is in flight; it is a no-op when StopFlag is clear (the common
  // case -- and when the entry was never actually stranded, this is one no-op
  // safepoint plus a Running->Running store).
  for (;;) {
    gcSafepoint();
    std::lock_guard<std::mutex> L(RegistryMtx);
    if (!StopFlag.load(std::memory_order_acquire) ||
