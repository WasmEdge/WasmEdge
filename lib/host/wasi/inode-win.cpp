...
static WasiExpect<winapi::HANDLE_> getWindowsHandle(int32_t Fd) {
  ...
  if (Fd < 0) {
    spdlog::error("Invalid file descriptor: {}", Fd);
    return WasiUnexpect(__WASI_ERRNO_BADF);
  }

  auto Raw = _get_osfhandle(Fd);
  if (Raw == -1) {
    spdlog::error("Invalid file descriptor {}: _get_osfhandle failed", Fd);
    return WasiUnexpect(__WASI_ERRNO_BADF);
  }

  auto Handle = reinterpret_cast<winapi::HANDLE_>(Raw);
  if (Handle == winapi::INVALID_HANDLE_VALUE_) {
    spdlog::error("Invalid handle for FD {}: got INVALID_HANDLE_VALUE", Fd);
    return WasiUnexpect(__WASI_ERRNO_BADF);
  }
  return Handle;
}
