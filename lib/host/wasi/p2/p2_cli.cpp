...
GetStdin::body(Runtime::Component::CallingFrame &) {
  ...
  auto Stream = std::make_unique<NodeInputStream>(Host, std::move(Node),
...
GetStdout::body(Runtime::Component::CallingFrame &) {
  ...
  auto Stream = std::make_unique<NodeOutputStream>(Host, std::move(Node),
...
newCliInstances(WasiComponent::Env &Env, IoHost &Host) {
