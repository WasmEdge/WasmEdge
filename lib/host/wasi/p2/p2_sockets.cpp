...
NetworkError getError(__wasi_errno_t Errno) noexcept {
  switch (Errno) {
...
NetStatus getStatus(WASI::WasiExpect<void> Res) noexcept {
  if (!Res) {
    return Unexpected<NetworkError>(getError(Res.error()));
  }
...
ResolveAddresses::body(Runtime::Component::CallingFrame &, NetworkBorrow,
  ...
  auto Stream = std::make_unique<ResolveAddressStream>();
...
Expect<NetStatus> TcpStart::body(Runtime::Component::CallingFrame &,
  ...
  auto Sock = Sockets.getSocket(Self);
  if (!Sock) {
  ...
  if (Which == Op::Bind) {
    ...
    Sock->Pending.reset();
    if (!Res) {
      Sock->Pending = getError(Res.error());
    } else {
      Sock->St = TcpSocket::State::Bound;
    }
    return NetStatus(Runtime::Component::WitUnit{});
  }
  if (Sock->St != TcpSocket::State::Unbound &&
      Sock->St != TcpSocket::State::Bound) {
    return NetStatus(Unexpected<NetworkError>(NetworkError::InvalidState));
...
TcpAccept::body(Runtime::Component::CallingFrame &, TcpBorrow Self) {
  ...
  if (!Node) {
    return NetResult<Accepted>(fail<Accepted>(getError(Node.error())));
  }
...
TcpAddress::body(Runtime::Component::CallingFrame &, TcpBorrow Self) {
  ...
  if (!Res) {
    return NetResult<IpSocketAddress>(
        fail<IpSocketAddress>(getError(Res.error())));
  }
...
Expect<NetStatus> TcpSetU64Option::body(Runtime::Component::CallingFrame &,
  ...
  return getStatus(WasiP3::setOption(*Sock->Node, Sock->Family, Option, Value));
}
...
CreateUdpSocket::body(Runtime::Component::CallingFrame &,
  ...
  if (!Node) {
    return NetResult<Runtime::Component::Own<UdpSocket>>(
        fail<Runtime::Component::Own<UdpSocket>>(getError(Node.error())));
  }
...
UdpStream::body(Runtime::Component::CallingFrame &, UdpBorrow Self,
  ...
  if (Remote) {
    ...
    if (auto Res = Sock->Node->sockConnect(Raw.Family, Raw.bytes(), Raw.Port);
        !Res) {
      return NetResult<Streams>(fail<Streams>(getError(Res.error())));
    }
  }
  Sock->Remote = std::move(Remote);
  auto In = std::make_unique<IncomingDatagramStream>();
  In->SocketRep = Self.Rep;
  auto OutStream = std::make_unique<OutgoingDatagramStream>();
  OutStream->SocketRep = Self.Rep;
...
IncomingReceive::body(Runtime::Component::CallingFrame &, IncomingBorrow Self,
                      uint64_t MaxResults) {
  UdpSocket *Sock = Sockets.getSocket(Self);
  if (!Sock) {
  ...
  while (Datagrams.size() < MaxResults && Sockets.Io.readable(*Sock->Node)) {
    ...
    if (!Res) {
      ...
      if (Datagrams.empty()) {
        return NetResult<std::vector<IncomingDatagram>>(
            fail<std::vector<IncomingDatagram>>(getError(Res.error())));
      }
      break;
    }
    auto From = WasiP3::getSocketAddress(Family, Bytes, WasiP3::getPort(Port));
...
IncomingSubscribe::body(Runtime::Component::CallingFrame &,
                        IncomingBorrow Self) {
  const UdpSocket *Sock = Sockets.getSocket(Self);
  const bool Active = Sock != nullptr;
  return Sockets.Io.readySource(Active ? Sock->Node : nullptr, Active, false);
}
...
OutgoingCheckSend::body(Runtime::Component::CallingFrame &,
  ...
  return NetResult<uint64_t>(Sockets.Io.writable(*Sock->Node) ? UINT64_C(1024)
                                                              : UINT64_C(0));
}
...
OutgoingSend::body(Runtime::Component::CallingFrame &, OutgoingBorrow Self,
  ...
  for (auto &D : Datagrams) {
    ...
    if (!Res) {
      ...
      if (Sent == 0) {
        return NetResult<uint64_t>(fail<uint64_t>(getError(Res.error())));
      }
      break;
    }
    ++Sent;
...
OutgoingSubscribe::body(Runtime::Component::CallingFrame &,
  ...
  const UdpSocket *Sock = Sockets.getSocket(Self);
  const bool Active = Sock != nullptr;
  return Sockets.Io.readySource(Active ? Sock->Node : nullptr, Active, true);
}
...
newSocketsInstances(SocketsHost &Sockets) {
