...
CliErrorCode getErrorCode(__wasi_errno_t Errno) noexcept {
  switch (Errno) {
...
ReadViaStream::body(Runtime::Component::CallingFrame &Frame) {
  ...
  Frame.spawn([Node, Data, Done](
    ...
    while (true) {
      ...
      if (!*Read) {
        Result = Unexpected<CliErrorCode>(getErrorCode(Read->error()));
        break;
      }
      if (**Read == 0) {
        break;
    ...
    Spawned.getExecutor().drop(
        *Data, Runtime::Instance::Component::Stream::End::Writable);
    Spawned.getExecutor().writeDetached(
        Spawned, Done,
...
WriteViaStream::body(Runtime::Component::CallingFrame &Frame,
  ...
  auto Done = Frame.getExecutor().newHostStream(Frame, true, ResultType);
  Runtime::Component::Future<StdioResult> DoneFuture{Done};
  ...
  Frame.getExecutor().sink(
      ...
      [Node, Failure](Span<const uint8_t> Bytes) {
        ...
        if (auto Res = writeAll(*Node, Bytes); !Res) {
          *Failure = getErrorCode(Res.error());
          return false;
        }
        return true;
      },
