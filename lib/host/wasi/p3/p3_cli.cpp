...
CliErrorCode getError(__wasi_errno_t Errno) noexcept {
  switch (Errno) {
...
Expect<void> Exit::body(
    ...
    Expected<Runtime::Component::WitUnit, Runtime::Component::WitUnit> Status) {
...
ReadViaStream::body(Runtime::Component::CallingFrame &Frame) {
  ...
  std::tuple<Runtime::Component::Stream<uint8_t>,
      ...
      Ends{Runtime::Component::Stream<uint8_t>{Data.second},
           Runtime::Component::Future<StdioResult>{Done.second}};
  if (!Node) {
    Frame.getExecutor().dropStreamHandle(Frame, Data.first);
    resolveLater<StdioResult>(Frame, Done.first,
  ...
  Frame.spawn([Node, Data, Done](
    ...
    StdioResult Result{Runtime::Component::WitUnit{}};
    while (true) {
      EXPECTED_TRY(WasiComponent::waitReady(Spawned, *Node, false));
      std::array<Span<uint8_t>, 1> IOVs{Span<uint8_t>(Buffer)};
      __wasi_size_t Count = 0;
      if (auto Res = Node->fdRead(IOVs, Count); !Res) {
        Result = Unexpected<CliErrorCode>(getError(Res.error()));
        break;
      }
      if (Count == 0) {
        break;
    ...
    resolveLater<StdioResult>(Spawned, Done.first, std::move(Result));
    return {};
  });
  return Ends;
}
...
WriteViaStream::body(Runtime::Component::CallingFrame &Frame,
  ...
  auto Done = Frame.getExecutor().newHostStream(Frame, true, ResultType);
  Runtime::Component::Future<StdioResult> DoneFuture{Done.second};
  Frame.spawn([Node, In, Done](
                  Runtime::Component::CallingFrame &Spawned) -> Expect<void> {
    CliErrorCode Failure = CliErrorCode::Io;
    ...
    resolveLater<StdioResult>(
        Spawned, Done.first,
        Failed ? StdioResult(Unexpected<CliErrorCode>(Failure))
               : StdioResult(Runtime::Component::WitUnit{}));
    return {};
  });
  return DoneFuture;
}
...
newCliInstances(WasiComponent::Env &Env) {
