...
FsError getError(__wasi_errno_t Errno) noexcept {
  return FsError(WasiComponent::getFsError(Errno));
}

DescriptorType getType(__wasi_filetype_t Type) noexcept {
  DescriptorType T;
  switch (Type) {
...
DescriptorStat getStat(const __wasi_filestat_t &S) noexcept {
  DescriptorStat Out;
  Out.Type = getType(S.filetype);
...
DescriptorReadViaStream::body(Runtime::Component::CallingFrame &Frame,
  ...
  Frame.spawn([Node, Data, Done, Offset](
    ...
    while (true) {
      ...
      EXPECTED_TRY(
          ...
          Spawned.getExecutor().writeStream(
              Spawned, Data.first, Span<const uint8_t>(Buffer.data(), Count)));
      if (Outcome.first !=
    ...
    resolveLater<FsStatus>(Spawned, Done.first, std::move(Status));
    return {};
  });
  return Ends;
}
...
Expect<Runtime::Component::Future<FsStatus>> DescriptorWriteViaStream::body(
  ...
  auto Done = Frame.getExecutor().newHostStream(Frame, true, ResultType);
  Runtime::Component::Future<FsStatus> DoneFuture{Done.second};
  auto Node = node(Self).value_or(nullptr);
  Frame.spawn([Node, In, Done, Offset](
                  Runtime::Component::CallingFrame &Spawned) -> Expect<void> {
    FsError Failure = FsErrorCode::BadDescriptor;
    uint64_t Pos = Offset;
    EXPECTED_TRY(
        const bool Failed,
        Spawned.getExecutor().drainStream(
            Spawned, In, [&Node, &Failure, &Pos](Span<const uint8_t> Bytes) {
              if (!Node) {
                return false;
              }
    ...
    resolveLater<FsStatus>(Spawned, Done.first,
                           Failed ? FsStatus(Unexpected<FsError>(Failure))
                                  : FsStatus(Runtime::Component::WitUnit{}));
    return {};
  });
  return DoneFuture;
}
...
DescriptorAppendViaStream::body(Runtime::Component::CallingFrame &Frame,
  ...
  Runtime::Component::Future<FsStatus> DoneFuture{Done.second};
  auto Node = node(Self).value_or(nullptr);
  Frame.spawn([Node, In, Done](
                  Runtime::Component::CallingFrame &Spawned) -> Expect<void> {
    FsError Failure = FsErrorCode::BadDescriptor;
    EXPECTED_TRY(const bool Failed,
                 Spawned.getExecutor().drainStream(
                     Spawned, In, [&Node, &Failure](Span<const uint8_t> Bytes) {
                       if (!Node) {
                         return false;
                       }
...
DescriptorGetFlags::body(Runtime::Component::CallingFrame &, Descriptor Self) {
  ...
  if (!Node) {
    return FsResult<DescriptorFlags>(
        Unexpected<FsError>(getError(Node.error())));
  }
  __wasi_fdstat_t Stat;
  std::memset(&Stat, 0, sizeof(Stat));
  if (auto Res = (*Node)->fdFdstatGet(Stat); !Res) {
    return FsResult<DescriptorFlags>(
        Unexpected<FsError>(getError(Res.error())));
  }
  uint32_t Flags = 0;
...
DescriptorGetType::body(Runtime::Component::CallingFrame &, Descriptor Self) {
  ...
  auto Stat = WasiComponent::getFdFilestat(**Node);
  if (!Stat) {
    return FsResult<DescriptorType>(
        Unexpected<FsError>(getError(Stat.error())));
  }
  return FsResult<DescriptorType>(getType(Stat->filetype));
}
...
DescriptorReadDirectory::body(Runtime::Component::CallingFrame &Frame,
                              Descriptor Self) {
  auto Data = Frame.getExecutor().newHostStream(Frame, false, EntryType);
  auto Done = Frame.getExecutor().newHostStream(Frame, true, ResultType);
  std::tuple<Runtime::Component::Stream<DirectoryEntry>,
             Runtime::Component::Future<FsStatus>>
      Ends{Runtime::Component::Stream<DirectoryEntry>{Data.second},
           Runtime::Component::Future<FsStatus>{Done.second}};
  auto Node = node(Self).value_or(nullptr);
  if (!Node) {
    Frame.getExecutor().dropStreamHandle(Frame, Data.first);
    ...
    while (More) {
      ...
      while (Off + sizeof(__wasi_dirent_t) <= Size) {
        ...
        if (Name != "." && Name != "..") {
          Entries.push_back(Runtime::Component::Wit<DirectoryEntry>::into(
              DirectoryEntry{getType(Dirent.d_type), std::move(Name)}));
        }
      }
      if (Off == 0 && Size > 0) {
      ...
      if (!Entries.empty()) {
        EXPECTED_TRY(auto Outcome,
                     Spawned.getExecutor().writeStream(Spawned, Data.first,
                                                       std::move(Entries)));
...
DescriptorCreateDirectoryAt::body(Runtime::Component::CallingFrame &,
  ...
  auto Node = node(Self);
  if (!Node) {
    return FsStatus(Unexpected<FsError>(getError(Node.error())));
  }
  if (auto Res = WASI::VINode::pathCreateDirectory(*Node, Path); !Res) {
    return FsStatus(Unexpected<FsError>(getError(Res.error())));
  }
  return FsStatus(Runtime::Component::WitUnit{});
}
...
DescriptorStatFunc::body(Runtime::Component::CallingFrame &, Descriptor Self) {
  ...
  return FsResult<DescriptorStat>(getStat(*Stat));
}
...
DescriptorStatAt::body(Runtime::Component::CallingFrame &, Descriptor Self,
  ...
  auto Stat =
      WasiComponent::getPathFilestat(*Node, getLookupFlags(Flags), Path);
  if (!Stat) {
    return FsResult<DescriptorStat>(
        Unexpected<FsError>(getError(Stat.error())));
  }
...
Expect<FsStatus> DescriptorSetTimesAt::body(Runtime::Component::CallingFrame &,
  ...
  if (auto Res = WASI::VINode::pathFilestatSetTimes(
          *Node, Path, getLookupFlags(Flags), Access.nanos(),
          Modification.nanos(),
          static_cast<__wasi_fstflags_t>(Access.fstFlags(true) |
                                         Modification.fstFlags(false)));
      !Res) {
...
DescriptorOpenAt::body(Runtime::Component::CallingFrame &, Descriptor Self,
  ...
  auto Opened = WASI::VINode::pathOpen(
      *Node, Path, getLookupFlags(Flags), static_cast<__wasi_oflags_t>(OFlags),
      static_cast<__wasi_rights_t>(Rights), AllRights,
      static_cast<__wasi_fdflags_t>(FdFlags));
  if (!Opened) {
    return FsResult<Runtime::Component::Own<WASI::VINode>>(
        Unexpected<FsError>(getError(Opened.error())));
  }
  return FsResult<Runtime::Component::Own<WASI::VINode>>(
...
DescriptorReadlinkAt::body(Runtime::Component::CallingFrame &, Descriptor Self,
  ...
  auto Node = node(Self);
  if (!Node) {
    return FsResult<std::string>(Unexpected<FsError>(getError(Node.error())));
  }
  std::vector<char> Buffer(4096);
  __wasi_size_t Read = 0;
  if (auto Res = WASI::VINode::pathReadlink(*Node, Path, Buffer, Read); !Res) {
    return FsResult<std::string>(Unexpected<FsError>(getError(Res.error())));
  }
...
Expect<bool> DescriptorIsSameObject::body(Runtime::Component::CallingFrame &,
  ...
  auto SA = WasiComponent::getFdFilestat(**A);
  auto SB = WasiComponent::getFdFilestat(**B);
  if (!SA || !SB) {
    return false;
  }
...
newFilesystemInstances(WasiComponent::Env &Env) {
