...
void resolveLater(
  ...
  Frame.getExecutor().writeDetached(
      Frame, Done, Runtime::Component::Wit<FsStatus>::into(std::move(Status)));
}
    ...
    while (true) {
      ...
      EXPECTED_TRY(
          ...
          Spawned.getExecutor().write(
              Spawned, *Data, Span<const uint8_t>(Buffer.data(), Count)));
      if (Outcome.first !=
...
Expect<Runtime::Component::Future<FsStatus>> DescriptorWriteViaStream::body(
  ...
  auto Done = Frame.getExecutor().newHostStream(Frame, true, ResultType);
  Runtime::Component::Future<FsStatus> DoneFuture{Done};
  auto Node = Table.get(Self.Rep);
...
DescriptorReadDirectory::body(Runtime::Component::CallingFrame &Frame,
  ...
  std::tuple<Runtime::Component::Stream<DirectoryEntry>,
             Runtime::Component::Future<FsStatus>>
      Ends{Runtime::Component::Stream<DirectoryEntry>{Data},
           Runtime::Component::Future<FsStatus>{Done}};
  auto Node = Table.get(Self.Rep);
  if (!Node) {
    Frame.getExecutor().drop(
  ...
  Frame.spawn([Node, Data, Done](
    ...
    while (More) {
      ...
      if (!Entries.empty()) {
        EXPECTED_TRY(auto Outcome, Spawned.getExecutor().write(
                                       Spawned, *Data, std::move(Entries)));
        if (Outcome.first !=
            Runtime::Instance::Component::Stream::Result::Completed) {
          break;
        }
