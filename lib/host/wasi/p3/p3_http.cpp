...
HttpHost::newBody(Runtime::Component::CallingFrame &Frame,
  ...
  auto NewBody = std::make_shared<Body>();
  if (Contents) {
    NewBody->Contents = std::move(Contents->Shared);
  }
  NewBody->TrailersEnd = std::move(Trailers.Shared);
  NewBody->Done = Frame.getExecutor().newHostStream(Frame, true, StatusType);
  Done = Runtime::Component::Future<HttpStatus>{NewBody->Done};
  return NewBody;
}

Expect<BodyEnds>
HttpHost::consume(Runtime::Component::CallingFrame &Frame,
                  const std::shared_ptr<Body> &Content,
  ...
  auto DropResult = [&Frame, &Res]() {
    auto End = Runtime::Instance::Component::Stream::from(Res.Shared);
    Frame.getExecutor().drop(
        *End, Runtime::Instance::Component::Stream::End::Readable);
  };
  if (!Content || Content->Consumed) {
    ...
    BodyEnds Ends{Runtime::Component::Stream<uint8_t>{Data},
                  Runtime::Component::Future<TrailersResult>{Trailers}};
    Frame.getExecutor().drop(
        *Data, Runtime::Instance::Component::Stream::End::Writable);
    Frame.getExecutor().writeDetached(
        Frame, Trailers,
        Runtime::Component::Wit<TrailersResult>::into(
  ...
  if (!Content->FromHost) {
    ...
    if (!Content->Done) {
      ...
      auto Outcome = Runtime::Instance::Component::Stream::from(Res.Shared);
      auto Done = Content->Done;
      Frame.spawn([Outcome, Done](Runtime::Component::CallingFrame &Spawned)
                      -> Expect<void> {
        std::vector<ComponentValVariant> Vals;
        EXPECTED_TRY(Spawned.getExecutor().read(Spawned, *Outcome, Vals, 1));
        if (Vals.empty()) {
          Spawned.getExecutor().drop(
              *Done, Runtime::Instance::Component::Stream::End::Writable);
          return {};
        }
        Spawned.getExecutor().writeDetached(Spawned, Done, std::move(Vals[0]));
        return {};
      });
    }
    Runtime::Component::Stream<uint8_t> Data;
    if (Content->Contents) {
      Data.Shared = Content->Contents;
    } else {
      auto Empty = Frame.getExecutor().newHostStream(
          Frame, false, ComponentValType(ComponentTypeCode::U8));
      Data.Shared = Empty;
      Frame.getExecutor().drop(
          *Empty, Runtime::Instance::Component::Stream::End::Writable);
    }
  ...
  if (Content->Trailers) {
    Value = std::optional<Runtime::Component::Own<Fields>>{
        Runtime::Component::Own<Fields>{FieldsTable.add(Content->Trailers)}};
  }
  auto Payload = std::make_shared<ComponentValVariant>(
      Runtime::Component::Wit<TrailersResult>::into(std::move(Value)));
  Frame.spawn([Content, Data, Trailers, Payload](
                  Runtime::Component::CallingFrame &Spawned) -> Expect<void> {
    if (!Content->Data.empty()) {
      EXPECTED_TRY(Spawned.getExecutor().write(Spawned, *Data, Content->Data));
    }
    Spawned.getExecutor().drop(
        *Data, Runtime::Instance::Component::Stream::End::Writable);
    Spawned.getExecutor().writeDetached(Spawned, Trailers, std::move(*Payload));
    return {};
  });
  return Ends;
}
...
ClientSend::body(Runtime::Component::CallingFrame &Frame,
  ...
  auto ReqData = Http.Requests.get(Req.Rep);
  ...
  Outgoing.Headers = ReqData->Headers;
  ...
  if (Content->FromHost) {
    ...
    if (Content->Contents) {
      auto In = Runtime::Instance::Component::Stream::from(Content->Contents);
      std::vector<uint8_t> Chunk;
      while (true) {
        EXPECTED_TRY(auto Got,
                     Frame.getExecutor().read(Frame, *In, Chunk, 65536));
      ...
      Frame.getExecutor().drop(
          *In, Runtime::Instance::Component::Stream::End::Readable);
    ...
    if (Content->TrailersEnd) {
      auto In =
          Runtime::Instance::Component::Stream::from(Content->TrailersEnd);
      std::vector<ComponentValVariant> Vals;
      EXPECTED_TRY(Frame.getExecutor().read(Frame, *In, Vals, 1));
      if (Frame.isCancelled()) {
        return HttpResult<Runtime::Component::Own<Response>>(
            Unexpected<HttpError>(HttpError::internal("cancelled")));
      }
  ...
  if (ReqData->Options) {
    Opts.ConnectTimeout = ReqData->Options->ConnectTimeout;
    Opts.FirstByteTimeout = ReqData->Options->FirstByteTimeout;
    Opts.BetweenBytesTimeout = ReqData->Options->BetweenBytesTimeout;
  }
  ...
  WriteStatus(HttpStatus(Runtime::Component::Unit{}));
  auto &Incoming = ***Reply;
