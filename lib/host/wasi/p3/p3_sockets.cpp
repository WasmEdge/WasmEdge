...
SocketError getError(__wasi_errno_t Errno) noexcept {
  switch (Errno) {
...
LookupError getLookupError(__wasi_errno_t Errno) noexcept {
  switch (Errno) {
...
__wasi_address_family_t getWasiFamily(IpAddressFamily Family) noexcept {
...
IpAddress getAddress(__wasi_address_family_t Family,
...
uint16_t getPort(uint16_t Wire) noexcept {
  uint8_t Bytes[2];
  std::memcpy(Bytes, &Wire, sizeof(Bytes));
  return static_cast<uint16_t>((Bytes[0] << 8) | Bytes[1]);
}

IpSocketAddress getSocketAddress(__wasi_address_family_t Family,
                                 Span<const uint8_t> Bytes,
                                 uint16_t Port) noexcept {
  const IpAddress Addr = getAddress(Family, Bytes);
  IpSocketAddress S;
...
WASI::WasiExpect<IpSocketAddress> nodeAddress(WASI::VINode &Node,
  ...
  return getSocketAddress(Family, Bytes, Port);
}
...
SockResult<IpSocketAddress> queryAddress(WASI::VINode &Node,
  ...
  if (!Res) {
    return Unexpected<SocketError>(getError(Res.error()));
  }
  return *Res;
}
...
SockStatus getStatus(WASI::WasiExpect<void> Res) noexcept {
  if (!Res) {
    return Unexpected<SocketError>(getError(Res.error()));
  }
...
TcpCreate::body(Runtime::Component::CallingFrame &, IpAddressFamily Family) {
  auto Node = WASI::VINode::sockOpen(getWasiFamily(Family),
  ...
  if (!Node) {
    return SockResult<Runtime::Component::Own<TcpSocket>>(
        Unexpected<SocketError>(getError(Node.error())));
  }
...
Expect<SockStatus> TcpBind::body(Runtime::Component::CallingFrame &,
  ...
  return getStatus(Sock->Node->sockBind(Raw.Family, Raw.bytes(), Raw.Port));
}
...
Expect<SockStatus> TcpConnect::body(Runtime::Component::CallingFrame &Frame,
  ...
  EXPECTED_TRY(auto Res,
               WasiComponent::sockConnect(Frame, *Sock->Node, Raw.Family,
                                          Raw.bytes(), Raw.Port));
  if (Frame.isCancelled()) {
    return SockStatus(Runtime::Component::WitUnit{});
  }
...
TcpListen::body(Runtime::Component::CallingFrame &Frame, TcpBorrow Self) {
  ...
  if (auto Res = Sock->Node->sockListen(static_cast<int32_t>(Sock->Backlog));
    ...
    return SockResult<
        Runtime::Component::Stream<Runtime::Component::Own<TcpSocket>>>(
        Unexpected<SocketError>(getError(Res.error())));
  }
  Sock->Listening = true;
  auto Data = Frame.getExecutor().newHostStream(Frame, false, SocketType);
  ...
  Frame.spawn([Node, Data, Family, Sockets](
                  Runtime::Component::CallingFrame &Spawned) -> Expect<void> {
    while (true) {
      EXPECTED_TRY(auto Res, WasiComponent::sockAccept(Spawned, *Node));
      if (!Res) {
        break;
      }
      ...
      Conn->Family = Family;
      const uint64_t Rep = Sockets->add(std::move(Conn));
      std::vector<ComponentValVariant> One{
          Runtime::Component::Wit<Runtime::Component::Own<TcpSocket>>::into(
              Runtime::Component::Own<TcpSocket>{Rep})};
      EXPECTED_TRY(auto Outcome, Spawned.getExecutor().writeStream(
                                     Spawned, Data.first, std::move(One)));
      if (Outcome.first !=
          Runtime::Instance::Component::StreamInstance::CopyResult::Completed) {
        // Nobody took the connection: its socket goes with it.
        Sockets->remove(Rep);
        break;
      }
    }
...
TcpSend::body(Runtime::Component::CallingFrame &Frame, TcpBorrow Self,
  ...
  auto Done = Frame.getExecutor().newHostStream(Frame, true, ResultType);
  Runtime::Component::Future<SockStatus> DoneFuture{Done.second};
  const uint64_t Rep = Self.Rep;
  TcpTable *Sockets = &Table;
  Frame.spawn([Rep, Sockets, In, Done](
                  Runtime::Component::CallingFrame &Spawned) -> Expect<void> {
    auto &Exec = Spawned.getExecutor();
    std::vector<uint8_t> Chunk;
    SockStatus Status{Runtime::Component::WitUnit{}};
    while (true) {
      EXPECTED_TRY(auto Outcome, Exec.readStream(Spawned, In, Chunk, 65536));
      if (Outcome.second > 0) {
        const TcpSocket *Sock = Sockets->get(Rep);
        if (!Sock) {
          Status = Unexpected<SocketError>(SocketError::InvalidState);
          break;
        }
        const std::shared_ptr<WASI::VINode> Node = Sock->Node;
        EXPECTED_TRY(auto Sent, WasiComponent::sockSend(Spawned, *Node, Chunk));
        if (!Sent) {
          Status = Unexpected<SocketError>(getError(Sent.error()));
          break;
        }
      }
      if (Outcome.first ==
    ...
    Exec.dropStreamHandle(Spawned, In);
    resolveLater<SockStatus>(Spawned, Done.first, std::move(Status));
    return {};
  });
  return DoneFuture;
}
...
TcpReceive::body(Runtime::Component::CallingFrame &Frame, TcpBorrow Self) {
  ...
  std::tuple<Runtime::Component::Stream<uint8_t>,
             Runtime::Component::Future<SockStatus>>
      Ends{Runtime::Component::Stream<uint8_t>{Data.second},
           Runtime::Component::Future<SockStatus>{Done.second}};
  auto Sock = Table.get(Self.Rep);
  if (!Sock) {
    Frame.getExecutor().dropStreamHandle(Frame, Data.first);
    resolveLater<SockStatus>(
        Frame, Done.first,
        Unexpected<SocketError>(SocketError(SocketError::InvalidState)));
    return Ends;
  }
  auto Node = Sock->Node;
  Frame.spawn([Node, Data, Done](
    ...
    SockStatus Status{Runtime::Component::WitUnit{}};
    while (true) {
      EXPECTED_TRY(auto Read, WasiComponent::sockRecv(Spawned, *Node, Buffer));
      if (!Read) {
        Status = Unexpected<SocketError>(getError(Read.error()));
        break;
      }
      if (*Read == 0) {
        break;
      ...
      EXPECTED_TRY(
          ...
          Spawned.getExecutor().writeStream(
              Spawned, Data.first, Span<const uint8_t>(Buffer.data(), *Read)));
      if (Outcome.first !=
          Runtime::Instance::Component::StreamInstance::CopyResult::Completed) {
        break;
      }
...
Expect<SockStatus> TcpSetBoolOption::body(Runtime::Component::CallingFrame &,
  ...
  return getStatus(setOption(*Sock->Node, Sock->Family, Option, Value ? 1 : 0));
}
...
Expect<SockStatus> TcpSetU64Option::body(Runtime::Component::CallingFrame &,
  ...
  return getStatus(setOption(*Sock->Node, Sock->Family, Option, Value));
}
...
Expect<SockStatus> UdpSend::body(Runtime::Component::CallingFrame &Frame,
  ...
  while (true) {
    ...
    auto Res = Sock->Node->sockSendTo(IOVs, static_cast<__wasi_siflags_t>(0),
                                      Raw.Family, Raw.bytes(), Raw.Port, Sent);
    if (Res || !WasiComponent::isPending(Res.error())) {
      return getStatus(Res);
    }
    EXPECTED_TRY(WasiComponent::waitReady(Frame, *Sock->Node, true));
    if (Frame.isCancelled()) {
      return SockStatus(Runtime::Component::WitUnit{});
    }
  }
...
UdpReceive::body(Runtime::Component::CallingFrame &Frame, UdpBorrow Self) {
  ...
  while (true) {
    ...
    std::array<uint8_t, 16> Bytes{};
    uint16_t Port = 0;
    auto Res = Node->sockRecvFrom(IOVs, static_cast<__wasi_riflags_t>(0),
                                  &Family, Bytes, &Port, Count, RoFlags);
    if (!Res && WasiComponent::isPending(Res.error())) {
      EXPECTED_TRY(WasiComponent::waitReady(Frame, *Node, false));
      if (Frame.isCancelled()) {
        return SockResult<std::tuple<std::vector<uint8_t>, IpSocketAddress>>(
            Unexpected<SocketError>(SocketError::InvalidState));
      }
    ...
    IpSocketAddress From = getSocketAddress(Family, Bytes, getPort(Port));
    if (Wanted.has_value()) {
...
resolveHostname(const std::string &Name) noexcept {
  ...
  std::array<Span<char>, Max> DataSpans;
  std::array<Span<char>, Max> NameSpans;
  ...
  // Only the addresses matter: one socket type keeps the duplicates out.
  Hint.ai_socktype = __WASI_SOCK_TYPE_SOCK_STREAM;
  __wasi_size_t Count = 0;
  EXPECTED_TRY(WASI::VINode::getAddrinfo(
      Name, "", Hint, Max, InfoPtrs, AddrPtrs, DataSpans, NameSpans, Count));
  std::vector<IpAddress> Addresses;
  for (uint32_t I = 0; I < Count; ++I) {
    const auto &Addr = Addrs[I];
    const auto *Bytes = reinterpret_cast<const uint8_t *>(Data[I].data());
    // sa_data holds the port, then the address (after the IPv6 flow info).
    IpAddress A;
    if (Addr.sa_family == __WASI_ADDRESS_FAMILY_INET4 &&
        Addr.sa_data_len >= 6) {
      A = getAddress(__WASI_ADDRESS_FAMILY_INET4,
                     Span<const uint8_t>(Bytes + 2, 4));
    } else if (Addr.sa_family == __WASI_ADDRESS_FAMILY_INET6 &&
               Addr.sa_data_len >= 22) {
      A = getAddress(__WASI_ADDRESS_FAMILY_INET6,
...
ResolveAddresses::body(Runtime::Component::CallingFrame &, std::string Name) {
  auto Result = resolveHostname(Name);
  if (!Result) {
    return Expected<std::vector<IpAddress>, LookupError>(
        Unexpected<LookupError>(getLookupError(Result.error())));
  }
