...
void resolveLater(
    ...
    const std::shared_ptr<Runtime::Instance::Component::Stream> &Done,
    SockStatus &&Status) noexcept {
  Frame.getExecutor().writeDetached(
      Frame, Done,
...
Expect<SockStatus> TcpConnect::body(Runtime::Component::CallingFrame &Frame,
  ...
  EXPECTED_TRY(Frame.runBlocking([Node, Raw, Res]() {
...
TcpListen::body(Runtime::Component::CallingFrame &Frame, TcpBorrow Self) {
  ...
  Runtime::Component::Stream<Runtime::Component::Own<TcpSocket>> Accepted{Data};
  auto Node = Sock->Node;
  ...
  Frame.spawn([Node, Data, Family, Sockets](
    ...
    while (true) {
      ...
      EXPECTED_TRY(Spawned.runBlocking([Node, Res]() {
      ...
      EXPECTED_TRY(auto Outcome,
                   Spawned.getExecutor().write(Spawned, *Data, std::move(One)));
      if (Outcome.first !=
          Runtime::Instance::Component::Stream::Result::Completed) {
        break;
      }
    }
    Spawned.getExecutor().drop(
        *Data, Runtime::Instance::Component::Stream::End::Writable);
    return {};
  });
  return SockResult<
      Runtime::Component::Stream<Runtime::Component::Own<TcpSocket>>>(
...
TcpSend::body(Runtime::Component::CallingFrame &Frame, TcpBorrow Self,
  ...
  auto In = Runtime::Instance::Component::Stream::from(Data.Shared);
  auto Done = Frame.getExecutor().newHostStream(Frame, true, ResultType);
  Runtime::Component::Future<SockStatus> DoneFuture{Done};
  auto Sock = Table.get(Self.Rep);
  auto Failure = std::make_shared<SocketError>(SocketError::InvalidState);
  Frame.getExecutor().sink(
      Frame, In,
      [Sock, Failure](Span<const uint8_t> Bytes) {
        ...
        if (auto Res = sendAll(*Sock->Node, Bytes); !Res) {
          *Failure = getError(Res.error());
          return false;
        }
        return true;
      },
      [Done, Failure](Runtime::Component::CallingFrame &Spawned, bool Failed) {
        resolveLater(Spawned, Done,
                     Failed ? SockStatus(Unexpected<SocketError>(*Failure))
...
TcpReceive::body(Runtime::Component::CallingFrame &Frame, TcpBorrow Self) {
  ...
  std::tuple<Runtime::Component::Stream<uint8_t>,
             Runtime::Component::Future<SockStatus>>
      Ends{Runtime::Component::Stream<uint8_t>{Data},
           Runtime::Component::Future<SockStatus>{Done}};
  auto Sock = Table.get(Self.Rep);
  if (!Sock) {
    Frame.getExecutor().drop(
        *Data, Runtime::Instance::Component::Stream::End::Writable);
    resolveLater(
        Frame, Done,
  ...
  Frame.spawn([Node, Data, Done](
    ...
    while (true) {
      EXPECTED_TRY(Spawned.runBlocking([Node, Buffer, Read]() {
      ...
      EXPECTED_TRY(
          auto Outcome,
          Spawned.getExecutor().write(
              Spawned, *Data, Span<const uint8_t>(Buffer->data(), **Read)));
      if (Outcome.first !=
          Runtime::Instance::Component::Stream::Result::Completed) {
        break;
      }
    ...
    Spawned.getExecutor().drop(
        *Data, Runtime::Instance::Component::Stream::End::Writable);
    resolveLater(Spawned, Done, std::move(Status));
    return {};
  });
  return Ends;
}
...
Expect<SockStatus> UdpSend::body(Runtime::Component::CallingFrame &,
  ...
  __wasi_size_t Sent = 0;
  return getStatus(
      Sock->Node->sockSendTo(IOVs, static_cast<__wasi_siflags_t>(0), Raw.Family,
                             Raw.bytes(), Raw.Port, Sent));
}
...
UdpReceive::body(Runtime::Component::CallingFrame &Frame, UdpBorrow Self) {
  ...
  while (true) {
    ...
    EXPECTED_TRY(Frame.runBlocking([Node, Buffer, Got]() {
      std::array<Span<uint8_t>, 1> IOVs{Span<uint8_t>(*Buffer)};
      __wasi_size_t Count = 0;
      __wasi_roflags_t RoFlags = static_cast<__wasi_roflags_t>(0);
    ...
    if (!Got->Count) {
      return SockResult<std::tuple<std::vector<uint8_t>, IpSocketAddress>>(
          Unexpected<SocketError>(getError(Got->Count.error())));
    }
    IpSocketAddress From =
        getSocketAddress(Got->Family, Got->Bytes, getPort(Got->Port));
    if (Wanted.has_value()) {
...
ResolveAddresses::body(Runtime::Component::CallingFrame &Frame,
  ...
  EXPECTED_TRY(Frame.runBlocking(
      [Result, Host]() { *Result = resolveHostname(*Host); }));
  if (Frame.isCancelled()) {
