...
stat2Filestat(struct stat SysFStat) noexcept {
  return {
      ...
      .atim = timespec2Timestamp(SysFStat.st_atimespec),
      .mtim = timespec2Timestamp(SysFStat.st_mtimespec),
      .ctim = timespec2Timestamp(SysFStat.st_ctimespec),
  };
}
...
struct WasiFile::Poller {
  ...
  __wasi_errno_t create(size_t) noexcept {
    if (unlikely(KQueueFd >= 0)) {
      reset();
    }
  ...
  __wasi_errno_t addTimer(const Subscription &Sub) noexcept {
    ...
    EV_SET(&Event, 0, EVFILT_TIMER, EV_ADD | EV_ENABLE | EV_ONESHOT,
           NOTE_NSECONDS, Sub.Clock.timeout, &Sub);
    if (const auto Ret = kevent(KQueueFd, &Event, 1, nullptr, 0, nullptr);
        unlikely(Ret < 0)) {
      return convertErrNo(errno);
    }
    ++Entries;
  ...
  __wasi_errno_t addFd(const Subscription &Sub, int Type, int Fd) noexcept {
    struct kevent Event;
  ...
  __wasi_errno_t addFdRead(const Subscription &Sub) noexcept {
    return addFd(Sub, DISPATCH_SOURCE_TYPE_READ, Sub.FdRead->Fd);
  }

  __wasi_errno_t addFdWrite(const Subscription &Sub) noexcept {
    return addFd(Sub, DISPATCH_SOURCE_TYPE_WRITE, Sub.FdWrite->Fd);
  }
  ...
  wait(std::function<void(const Subscription &)> RecordClock,
       std::function<void(const Subscription &, __wasi_filesize_t,
           ...
           RecordFd) noexcept {
    std::vector<struct kevent> Events(Entries);
    if (const auto Ret =
            kevent(KQueueFd, nullptr, 0, Events.data(), Entries, nullptr);
        unlikely(Ret < 0)) {
      return convertErrNo(errno);
    } else {
      Events.resize(Ret);
    ...
    for (const struct kevent &Event : Events) {
      const Subscription *Sub = Event.udata;
      ...
      case __WASI_EVENTTYPE_FD_WRITE: {
        ...
        const __wasi_eventrwflags_t Flags =
            Event.flags & EV_EOF ? __WASI_EVENTRWFLAGS_FD_READWRITE_HANGUP
                                 : static_cast<__wasi_eventrwflags_t>(0);
        RecordFd(Sub, Event.data, Flags);
      } break;
      default:
        __builtin_unreachable();
      }
