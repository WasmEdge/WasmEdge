...
enum class ShiftKind { Left, LogicalRight, ArithmeticRight };
...
llvm::Function *addRoundingCore(llvm::Module &Module, llvm::StringRef Name,
  ...
  auto *NaN = Builder.CreateAnd(
      Special,
      Builder.CreateICmpNE(Builder.CreateAnd(Bits, llvm::ConstantInt::get(
                                                       Integer, FractionMask)),
                           Zero));
...
llvm::Function *addShiftCore(llvm::Module &Module, llvm::StringRef Name,
                             ShiftKind Kind) {
  auto &Context = Module.getContext();
  ...
  auto *Low = Builder.CreateTrunc(Function->getArg(0), I32);
  auto *High = Builder.CreateTrunc(
      Builder.CreateLShr(Function->getArg(0), llvm::ConstantInt::get(I64, 32)),
      I32);
  auto *Amount = Function->getArg(1);
  auto *Narrow = Builder.CreateAnd(Amount, llvm::ConstantInt::get(I32, 31));
  auto *Wide = Builder.CreateICmpNE(
      Builder.CreateAnd(Amount, llvm::ConstantInt::get(I32, 32)), Zero32);
  auto *Untouched = Builder.CreateICmpEQ(Narrow, Zero32);
  auto *Opposite = Builder.CreateAnd(
      Builder.CreateSub(llvm::ConstantInt::get(I32, 32), Narrow),
      llvm::ConstantInt::get(I32, 31));
  llvm::Value *ResultLow = nullptr;
  llvm::Value *ResultHigh = nullptr;
  if (Kind == ShiftKind::Left) {
    auto *Carry = Builder.CreateSelect(Untouched, Zero32,
                                       Builder.CreateLShr(Low, Opposite));
    auto *Crossed = Builder.CreateShl(Low, Narrow);
    ResultLow = Builder.CreateSelect(Wide, Zero32, Crossed);
    ResultHigh = Builder.CreateSelect(
        Wide, Crossed,
        Builder.CreateOr(Builder.CreateShl(High, Narrow), Carry));
  } else {
    auto *Carry = Builder.CreateSelect(Untouched, Zero32,
                                       Builder.CreateShl(High, Opposite));
    auto *Merged = Builder.CreateOr(Builder.CreateLShr(Low, Narrow), Carry);
    if (Kind == ShiftKind::LogicalRight) {
      auto *Crossed = Builder.CreateLShr(High, Narrow);
      ResultLow = Builder.CreateSelect(Wide, Crossed, Merged);
      ResultHigh = Builder.CreateSelect(Wide, Zero32, Crossed);
    } else {
      auto *Crossed = Builder.CreateAShr(High, Narrow);
      auto *Sign = Builder.CreateAShr(High, llvm::ConstantInt::get(I32, 31));
      ResultLow = Builder.CreateSelect(Wide, Crossed, Merged);
      ResultHigh = Builder.CreateSelect(Wide, Sign, Crossed);
    }
  }
  Builder.CreateRet(
      Builder.CreateOr(Builder.CreateZExt(ResultLow, I64),
                       Builder.CreateShl(Builder.CreateZExt(ResultHigh, I64),
...
addARMRuntimeLibcalls(Module &LLModule,
  ...
  const std::array<std::pair<const char *, ShiftKind>, 3> Shifts{{
      {"__aeabi_llsl", ShiftKind::Left},
      {"__aeabi_llsr", ShiftKind::LogicalRight},
      {"__aeabi_lasr", ShiftKind::ArithmeticRight},
  }};
  for (const auto &[Name, Kind] : Shifts)
    Used.push_back(addShiftCore(Module, Name, Kind));
