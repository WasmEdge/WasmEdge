...
Expect<void> CodeGen::codegen(Span<const Byte> WasmData, Data D,
  ...
  // If WasmData is WAT text, serialize it into a WASM binary. The embedded
  // data is then always valid WASM. The text format loader is experimental,
  // so it is on only by request.
