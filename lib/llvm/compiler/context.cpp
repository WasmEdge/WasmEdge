...
Compiler::CompileContext::CompileContext(LLVM::Context C, LLVM::Module &M,
  ...
  // Record whether GC codegen (safepoint poll + shadow-root spill) is emitted
  // for this artifact. Assigned in the body (not the init list) so the member
  // keeps its header declaration order.
