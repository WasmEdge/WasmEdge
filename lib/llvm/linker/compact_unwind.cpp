...
constexpr uint64_t FDERecordHeaderSize = 8;
constexpr uint64_t FDEPointerWidth = 8;
...
bool read64(Span<const Byte> Bytes, uint64_t Offset, uint64_t &Value) noexcept {
...
Expect<NativeLayout> buildNativeLayout(const LinkGraph &Graph, bool Final) {
  ...
  if (canonicalEHFrameSection(Graph, EHFrameSection)) {
    const uint32_t DwarfMode = Graph.target() == Target::AArch64
                                   ? UnwindArm64ModeDwarf
                                   : UnwindX8664ModeDwarf;
    ...
    for (const auto &Reference : Graph.ehFrameReferences()) {
      if (Reference.Section != EHFrameSection ||
          ...
          Reference.Offset < FDERecordHeaderSize)
        continue;
      ...
      if (CoveredFunctions.count(Identity) != 0 ||
        ...
        continue;
      const uint64_t FDEOffset = Reference.Offset - FDERecordHeaderSize;
      uint64_t Length = 0;
      if (FDEOffset > UnwindDwarfOffsetMask ||
          !read64(Content, Reference.Offset + FDEPointerWidth, Length) ||
          Length > UINT32_MAX)
        return Unexpect(ErrCode::Value::IllegalPath);
      ...
      if (!FunctionAddress)
        return Unexpect(ErrCode::Value::IllegalPath);
      Result.Records.push_back({*FunctionAddress, static_cast<uint32_t>(Length),
