...
uint64_t loadDylibCommandSize() noexcept {
  uint64_t Result = DylibCommandPrefixSize + SystemLibraryName.size() + 1;
  return (Result + 7) & ~UINT64_C(7);
}
...
bool loadCommandSize(const LinkGraph &Graph, uint64_t &Result) noexcept {
  ...
  Result = SegmentCommandSize * 4 + DyldInfoCommandSize + SymtabCommandSize +
           DysymtabCommandSize + dylibCommandSize() + loadDylibCommandSize() +
           UUIDCommandSize;
...
Expect<void> copyAt(Span<Byte> Bytes, uint64_t Offset, Iterator Begin,
                    Iterator End) {
...
Expect<void> putName(Span<Byte> Bytes, uint64_t Offset, std::string_view Name) {
  const auto Trimmed = Name.substr(0, 16);
...
Expect<void> MachOWriter::write(const LinkGraph &Graph,
  ...
  try {
    ...
    uint64_t FileSize = 0;
    if (!add(RebaseOffset, RebaseStream.size(), RebaseEnd) ||
        !align(RebaseEnd, 8, ExportOffset) ||
    ...
    put(Bytes, Command, llvm::MachO::LC_LOAD_DYLIB, 4);
    put(Bytes, Command + 4, loadDylibCommandSize(), 4);
    put(Bytes, Command + 8, DylibCommandPrefixSize, 4);
    put(Bytes, Command + 16, UINT32_C(0x10000), 4);
    put(Bytes, Command + 20, UINT32_C(0x10000), 4);
    EXPECTED_TRY(copyAt(Bytes, Command + DylibCommandPrefixSize,
                        SystemLibraryName.begin(), SystemLibraryName.end()));
    Command += loadDylibCommandSize();
    UUIDOffset = Command;
    put(Bytes, Command, llvm::MachO::LC_UUID, 4);
    put(Bytes, Command + 4, UUIDCommandSize, 4);
    ...
    const llvm::MD5::MD5Result Digest = llvm::MD5::hash(llvm::ArrayRef<uint8_t>(
    ...
    EXPECTED_TRY(copyAt(Bytes, UUIDOffset + 8, UUID.begin(), UUID.end()));
