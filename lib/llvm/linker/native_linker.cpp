...
Expect<TempFile> createUniqueSibling(const std::filesystem::path &Output) {
  ...
  for (uint32_t Attempt = 0; Attempt < 128; ++Attempt) {
    ...
    for (uint32_t Digit = 0; Digit < 6; ++Digit) {
      Candidate += Hex[llvm::sys::Process::GetRandomNumber() & 15];
    }
    int File = -1;
    const auto Error = llvm::sys::fs::openFileForWrite(
        Candidate.u8string(), File, llvm::sys::fs::CD_CreateNew);
    if (!Error)
...
Expect<uint32_t> prepareSigningCopy(const std::filesystem::path &Source,
  ...
  while (true) {
    ...
    if (Read == 0) {
      if (::fchmod(Destination, SourceStat.st_mode & 07777) != 0)
        return Unexpect(ErrCode::Value::IllegalPath);
      return static_cast<uint32_t>(SourceStat.st_mode & 07777);
    }
    if (Read < 0)
      return Unexpect(ErrCode::Value::IllegalPath);
    ssize_t Written = 0;
...
Internal::publishMachO(const std::filesystem::path &Temporary,
                       ...
                       const std::filesystem::path &SignExecutable) noexcept {
  try {
    auto SignTempResult = createUniqueSibling(Temporary);
    if (SignTempResult) {
      auto SignTemp = std::move(*SignTempResult);
      TempGuard SignGuard(std::move(SignTemp.Path), SignTemp.File);
      auto Prepared = prepareSigningCopy(Temporary, SignTemp.File);
      if (Prepared) {
        const auto CloseError =
            llvm::sys::Process::SafelyCloseFileDescriptor(SignGuard.release());
        if (!CloseError) {
          std::error_code Error;
          if (signMachO(SignGuard.path(), SignExecutable) &&
            ...
            if (!Error) {
              auto Published = publishAtomically(
                  SignGuard.path(), Output,
...
#endif
    if (Graph.format() == ObjectFormat::ELF &&
        !Internal::armFloatABICompatible(Graph.format(), Graph.elfFlags(),
                                         HardFloatHost)) {
