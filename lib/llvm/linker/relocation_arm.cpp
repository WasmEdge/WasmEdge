...
LinkExpect<RelocationResult> applyARM(const LinkGraph &Graph) {
  ...
  for (const auto &Rel : Graph.relocations()) {
    ...
    if (Rel.Type == llvm::ELF::R_ARM_ABS32) {
      ...
      Target |= T;
      if (!signedDelta(Target, P, Value) || !signedBits(Value, WordBits) ||
      ...
      const uint64_t BranchP = CrossState ? (P & ~UINT64_C(3)) : P;
      int64_t Value = 0;
      ...
      uint64_t Target = 0;
      int64_t Value = 0;
      if (!addSigned(S, Implicit, Target) || !signedDelta(Target, P, Value)) {
