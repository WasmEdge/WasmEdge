...
Expect<void> writeError(std::string_view Reason) noexcept {
  spdlog::error("universal writer: {}"sv, Reason);
  return Unexpect(ErrCode::Value::IllegalPath);
}

template <typename T> Expect<T> sectionsError(std::string_view Reason) {
  spdlog::error("universal writer: {}"sv, Reason);
  return Unexpect(ErrCode::Value::IllegalPath);
}
...
Expect<std::vector<OutputSection>> outputSections(const LinkGraph &Graph,
  ...
  std::vector<std::pair<uint64_t, uint64_t>> Reserved;
  for (const auto &Value : Graph.sections()) {
    if (Value.Purpose != SectionPurpose::PData || Value.VirtualSize == 0)
      continue;
    const auto RelativeAddress = subtract(Value.Address, ImageBase);
    if (!RelativeAddress)
      return sectionsError<std::vector<OutputSection>>(
          "PE runtime function table address overflow");
    Reserved.emplace_back(*RelativeAddress, Value.VirtualSize);
  }
  const auto crossesReserved = [&Reserved](const OutputSection &Output,
    ...
    if (Address < Output.Address || RelativeEnd < Output.Address)
      return false;
    const uint64_t Span = RelativeEnd - Output.Address;
    return std::any_of(
        Reserved.begin(), Reserved.end(), [&](const auto &Value) {
          return overlaps(Value.first, Value.second, Output.Address, Span);
        });
  };
  ...
  for (const auto *Value : Ordered) {
    ...
    if (Value->Content.size() > Value->VirtualSize)
      return sectionsError<std::vector<OutputSection>>(
          "section content exceeds its virtual size");
    ...
    if (!RelativeEnd)
      return sectionsError<std::vector<OutputSection>>(
          "section end precedes the image base");
    ...
    if (Output.Kind != AOTSectionKind::BSS) {
      if (Gap > Output.Content.max_size() ||
        ...
        return sectionsError<std::vector<OutputSection>>(
            "merged section content is too large");
  ...
  if (PDataSection != Graph.sections().end()) {
    if (Graph.format() != ObjectFormat::COFF)
      return sectionsError<std::vector<OutputSection>>(
          "PE runtime function table outside a COFF object");
...
Expect<void> writeAddresses(Writer &Output, const LinkGraph &Graph,
    ...
    if (*Name == "version") {
      ...
      if (indexedSymbol(*Name, 't', Index) &&
        ...
        TypePresent[Index] = true;
                 ...
                 std::to_string(Index) == Name->substr(1)) {
        if (Index == std::numeric_limits<uint64_t>::max()) {
          return writeError("code index out of range");
        }
        const uint64_t NextSize = Index + 1;
        if (!fitsSize(NextSize) || NextSize > Codes.max_size() ||
            NextSize > CodePresent.max_size())
          return writeError("code table is too large");
        if (CodePresent.size() > Index && CodePresent[Index])
          return writeError("duplicate code index");
  ...
  if (!HasVersion || !HasIntrinsics ||
    ...
    return writeError("invalid semantic symbol tables");
  }
