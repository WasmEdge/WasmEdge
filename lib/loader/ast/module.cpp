...
Expect<void> Loader::loadUniversalWASM(AST::Module &Mod) {
  if (Conf.getRuntimeConfigure().getRunMode() == RunMode::AOT) {
    // Capability admission: a GC-enabled runtime must not bind native code
    // compiled without GC support -- it emits no cooperative safepoint poll and
    // no shadow-root spill, so a concurrent collection would hang on it or miss
    // its roots. Refuse it here rather than at instantiate: loading an AOT
    // artifact SKIPS parsing the function bodies (see Loader::loadSegment), so
    // a later deopt would have no instructions to fall back to. Declining the
    // bind here takes the interpreter fallback below, which re-reads the
    // skipped code section.
    if (Conf.hasProposal(Proposal::GC) && !Mod.getAOTSection().getGCCapable()) {
      ...
      if (auto Res = Exec->load(Mod.getAOTSection()); unlikely(!Res)) {
        ...
        if (loadExecutable(Mod, Exec)) {
          // Durable capability: the native code now bound to this module came
          // from the AOT section, so its GC capability is whatever the compiler
          // recorded there -- not the interpreter default.
