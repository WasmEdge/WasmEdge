...
Loader::parseWasmUnit(const std::filesystem::path &FilePath) {
  ...
  case FileMgr::FileHeader::MachO_64: {
    ...
    // Capability admission: refuse to bind native code compiled without GC
    // support under a GC-enabled runtime -- it emits no cooperative safepoint
    // poll and no shadow-root spill. This must be decided here rather than at
    // instantiate, because loading in AOT run mode SKIPS parsing the function
    // bodies (see Loader::loadSegment), leaving a later deopt with no
    // instructions to fall back to. Re-parsing the embedded WASM below reads
    // those bodies, so the module arrives fully interpretable.
    ...
    if (auto Ptr = std::get_if<std::unique_ptr<AST::Module>>(&Unit);
      ...
      // Durable capability: the module is now bound to this library's native
      // code, so its GC capability is the library's, not the interpreter
      // default.
