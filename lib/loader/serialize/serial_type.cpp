...
Serializer::serializeHeapType(const ValType &Type, ASTNodeAttr From,
  ...
  case TypeCode::ArrayRef:
    ...
    serializeS33(static_cast<int64_t>(Code), OutVec);
    return {};
...
Expect<void> Serializer::serializeValType(const ValType &Type, ASTNodeAttr From,
                                          std::vector<uint8_t> &OutVec,
                                          bool IsStorageType) const noexcept {
  TypeCode Code = Type.getCode();
  switch (Code) {
  case TypeCode::I32:
  case TypeCode::I64:
