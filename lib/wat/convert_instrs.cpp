...
// operand tokens. The folded form is a sexpr.
Expect<void> Converter::convertMixedInstrSeq(Cursor &C, AST::InstrVec &Instrs) {
  // Track the open flat blocks, so that 'end' can back-patch them.
  ...
  while (C.valid()) {
    ...
    if (auto Type = nodeType(Child); Type == NodeType::Sexpr) {
      ...
      case OpCode::Else: {
        // An 'else' is valid only if the innermost open block is an 'if'.
        if (BlockStack.empty() ||
          ...
          if (const auto BackOp = Instrs[BlockIdx].getOpCode();
            ...
            if (BackOp == OpCode::If) {
              if (Instrs[BlockIdx].getJumpElse() == 0) {
                ...
                Instrs[ElseIdx].setJumpEnd(
                    static_cast<uint32_t>(Instrs.size() - 1 - ElseIdx));
              }
            }
      ...
      case OpCode::Try_table: {
        // A flat try_table has the form label? blocktype catch* ... end.
        // Parse the catch clauses and make the TryCatch here, because
        // convertPlainInstr has no Try_table case. Then push the block onto
        // BlockStack. The code for End completes it.
        std::string_view Label;
        if (auto Imm = C.node(); nodeType(Imm) == NodeType::Id) {
          Label = nodeText(Imm);
          C.next();
        ...
        // The catch clauses come before the body, and they resolve their label
        // targets in the outer scope. Parse them before you push the label of
        // this block.
        while (C.valid()) {
  ...
  // An open block that stays shows that a block, loop, if, or try_table has no
  // 'end'.
  if (!BlockStack.empty()) {
...
Expect<void> Converter::convertBlockInstr(Cursor &C, OpCode Code,
  ...
  // For a folded if, write the folded instructions of the condition first.
  if (Code == OpCode::If) {
    for (; C.valid(); C.next()) {
      Node Child = C.node();
      if (nodeType(Child) != NodeType::Sexpr) {
  ...
  // For a try_table, convert the catch clauses.
  ...
  // Convert the body. For an 'if', the (then ...) and (else ...) sub-sexprs
  // are special.
  if (Code == OpCode::If) {
  ...
  if (Code == OpCode::Block || Code == OpCode::Loop || Code == OpCode::If) {
    ...
    if (Code == OpCode::If) {
      if (Instrs[BlockIdx].getJumpElse() == 0) {
        ...
        uint32_t ElseIdx =
            static_cast<uint32_t>(BlockIdx + Instrs[BlockIdx].getJumpElse());
        Instrs[ElseIdx].setJumpEnd(
            static_cast<uint32_t>(Instrs.size() - 1 - ElseIdx));
      }
    }
...
//               | ( blockinstr )
// This function converts the nested sub-instructions first. Then it writes
// this instruction.
Expect<void> Converter::convertFoldedInstr(Node N, AST::InstrVec &Instrs,
                                           std::string_view *ErrorKeyword) {
  // Bound the recursion depth to prevent a stack overflow on deeply nested
  // folded instructions. All the recursion of the converter comes back to this
  // function.
  struct DepthGuard {
    uint32_t &D;
    ~DepthGuard() { --D; }
  } Guard{++InstrNestDepth};
  if (InstrNestDepth > MaxInstrNestDepth) {
    return Unexpect(ErrCode::Value::WatNestingTooDeep);
  }
  Cursor C(N);
  auto KWNode = C.node();
  ...
  // These folded instructions are like a block.
  if (Code == OpCode::Block || Code == OpCode::Loop || Code == OpCode::If ||
  ...
  // This is a plain folded instruction: (op nested-instrs... immediates...)
  for (; C.valid(); C.next()) {
    auto Child = C.node();
    if (auto Type = nodeType(Child); Type != NodeType::Sexpr) {
      return Unexpect(ErrCode::Value::WatUnexpectedToken);
