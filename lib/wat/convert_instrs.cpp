...
#undef Line_FE
#undef UseOpCode
      // Manual entries: enum.inc strings contain spaces, won't match keywords.
...
// SIMD lane load/store that also needs a lane byte immediate.
...
// Handles both flat (keyword + operand tokens) and folded (sexpr) forms.
Expect<void> Converter::convertMixedInstrSeq(Cursor &C, AST::InstrVec &Instrs) {
  // Tracks open flat blocks so 'end' can back-patch them.
  ...
  while (C.valid()) {
    ...
    if (auto Type = nodeType(Child); Type == NodeType::Sexpr) {
      ...
      case OpCode::Else: {
        // 'else' is only valid when the innermost open block is an 'if'.
        if (BlockStack.empty() ||
      ...
      case OpCode::Try_table: {
        // Flat try_table: label? blocktype catch* ... end. Parse the catch
        // clauses and set up the TryCatch here (convertPlainInstr has no
        // Try_table case), then push onto BlockStack for the End handler to
        // finalize.
        std::string_view Label;
        if (auto Imm = C.node(); nodeType(Imm) == NodeType::Id) {
          Label = nodeText(Imm);
        ...
        // Catch clauses precede the body and resolve their label targets in
        // the enclosing scope, so parse them before pushing this block's label.
        while (C.valid()) {
  ...
  // A leftover open block means a block/loop/if/try_table is missing its 'end'.
  if (!BlockStack.empty()) {
...
// Shared by the flat and folded paths; C points to the first immediate.
...
Expect<AST::Instruction> Converter::convertSimdConstOp(Cursor &C, OpCode Code,
  ...
  case OpCode::V128__const: {
    ...
    // Remaining immediates are lane values.
...
Expect<void> Converter::convertBlockInstr(Cursor &C, OpCode Code,
  ...
  // Convert body. For 'if', (then ...) and (else ...) sub-sexprs are special.
  if (Code == OpCode::If) {
    // then
    {
      Cursor FC(C.node());
...
Expect<void> Converter::convertFoldedInstr(Node N, AST::InstrVec &Instrs,
                                           std::string_view *ErrorKeyword) {
  // Bound recursion depth to avoid stack overflow on deeply nested folded
  // instructions; all converter recursion re-enters here.
  struct DepthGuard {
    uint32_t &D;
    ~DepthGuard() { --D; }
