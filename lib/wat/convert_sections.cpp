  ...
  if (IsLimits) {
    ...
    while (C.valid()) {
      ...
      if (sexprMatch(C, "data"sv)) {
        ...
        // Only (data ...) segments and bare strings are valid here. Anything
        // else (e.g. "(memory foo)" or "(memory (elem))") is invalid WAT and
        // must not be silently turned into an empty memory.
        return Unexpect(ErrCode::Value::WatUnexpectedToken);
      }
      C.next();
    }
