...
Expect<uint32_t> Converter::resolveTypeUse(Cursor &C, AST::Module &Mod,
  ...
  // A typeuse must denote a function type. Reject an already-built GC
  // struct/array type here so the getFuncType() accesses below and in callers
  // (e.g. convertFunc) do not dereference a null FunctionType variant.
  if (HasType) {
    auto &Types = Mod.getTypeSection().getContent();
    if (TypeUseIdx < Types.size() &&
        !Types[TypeUseIdx].getCompositeType().isFunc()) {
      return Unexpect(ErrCode::Value::WatUnknownType);
    }
  }
  // With both type_use and inline type, they must agree.
  ...
  // Inline type only: find or create.
...
Expect<AST::Limit> Converter::parseLimits(Cursor &C) {
  // Limits are number tokens among the parent sexpr's children, plus optional
  // "shared" and address-type (i32/i64) keywords.
  bool Is64 = false;
  bool IsShared = false;
  ...
  while (C.valid()) {
    ...
    if (CType == NodeType::U && Nats.size() < 2) {
      ...
      if (CText == "i64"sv) {
        ...
        // Explicit 32-bit address type (the default); consume so `(table i32
        // ...)` parses like `(table i64 ...)`.
...
Expect<AST::GlobalType> Converter::parseGlobaltype(Cursor &C) {
  // Either a plain valtype keyword or a (mut valtype) sexpr.
  auto GChild = C.node();
  C.next();
...
Expect<void> Converter::convertExport(Node N, AST::Module &Mod) {
  ...
  // Reject trailing tokens after the index, e.g. (export "a" (func 0 1)).
  IC.next();
  if (IC.valid()) {
    return Unexpect(ErrCode::Value::WatUnexpectedToken);
  }
...
Expect<void> Converter::convertFunc(Node N, AST::Module &Mod) {
  ...
  while (peekType(C) == NodeType::Sexpr) {
    ...
    if (sexprMatch(C, "local"sv)) {
      // local ::= ( local id valtype ) | ( local valtype* ). A named local
      // takes exactly one id and one valtype; the forms cannot be mixed.
      Cursor LC(Child);
      LC.next(); // skip "local" keyword
...
Expect<void> Converter::convertTable(Node N, AST::Module &Mod) {
  ...
  // Peek at addr_type without consuming, so parseTabletype can still see it.
  bool Is64 = false;
  ...
  // Limits path if a number follows the optional addr_type.
  {
    auto Ahead = C.copy();
    if (HasAddrType) {
      Ahead.next();
    ...
    if (peekType(Ahead) == NodeType::U) {
      ...
      // parseTabletype -> parseLimits consumes the addr_type itself.
      EXPECTED_TRY(auto TT, parseTabletype(C));
      Seg.getTableType() = TT;

      // Optional init expression after the table type.
      ...
      // parseLimits isn't called here, so consume any addr_type directly.
      if (HasAddrType) {
        C.next();
      }
      ValType RefTy(TypeCode::FuncRef);
      ...
      std::vector<AST::Expression> InitExprs;
      {
        ...
        Node EChild = C.node();
        // (elem $func1 $func2 ...) or (elem (ref.func $f) ...)
...
Expect<void> Converter::convertMemory(Node N, AST::Module &Mod) {
  ...
  // Limits if a number follows, optionally prefixed by an i32/i64 addr_type.
  bool IsLimits = false;
  bool Is64 = false;
  {
    ...
    if (PeekT == NodeType::U) {
      ...
      if (Text == "i64"sv || Text == "i32"sv) {
        // Limits only if followed by a number.
        auto Ahead = C.copy();
        Ahead.next();
        ...
        if (NextT == NodeType::U) {
          ...
          // Bare i64/i32 before inline data; consume and take the data path.
          Is64 = (Text == "i64"sv);
          C.next();
        }
      }
  ...
  if (IsLimits) {
    ...
    // Inline data abbreviation: (memory id? (data b*)). Require real data and
    // reject unknown children so (memory) or (memory bogus) don't silently
    // produce a zero-page memory.
    std::vector<Byte> DataBytes;
    bool HasInlineData = false;
    while (C.valid()) {
      Node Child = C.node();
      if (sexprMatch(C, "data"sv)) {
        // (data "...") form: collect the inner strings.
        HasInlineData = true;
