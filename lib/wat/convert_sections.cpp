...
Expect<uint32_t> Converter::resolveTypeUse(Cursor &C, AST::Module &Mod,
  ...
  while (C.valid()) {
    ...
    if (nodeType(Child) == NodeType::Sexpr) {
      ...
      if (Keyword == "type"sv) {
        ...
        // This is the type index. The child is an Id or a number.
        if (!FC.valid()) {
          return Unexpect(ErrCode::Value::WatUnexpectedToken);
        }
        Node TChild = FC.node();
  ...
  // A typeuse must give a function type. Reject a GC struct type or array type
  // that is already complete. The calls to getFuncType() in this function and
  // in a caller such as convertFunc must not dereference a null FunctionType
  // variant.
  if (HasType) {
...
Expect<AST::Limit> Converter::parseLimits(Cursor &C) {
  // The limits are the number tokens in the children of the parent sexpr. The
  // children can also hold the optional keywords "shared", "i32", and "i64".
  bool Is64 = false;
  bool IsShared = false;
  ...
  while (C.valid()) {
    ...
    if (CType == NodeType::U && Nats.size() < 2) {
      ...
      if (CText == "i64"sv) {
        ...
        // Consume it, so that `(table i32 ...)` parses in the same way as
        // `(table i64 ...)`.
...
Expect<void> Converter::convertFunc(Node N, AST::Module &Mod) {
  ...
  // Register the parameter identifiers as locals.
...
Expect<void> Converter::convertTable(Node N, AST::Module &Mod) {
  ...
  // If a number comes after the optional addr_type, take the limits path.
  {
    ...
    if (peekType(Ahead) == NodeType::U) {
      ...
      // The optional init expression comes after the table type.
      ...
      std::vector<AST::Expression> InitExprs;
      {
        ...
        Node EChild = C.node();
        // The form is (elem $func1 $func2 ...) or (elem (ref.func $f) ...).
        for (Cursor EC(EChild); EC.next();) {
          ...
          if (EType == NodeType::Id || EType == NodeType::U) {
            ...
            Cursor TC(EEChild);
            // This is a (ref.func $f) or an (item ...) expression.
            AST::Expression Expr;
...
Expect<void> Converter::convertMemory(Node N, AST::Module &Mod) {
    ...
    if (PeekT == NodeType::U) {
      ...
      if (Text == "i64"sv || Text == "i32"sv) {
        ...
        if (NextT == NodeType::U) {
          ...
          // A bare i64 or i32 comes before the inline data. Consume it and
          // take the data path.
          Is64 = (Text == "i64"sv);
          C.next();
        }
  ...
  if (IsLimits) {
    ...
    // must hold real data. Reject a child that the converter does not know, so
    // that (memory) and (memory bogus) do not make a memory of zero pages.
    std::vector<Byte> DataBytes;
    bool HasInlineData = false;
    while (C.valid()) {
      Node Child = C.node();
      if (sexprMatch(C, "data"sv)) {
        // This is the (data "...") form. Collect the inner strings.
        HasInlineData = true;
...
Expect<void> Converter::convertElem(Node N, AST::Module &Mod) {
  ...
  while (C.valid()) {
    ...
    if (auto Type = nodeType(Child); Type == NodeType::Sexpr) {
      ...
      if (sexprMatch(C, "item"sv)) {
        // The form (item expr) is an elemexpr.
        Cursor IC(Child);
  ...
  // An active elem segment can have no explicit ref type. For such a segment,
  // take the ref type from the table.
...
Expect<void> Converter::convertData(Node N, AST::Module &Mod) {
  ...
  if (auto PType = peekType(C); PType == NodeType::Sexpr) {
    ...
    HasMemUse = true;
    Seg.setMode(AST::DataSegment::DataMode::Active);
    // A bare nat is the abbreviated memory index.
    Node IChild = C.node();
  ...
  if (HasMemUse) {
    ...
    if (peekType(OC) == NodeType::Keyword &&
      ...
      // The offset expression can also be a folded instruction.
