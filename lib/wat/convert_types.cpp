...
Expect<void> Converter::convertTypedef(Node N, AST::Module &Mod) {
  ...
  // The typedef body must be a subtype/comptype sexpr. A missing body
  // ((type) / (type $t)) or a non-sexpr body ((type $t i32)) is malformed and
  // must be rejected: silently returning success would drop the type and shift
  // every subsequent type index.
  if (!C.valid()) {
  ...
  if (KW == "sub"sv) {
    ...
    // Body keyword is not sub/func/struct/array: unknown composite type.
    return Unexpect(ErrCode::Value::WatUnexpectedToken);
  }
  return {};
}
...
Expect<AST::SubType> Converter::convertSubType(Node N, uint32_t TypeIdx) {
  ...
  // Consume comptype sexpr. The composite type is required: a (sub ...) with no
  // comptype, or with an unknown keyword, would leave SubTy's CompositeType
  // default-constructed with an uninitialized TypeCode, which is undefined
  // behavior on any later use. Reject such input.
...
// A type index is a heaptype, valid only inside ( ref null? <heaptype> ); it is
// never a bare valtype/reftype (WebAssembly 3.0 text format). A bare Id/U node
// in valtype position is therefore not a type — in param/field contexts it is
// an identifier consumed by the caller — so it is rejected here.
...
Expect<ValType> Converter::convertRefType(Node N) {
  ...
  if (Type == NodeType::Keyword) {
    ...
    // Lenient fallback resolving a bare type index to a non-nullable typed
    // reference. Spec-valid WAT writes a type index as a heaptype inside
    // ( ref null? <heaptype> ) (handled by convertRefTypeSexpr), so this branch
    // only fires on non-standard input.
