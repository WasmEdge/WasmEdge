...
Expect<void> Converter::convertTypedef(Node N, AST::Module &Mod) {
  ...
  // The subtype/comptype sexpr is mandatory: pass 1 already reserved an index
  // for this type, so omitting it would desync the type section from the
  // symbol table.
  if (!C.valid()) {
  ...
  if (KW == "sub"sv) {
    ...
    // Not a sub/func/struct/array comptype: reject rather than drop the
    // reserved type index.
    return Unexpect(ErrCode::Value::WatUnexpectedToken);
  }
  return {};
...
Expect<void> Converter::convertRecType(Node N, AST::Module &Mod) {
  ...
  // Tag every type in this rec group with its recursive info.
  if (RecSize > 1) {
...
// Parse a (mut storagetype) sexpr, returning {ValType, Var}.
...
Converter::convertStructFields(Node N, uint32_t TypeIdx) {
  ...
  while (SC.valid()) {
    ...
    if (CType == NodeType::Keyword) {
      // A bare storage-type keyword ((struct i32)) is invalid: unlike
      // arraytype, structtype has no abbreviation dropping the "field" keyword,
      // so every member must be wrapped in (field ...).
      return Unexpect(ErrCode::Value::WatUnexpectedToken);
      ...
      if (KW == "field"sv) {
        ...
        // Any non-(field ...) group (e.g. a bare (mut ...) or (ref ...)) is
        // rejected: structtype requires each member wrapped in
        // (field id? fieldtype).
        return Unexpect(ErrCode::Value::WatUnexpectedToken);
      }
    }
...
Expect<ValType> Converter::convertRefType(Node N) {
  ...
  if (Type == NodeType::Keyword) {
    ...
    // Bare type index as a non-nullable reftype (ref $t); convertValType
    // handles the nullable valtype case.
...
Expect<ValType> Converter::convertRefTypeSexpr(Node N) {
  ...
  // A (ref ...) type takes exactly one heap type; reject trailing operands like
