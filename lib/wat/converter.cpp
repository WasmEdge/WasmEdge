...
Expect<void> Converter::buildModuleField(Node Field, AST::Module &Mod) {
  ...
  // Annotations are tree-sitter extras and never appear as fields here, so any
  // keyword reaching this point is an unrecognized module field (e.g. a typo
  // like "(funcc ...)"). Reject it instead of silently dropping the field,
  // which would let the loader accept a module different from the input text.
  return Unexpect(ErrCode::Value::WatUnexpectedToken);
}
