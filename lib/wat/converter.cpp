...
Expect<void> Converter::collectModuleField(Node Field) {
  ...
  if (KW == "type"sv) {
    ...
    // NOTE: the leading id in `(elem $x ...)` is overloaded — it is the
    // segment's own identifier only in the full form; in the active
    // abbreviation `(elem $tab (offset ...) ...)` it is a table reference.
    // Disambiguating requires lookahead, so we do not duplicate-check it here
    // (a naive check would reject valid active segments that reuse a table).
    EXPECTED_TRY(auto Id, GetId(FC));
    if (!Id.empty()) {
    ...
    // Same overloaded-id caveat as elem: `(data $mem (offset ...) ...)` uses a
...
Expect<void> Converter::buildModuleField(Node Field, AST::Module &Mod) {
  ...
  // first token was already rejected above. Any other keyword is an
  // unrecognized module field — reject it instead of silently dropping it
  // (e.g. `(module (bogus))` must not parse as an empty valid module).
  return Unexpect(ErrCode::Value::WatUnexpectedToken);
}
