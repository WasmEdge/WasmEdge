...
/// Node types for the simplified grammar: it produces only S-expression
/// structure and raw token types; semantics come from keyword matching.
enum class NodeType {
  Sexpr, // tree structure (root and sexpr both map here)
...
private:
  ...
  struct SymbolTable {
    ...
    // Per-type struct field name -> field index.
  ...
  // Category handlers, shared by the flat (convertMixedInstrSeq) and folded
  // (convertPlainInstr) paths. Cursor points at the first immediate after the
  // opcode keyword.
  ...
  /// Find an existing function type matching FuncTy, or create one; returns
  /// its type index.
  ...
  // Recursion guard for folded-instruction nesting, to avoid native stack
  // overflow on pathological input. Specific to the recursive WAT converter
  // (the binary loader parses instructions iteratively).
  //
  // One nesting level costs ~3 KiB of stack when optimized and ~5 KiB
  // unoptimized, so the bound must hold against Windows' 1 MiB default thread
  // stack under the least favourable codegen. 64 levels stays under 512 KiB
  // even at a pessimistic 8 KiB per level, while the deepest folded nesting in
  // the WebAssembly spec suite is 41.
