...
Node Tree::rootNode() const {
  // ts_tree_root_node(nullptr) is UB; a null tree yields a null Node instead.
...
Tree Parser::parse(std::string_view Source) const {
  // Refuse oversized input; ts_parser_parse_string's uint32_t length would
  // otherwise truncate to a parsed prefix.
