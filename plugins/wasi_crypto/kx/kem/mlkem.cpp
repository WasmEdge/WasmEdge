...
MlKem<Bits>::KeyPair::import(Span<const uint8_t> Encoded,
  ...
  case __WASI_KEYPAIR_ENCODING_RAW: {
    ...
    // The decapsulation key embeds its own encapsulation key. Passing both
    // halves lets OpenSSL compare them during import, so inputs whose halves
    // belong to different keypairs are rejected without trusting dk alone.
    OSSL_PARAM Params[3];
    Params[0] = OSSL_PARAM_construct_octet_string(
        OSSL_PKEY_PARAM_PUB_KEY, const_cast<uint8_t *>(Encoded.data()), PkSize);
