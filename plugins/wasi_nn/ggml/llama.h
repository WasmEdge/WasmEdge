...
struct llama_context_params {
  ...
  int32_t
      n_gqa; // grouped-query attention (TEMP - will be moved to model hparams)
  float
      rms_norm_eps; // rms norm epsilon (TEMP - will be moved to model hparams)
...
LLAMA_API DEPRECATED(struct llama_context *llama_init_from_file(
                         ...
                         struct llama_context_params params),
                     "please use llama_load_model_from_file combined with "
                     "llama_new_context_with_model instead");
...
LLAMA_API int llama_get_vocab(const struct llama_context *ctx,
                              const char **strings, float *scores,
                              int capacity);

LLAMA_API int llama_get_vocab_from_model(const struct llama_model *model,
                                         const char **strings, float *scores,
                                         int capacity);
...
LLAMA_API const char *llama_token_to_str(const struct llama_context *ctx,
                                         llama_token token);

LLAMA_API const char *
llama_token_to_str_with_model(const struct llama_model *model,
                              llama_token token);
