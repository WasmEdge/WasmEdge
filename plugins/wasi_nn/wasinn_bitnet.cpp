...
namespace {
ErrNo parseMetadata(common_params &Params, LocalConfig &Conf, bool &EnableLog,
  ...
  if (Doc.at_key("enable-log").error() == simdjson::SUCCESS) {
    if (Doc["enable-log"].get<bool>().get(EnableLog) != simdjson::SUCCESS) {
      spdlog::warn(
          "[WASI-NN] BitNet: Failed to parse 'enable-log', using default.");
    }
  }

  if (Doc.at_key("n-gpu-layers").error() == simdjson::SUCCESS) {
    int64_t Val;
    if (Doc["n-gpu-layers"].get(Val) == simdjson::SUCCESS)
      Params.n_gpu_layers = Val;
  }

  if (Doc.at_key("ctx-size").error() == simdjson::SUCCESS) {
    uint64_t Val;
    if (Doc["ctx-size"].get(Val) == simdjson::SUCCESS)
      Params.n_ctx = Val;
  }
  if (Doc.at_key("batch-size").error() == simdjson::SUCCESS) {
    uint64_t Val;
    if (Doc["batch-size"].get(Val) == simdjson::SUCCESS)
      Params.n_batch = Val;
  }
  ...
  if (Doc.at_key("temperature").error() == simdjson::SUCCESS) {
    double Val;
    if (Doc["temperature"].get(Val) == simdjson::SUCCESS)
      SParams.temp = Val;
  }
  if (Doc.at_key("top-k").error() == simdjson::SUCCESS) {
    int64_t Val;
    if (Doc["top-k"].get(Val) == simdjson::SUCCESS)
      SParams.top_k = Val;
  }
  if (Doc.at_key("top-p").error() == simdjson::SUCCESS) {
    double Val;
    if (Doc["top-p"].get(Val) == simdjson::SUCCESS)
      SParams.top_p = Val;
  }
  if (Doc.at_key("repeat-penalty").error() == simdjson::SUCCESS) {
    double Val;
    if (Doc["repeat-penalty"].get(Val) == simdjson::SUCCESS)
      SParams.penalty_repeat = Val;
  }
  if (Doc.at_key("repeat-last-n").error() == simdjson::SUCCESS) {
    int64_t Val;
    if (Doc["repeat-last-n"].get(Val) == simdjson::SUCCESS)
      SParams.penalty_last_n = Val;
  }
  if (Doc.at_key("penalize-nl").error() == simdjson::SUCCESS) {
    bool Val;
    if (Doc["penalize-nl"].get(Val) == simdjson::SUCCESS)
      SParams.penalize_nl = Val;
  }
  if (Doc.at_key("seed").error() == simdjson::SUCCESS) {
    uint64_t Val;
    if (Doc["seed"].get(Val) == simdjson::SUCCESS)
      SParams.seed = Val;
  }
  if (Doc.at_key("grammar").error() == simdjson::SUCCESS) {
    std::string_view Val;
    if (Doc["grammar"].get(Val) == simdjson::SUCCESS)
      SParams.grammar = Val;
  }
...
Expect<WASINN::ErrNo> load(WASINN::WasiNNEnvironment &Env,
  ...
  if (Builders.empty()) {
    spdlog::error(
        "[WASI-NN] BitNet: Expects at least one builder (the model).");
  ...
  if (Builders.size() > 1) {
    ...
    if (auto Res =
            parseMetadata(GraphRef.Params, GraphRef.Conf, EnableLog, Metadata);
        Res != ErrNo::Success) {
      Env.deleteGraph(GId);
      return Res;
    }
  ...
  if (ModelPathStr.substr(0, 8) == "preload:"sv) {
    ...
    if (!TempFile) {
      spdlog::error(
          "[WASI-NN] BitNet: Failed to create fallback model file '{}'.",
  ...
  llama_model *ModelPtr =
      llama_load_model_from_file(GraphRef.Params.model.c_str(), MParams);

  if (ModelPtr == nullptr) {
    spdlog::error("[WASI-NN] BitNet: Failed to load model from '{}'.",
                  GraphRef.Params.model);
    llama_backend_free();
    Env.deleteGraph(GId);
    return WASINN::ErrNo::InvalidArgument;
  ...
  if (GraphRef.Params.n_ctx == 0) {
    ...
    // reasonable default.
    if (GraphRef.Params.n_ctx == 0) {
      spdlog::warn("[WASI-NN] BitNet: Model does not specify context size. "
...
Expect<WASINN::ErrNo> setInput(WASINN::WasiNNEnvironment &Env,
  ...
  if (Index == 1) {
    ...
    if (auto Res = parseMetadata(GraphRef.Params, CxtRef.Conf, DummyEnableLog,
                                 Metadata);
        Res != ErrNo::Success) {
    ...
    if (!CxtRef.LlamaSampler) {
      spdlog::error("[WASI-NN] BitNet: Failed to re-initialize sampler with "
                    "runtime options.");
      return WASINN::ErrNo::RuntimeError;
    }
    return WASINN::ErrNo::Success;
  ...
  if (Index != 0) {
    spdlog::error("[WASI-NN] BitNet: Only one input tensor (the prompt) is "
  ...
  if (Tensor.RType != WASINN::TensorType::U8) {
    spdlog::error(
        "[WASI-NN] BitNet: Input tensor must be a UTF-8 string (U8).");
    return WASINN::ErrNo::InvalidArgument;
  }
  ...
  if (CxtRef.LlamaInputs.size() >= static_cast<size_t>(GraphRef.Params.n_ctx)) {
    spdlog::error(
        ...
        CxtRef.LlamaInputs.size(), GraphRef.Params.n_ctx);
...
Expect<WASINN::ErrNo> compute(WASINN::WasiNNEnvironment &Env,
  ...
  for (size_t i = 0; i < CxtRef.LlamaInputs.size();
       i += GraphRef.Params.n_batch) {
    const size_t n_tokens_to_process = std::min((size_t)GraphRef.Params.n_batch,
                                                CxtRef.LlamaInputs.size() - i);

    common_batch_clear(CxtRef.LlamaBatch);
    for (size_t j = 0; j < n_tokens_to_process; ++j) {
      common_batch_add(CxtRef.LlamaBatch, CxtRef.LlamaInputs[i + j],
                       CxtRef.NPos, {0}, false);
      CxtRef.NPos++;
    }
...
Expect<WASINN::ErrNo> getOutput(WASINN::WasiNNEnvironment &Env,
  ...
  const size_t len =
      std::min(static_cast<size_t>(OutBuffer.size()), output_text.length());
...
Expect<WASINN::ErrNo> computeSingle(WASINN::WasiNNEnvironment &Env,
  ...
  if (!CxtRef.ComputeSingleStarted) {
    if (CxtRef.LlamaInputs.empty()) {
      spdlog::error(
          "[WASI-NN] BitNet: Input prompt not set before computeSingle.");
      return WASINN::ErrNo::InvalidArgument;
    }
    ...
    for (size_t i = 0; i < CxtRef.LlamaInputs.size();
         i += GraphRef.Params.n_batch) {
      const size_t n_tokens_to_process = std::min(
          (size_t)GraphRef.Params.n_batch, CxtRef.LlamaInputs.size() - i);
  ...
  // Generate ONE token
  if (CxtRef.NPos >= NCtx) {
    spdlog::warn("[WASI-NN] BitNet: Context is full (n_pos = {}).",
                 CxtRef.NPos);
  ...
  common_batch_clear(CxtRef.LlamaBatch);
  common_batch_add(CxtRef.LlamaBatch, new_token_id, CxtRef.NPos, {0},
                   true); // `logits` is true
  CxtRef.NPos++;
