...
Expect<ErrNo> load(WasiNNEnvironment &Env, Span<const Span<uint8_t>> Builders,
  ...
  if (BinModel.substr(0, 8) == "preload:"sv) {
    ...
    LOG_DEBUG(
        ...
        "load: Model path not found in nn-preload, write model into a tmpfile."sv)
  ...
  LOG_DEBUG(GraphRef.EnableDebugLog,
            "load: nitialize ggml model with given parameters...Done"sv)
...
Expect<ErrNo> setInput(WasiNNEnvironment &Env, uint32_t ContextId,
  ...
  if (Index == 1) {
    ...
    if (IsContextParamsUpdated) {
      LOG_INFO(GraphRef.EnableLog, "Reloaded model due to parameters change."sv)
  ...
  if (GraphRef.MMProjModelPath != ""sv) {
    ...
    if (ContainsBase64Image) {
      ...
      CxtRef.LlavaImageEmbd =
          llavaLoadBase64ImageFromPrompt(GraphRef, ClipContext, Prompt);
      // Replace the base64 image in the prompt with a placeholder.
