...
static bool evaluateQwen2vlImageEmbed(
  ...
  *StPosId += std::max(Pw, Ph);
  ...
  for (int I = 0; I < ImgTokens; I += NBatch) {
    ...
    memcpy(BatchMRopePos.data(), &MRopePos[Processed],
           NEval * sizeof(llama_pos));
    memcpy(&BatchMRopePos[NEval * 1], &MRopePos[ImgTokens * 1 + Processed],
           NEval * sizeof(llama_pos));
    memcpy(&BatchMRopePos[NEval * 2], &MRopePos[ImgTokens * 2 + Processed],
           NEval * sizeof(llama_pos));
    memcpy(&BatchMRopePos[NEval * 3], &MRopePos[ImgTokens * 3 + Processed],
           NEval * sizeof(llama_pos));
...
ErrNo batchDecode(llama_context *LlamaContext, llama_batch &Batch,
                  float *Output, int NEmbd, int EmbdNormalize) noexcept {
  ...
  for (int I = 0; I < Batch.n_tokens; I++) {
    ...
    common_embd_normalize(Embd, Output, NEmbd, EmbdNormalize);
  }
...
Expect<ErrNo> load(WasiNNEnvironment &Env, Span<const Span<uint8_t>> Builders,
  ...
  GraphRef.EmbdNormalize = ParamsDefault.embd_normalize;
