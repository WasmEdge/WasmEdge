...
Expect<WASINN::ErrNo>
VLMPipelineBackend::SetContextInput(Context &CxtRef, uint32_t Index,
                                    const TensorData &Tensor) {
  if (Index != 0) {
    spdlog::error("[WASI-NN] The input index {} is out of range."sv, Index);
    return WASINN::ErrNo::InvalidArgument;
...
Expect<WASINN::ErrNo> load(WASINN::WasiNNEnvironment &Env,
  ...
  try {
    ...
    } else if (ModelType == "VLMPipeline") {
      GraphRef.OpenVINOGenAI =
          std::make_shared<VLMPipelineBackend>(ModelPath, DeviceString);
    } else {
