...
class VLMPipelineBackend : public OpenVINOGenAIBackend {
public:
  VLMPipelineBackend(std::string Path, std::string Device) {
    Model = std::make_shared<ov::genai::VLMPipeline>(Path, Device);
  }
  ~VLMPipelineBackend() noexcept {}
  Expect<WASINN::ErrNo> SetContextInput(Context &CxtRef, uint32_t Index,
                                        const TensorData &Tensor) override;
  Expect<WASINN::ErrNo> Generate(Context &CxtRef) override;
  Expect<WASINN::ErrNo> GetContextOutput(Context &CxtRef, uint32_t Index,
...
private:
  std::shared_ptr<ov::genai::VLMPipeline> Model;
};
