...
bool checkAudioRIFF(const std::string_view Buf, const std::string_view Format) {
  if (Buf.size() < 12 || Buf.substr(0, 4) != "RIFF") {
    return false;
  }
...
Expect<ErrNo> load(WasiNNEnvironment &Env, Span<const Span<uint8_t>> Builders,
  ...
  if (BinModel.substr(0, 8) == "preload:"sv) {
    ...
    if (GraphRef.EnableDebugLog) {
      spdlog::info("[WASI-NN][Debug] Whisper backend: Model path not found in "
                   "nn-preload, write model into a tmpfile."sv);
    }
    // Write model to file.
    ModelFilePath = "whisper-model.bin"sv;
    ...
    if (!TempFile) {
      spdlog::error(
          ...
          "parameter to the ggml whisper library."sv);
    ...
    if (GraphRef.EnableDebugLog) {
      spdlog::info(
          "[WASI-NN][Debug] Whisper backend: Write model into a tmpfile...Done"sv);
    }
  }
  if (GraphRef.EnableDebugLog) {
    spdlog::info(
        "[WASI-NN][Debug] Whisper backend: Finished handling model path."sv);
  }
  // Check if the model exists.
  if (!std::filesystem::exists(std::filesystem::u8path(ModelFilePath))) {
    spdlog::error("[WASI-NN] Whisper backend: Model file not found."sv);
    Env.NNGraph.pop_back();
  ...
  if (GraphRef.EnableDebugLog) {
    spdlog::info(
        "[WASI-NN][Debug] Whisper backend: Initialize whisper context from "
        "model file with given parameters"sv);
  }
  ...
  if (GraphRef.WhisperCtx == nullptr) {
    spdlog::error(
        "[WASI-NN] Whisper backend: Error: unable to init whisper context from model."sv);
    Env.NNGraph.pop_back();
    return ErrNo::InvalidArgument;
  }
  if (GraphRef.EnableDebugLog) {
    spdlog::info(
        "[WASI-NN][Debug] Whisper backend: Initialize whisper context from "
        "model file with given parameters...Done"sv);
  }
