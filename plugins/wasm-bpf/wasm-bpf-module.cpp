...
class PluginModule : public Runtime::Instance::ModuleInstance {
   public:
    PluginModule() : ModuleInstance("wasm_bpf") {
        ...
        addHostFunc("wasm_load_bpf_object",
                    std::make_unique<LoadBpfObject>(state));
        addHostFunc("wasm_close_bpf_object",
                    std::make_unique<CloseBpfObject>(state));
        addHostFunc("wasm_attach_bpf_program",
                    std::make_unique<AttachBpfProgram>(state));
        addHostFunc("wasm_bpf_buffer_poll",
                    std::make_unique<BpfBufferPoll>(state));
        addHostFunc("wasm_bpf_map_fd_by_name",
                    std::make_unique<BpfMapFdByName>(state));
        addHostFunc("wasm_bpf_map_operate",
                    std::make_unique<BpfMapOperate>(state));
    }
};
