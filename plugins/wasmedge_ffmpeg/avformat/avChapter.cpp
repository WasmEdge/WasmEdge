...
namespace {
// Returns the chapter slot for Idx, or nullptr when the context is null or the
// index is out of range, so a guest-supplied index cannot walk past the array.
...
Expect<int32_t> AVChapterSetMetadata::body(const Runtime::CallingFrame &,
  ...
  int const Ret = setMetadataCopy(&(*AvChapter)->metadata, Src);
  if (Ret < 0) {
    spdlog::error("[WasmEdge-FFmpeg] AVChapterSetMetadata: metadata "
