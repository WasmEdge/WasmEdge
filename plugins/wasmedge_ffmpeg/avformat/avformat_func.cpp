...
Expect<int32_t> AVChapterDynarrayAdd::body(const Runtime::CallingFrame &Frame,
  ...
  // av_dynarray_add() returns void. On a realloc failure FF_DYNARRAY_ADD sets
  // *nb_ptr = 0 and av_freep()s the array (libavutil/mem.c): the array is freed
  // and NULLed while nb_chapters still holds the old nonzero count. Detect this
  // via the array pointer / count and resynchronise nb_chapters with the freed
  // (NULL) array; otherwise the next avformat_free_context() iterates the stale
  // nb_chapters over a NULL array and crashes the host. The AVChapter structs
  // appended on earlier successful calls leak here: the array that held their
  // pointers was freed, and their handles are marked borrowed so AVFreeP will
  // not release them.
