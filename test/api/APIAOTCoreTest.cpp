  ...
  T.onParse = [](SpecTest::ContextHandle Ctx, std::string_view Source,
    ...
    if (Type == Wast::ModuleType::BinaryFile ||
      ...
      if (CompileResult) {
        ...
        // Compilation failed. Malformed/invalid inputs (assert_malformed /
        // assert_invalid tests) fail in the loading or validation phase; fall
        // back to parsing the original file so the harness observes that error
        // in the expected phase. A genuine compiler failure on a well-formed
        // module must not be masked as an interpreter pass, so propagate it.
