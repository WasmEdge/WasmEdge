...
TEST(APICoreTest, GCManagedTableRejectsRawCApiMutation) {
  // The direct C-API table mutators refuse a raw mutation on a table a GC
  ...
  WasmEdge_LimitContext *TabLim =
      WasmEdge_LimitCreateWithMax(1, 4, false, false);
  WasmEdge_TableTypeContext *TabType =
...
TEST(APICoreTest, LegacyManagedRefGetterExclusionPaths) {
  ...
  // managed-ref slot's legacy BORROWED getter to a typed-null sentinel (see
  ...
  EXPECT_EQ(WasmEdge_ValueGetFuncRef(WasmEdge_GlobalInstanceGetValue(FuncGCxt)),
            HostFuncCxt)
  ...
  EXPECT_EQ(
      ...
      << "externref getter must return the exact live host pointer, not a "
         "sentinel (the externref exclusion)";
...
TEST(APICoreTest, RetainedManagedRefGettersRootAndRelease) {
  ...
  // alternatives to the legacy borrowed getters (restricted to a typed-null
  // sentinel for managed-ref slots). Each takes the producer
  ...
  //
  // This test drives the REAL exported C symbols. Coverage is SPLIT because
  // the C-API has no public force-collect, so the "retained ref survives a
  // real collection"
  ...
  // GC module (built with wasm-tools): an exported global
...
TEST(APICoreTest, RefusedAsyncInvokeReturnsNullAndOpsAreSafe) {
  // A refused asyncInvoke produces an invalid Async (empty shared_future).
...
TEST(APIVMSelfDelete, RejectedInsideOwnHostCallback) {
  ...
  static const uint8_t SelfDelWasm[] = {
      ...
      0x01, 0x60, 0x00, 0x00, 0x02, 0x0d, 0x01, 0x04, 0x68, 0x6f,
      0x73, 0x74, 0x04, 0x62, 0x6f, 0x6f, 0x6d, 0x00, 0x00, 0x03,
      0x02, 0x01, 0x00, 0x07, 0x06, 0x01, 0x02, 0x67, 0x6f, 0x00,
