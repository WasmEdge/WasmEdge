...
TEST(APIWasmCExampleTest, Threads) {
  ...
  // GoogleTest assertions are not thread-safe on every supported platform, so
  // workers record their failures and the main thread asserts after joining.
  std::vector<std::string> worker_errors(ThreadsCount);
  std::vector<std::thread> workers;
  workers.reserve(ThreadsCount);
  for (int id = 0; id < ThreadsCount; ++id) {
    workers.emplace_back([engine, shared, id, &worker_errors]() {
      std::string &err = worker_errors[static_cast<size_t>(id)];
      wasm_store_t *store = wasm_store_new(engine);
      wasm_module_t *module = wasm_module_obtain(store, shared);
      if (module == nullptr) {
        err = "wasm_module_obtain returned null";
        wasm_store_delete(store);
        return;
      }

      for (int rep = 0; rep < ThreadsReps && err.empty(); ++rep) {
        ...
        if (instance == nullptr) {
          err = "wasm_instance_new returned null";
          break;
        }
        ...
        wasm_instance_exports(instance, &exports);
        if (exports.size != 1U) {
          err = "expected 1 export";
        } else if (wasm_extern_as_func(exports.data[0]) == nullptr) {
          err = "export is not a function";
        } else {
          wasm_val_vec_t no_args = {0, nullptr};
          wasm_val_vec_t no_res = {0, nullptr};
          wasm_trap_t *trap = wasm_func_call(
              wasm_extern_as_func(exports.data[0]), &no_args, &no_res);
          if (trap != nullptr) {
            err = "run() trapped";
            wasm_trap_delete(trap);
          }
  ...
  for (int id = 0; id < ThreadsCount; ++id) {
    EXPECT_TRUE(worker_errors[static_cast<size_t>(id)].empty())
        << "worker " << id << ": " << worker_errors[static_cast<size_t>(id)];
  }
...
TEST(APIWasmCExampleTest, ThreadsCpp) {
  ...
  workers.reserve(ThreadsCount);
  for (int id = 0; id < ThreadsCount; ++id) {
    workers.emplace_back([&shared, &engine, id, &worker_errors]() {
      std::string &err = worker_errors[static_cast<size_t>(id)];
      auto store = wasm::Store::make(engine.get());
      auto module = wasm::Module::obtain(store.get(), shared.get());
      if (!module) {
        err = "Module::obtain returned null";
        return;
      }
      ...
      for (int rep = 0; rep < ThreadsReps && err.empty(); ++rep) {
        ...
        if (!instance) {
          err = "Instance::make returned null";
          break;
        }
        ...
        if (run_func->call(no_args, no_res)) {
          err = "run() trapped";
        }
      }
    });
