...
std::vector<uint8_t> getMdupComponent() {
  return {
...
TEST(Component, LoadAndRun_SimpleBinary) {
  ...
  std::vector<uint8_t> Vec = getMdupComponent();
  ASSERT_TRUE(VM.loadWasm(Vec));
  ASSERT_TRUE(VM.validate());
...
TEST(Component, Invoke_BadArguments) {
  Configure Conf;
  Conf.addProposal(WasmEdge::Proposal::Component);
  VM::VM VM(Conf);
  ...
  int64_t V = 100;
  auto TooFew = VM.executeComponent("mdup", Span<const ComponentValVariant>{},
                                    Span<const ComponentValType>{});
  ASSERT_FALSE(TooFew);
  EXPECT_EQ(TooFew.error(), ErrCode::Value::FuncSigMismatch);

  auto TooMany = VM.executeComponent(
      "mdup",
      {ComponentValVariant(ValVariant(V)), ComponentValVariant(ValVariant(V))},
      {ComponentValType(ComponentTypeCode::S64),
       ComponentValType(ComponentTypeCode::S64)});
  ASSERT_FALSE(TooMany);
  EXPECT_EQ(TooMany.error(), ErrCode::Value::FuncSigMismatch);

  auto MissingTypes =
      VM.executeComponent("mdup", {ComponentValVariant(ValVariant(V))},
                          Span<const ComponentValType>{});
  ASSERT_FALSE(MissingTypes);
  EXPECT_EQ(MissingTypes.error(), ErrCode::Value::FuncSigMismatch);

  uint64_t U64Value = 100;
  auto WrongComponentType =
      VM.executeComponent("mdup", {ComponentValVariant(ValVariant(U64Value))},
                          {ComponentValType(ComponentTypeCode::U64)});
  ASSERT_FALSE(WrongComponentType);
  EXPECT_EQ(WrongComponentType.error(), ErrCode::Value::FuncSigMismatch);
}

TEST(Component, Invoke_MissingLowerFunction) {
  Configure Conf;
  Conf.addProposal(WasmEdge::Proposal::Component);
  Executor::Executor Exec(Conf);

  AST::Component::FuncType FuncType;
  Runtime::Instance::Component::FunctionInstance FuncInst(FuncType, nullptr,
                                                          nullptr, nullptr);

  auto Res = Exec.invoke(&FuncInst, Span<const ComponentValVariant>{},
                         Span<const ComponentValType>{});
  ASSERT_FALSE(Res);
  EXPECT_EQ(Res.error(), ErrCode::Value::FuncNotFound);
}
