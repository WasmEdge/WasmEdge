...
TEST(Component, InstanceOutlivesSourceAst) {
  Configure Conf;
  Conf.addProposal(WasmEdge::Proposal::Component);
  Loader::Loader LoaderEngine(Conf);
  Validator::Validator ValidatorEngine(Conf);
  Executor::Executor CoreExecutor(Conf);
  Executor::ComponentExecutor ExecutorEngine(CoreExecutor);
  ...
  //   (core instance $m (instantiate $M))
  //   (func (export "mdup") (param "a" s64) (result s64)
  //     (canon lift (core func $m "dup"))))
  std::vector<uint8_t> Vec = {
      0x00, 0x61, 0x73, 0x6d, 0x0d, 0x00, 0x01, 0x00, 0x01, 0x33, 0x00, 0x61,
      0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x06, 0x01, 0x60, 0x01, 0x7e,
      0x01, 0x7e, 0x03, 0x02, 0x01, 0x00, 0x07, 0x07, 0x01, 0x03, 0x64, 0x75,
      ...
      0x00, 0x1f, 0x0e, 0x63, 0x6f, 0x6d, 0x70, 0x6f, 0x6e, 0x65, 0x6e, 0x74,
      0x2d, 0x6e, 0x61, 0x6d, 0x65, 0x01, 0x06, 0x00, 0x11, 0x01, 0x00, 0x01,
      0x4d, 0x01, 0x06, 0x00, 0x12, 0x01, 0x00, 0x01, 0x6d,
  };
  auto Comp = LoaderEngine.parseComponent(Vec);
  ASSERT_TRUE(Comp);
  ASSERT_TRUE(ValidatorEngine.validate(**Comp));
  auto Inst = ExecutorEngine.instantiateComponent(StoreMgr, **Comp);
  ASSERT_TRUE(Inst);
  // The core module was instantiated and never exported: its entry no
  // longer points into the source AST.
  auto Mod = (*Inst)->getModule(0);
  ASSERT_TRUE(Mod);
  EXPECT_EQ(*Mod, nullptr);
  // The instance owns what it keeps: the source AST goes away first.
  Comp->reset();

  const auto *Func = (*Inst)->findExportedFunction("mdup");
  ASSERT_NE(Func, nullptr);
  int64_t V = 100;
  auto Res = ExecutorEngine.invoke(Func, {ComponentValVariant{V}},
                                   {ComponentValType(ComponentTypeCode::S64)});
  ASSERT_TRUE(Res);
  EXPECT_EQ(std::get<int64_t>((*Res)[0].first), 200);
}

TEST(Component, ExportedComponentOutlivesSourceAst) {
  Configure Conf;
  Conf.addProposal(WasmEdge::Proposal::Component);
  VM::ComponentVM VM(Conf);

  // Registered as "a". Each instance of $C exports $Inner, which closes over
  // the module supplied to that instantiation; the VM drops the AST of a
  // registered component once it is instantiated.
  // (component
  //   (component $C
...
TEST(Component, InlineExportedComponentResolves) {
  ...
  ASSERT_TRUE(VM.validate());
  ASSERT_TRUE(VM.instantiate());
  // The inline-export instance owns the copy it exports; the root's own
  // entry for $X pointed into the source AST and was released.
  auto CompDef = VM.getActiveComponent()->getComponentDefinition(0);
  ASSERT_TRUE(CompDef);
  EXPECT_EQ((*CompDef)->Ast, nullptr);
  auto Res = VM.execute("get");
  ASSERT_TRUE(Res);
