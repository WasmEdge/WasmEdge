...
//     (export "hello" (func 0))
//   )
//   (instance $i (instantiate $c (with "f" (func 0))))
//   (alias export $i "hello" (func))
// )
TEST(ComponentValidatorTest, AliasExportSortMatch) {
  Configure Conf;
  Conf.addProposal(Proposal::Component);
  VM::VM VM(Conf);
  // clang-format off
  std::vector<uint8_t> Vec = {
      ,
  };
  // clang-format on
  ASSERT_TRUE(VM.loadWasm(Vec));
...
//   )
//   (instance $i (instantiate $c (with "f" (func 0))))
//   (alias export $i "hello" (component))  ;; MISMATCH: export is func
// )
TEST(ComponentValidatorTest, AliasExportSortMismatch) {
  Configure Conf;
  Conf.addProposal(Proposal::Component);
  VM::VM VM(Conf);
  // clang-format off
  std::vector<uint8_t> Vec = {
      ...
      0x00, 0x00, 0x13, 0x0e, 0x63, 0x6f, 0x6d, 0x70, 0x6f, 0x6e, 0x65, 0x6e,
      0x74, 0x2d, 0x6e, 0x61, 0x6d, 0x65, 0x00, 0x02, 0x01, 0x63, 0x05, 0x08,
      0x01, 0x00, 0x00, 0x01, 0x01, 0x66, 0x01, 0x00, 0x06, 0x0a, 0x01, 0x04,
      0x00, 0x00, 0x05, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x00, 0x1d, 0x0e, 0x63,
      0x6f, 0x6d, 0x70, 0x6f, 0x6e, 0x65, 0x6e, 0x74, 0x2d, 0x6e, 0x61, 0x6d,
      0x65, 0x01, 0x05, 0x04, 0x01, 0x00, 0x01, 0x63, 0x01, 0x05, 0x05, 0x01,
      0x00, 0x01, 0x69,
...
//   (alias export $i "hello" (func))
// )
TEST(ComponentValidatorTest, AliasInlineExportSortMatch) {
  Configure Conf;
  Conf.addProposal(Proposal::Component);
  VM::VM VM(Conf);
  // clang-format off
  std::vector<uint8_t> Vec = {
      0x00, 0x61, 0x73, 0x6d, 0x0d, 0x00, 0x01, 0x00, 0x07, 0x05, 0x01, 0x40,
      0x00, 0x01, 0x00, 0x0a, 0x06, 0x01, 0x00, 0x01, 0x66, 0x01, 0x00, 0x05,
      0x0c, 0x01, 0x01, 0x01, 0x00, 0x05, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x01,
      0x00, 0x06, 0x0a, 0x01, 0x01, 0x00, 0x00, 0x05, 0x68, 0x65, 0x6c, 0x6c,
      0x6f, 0x00, 0x16, 0x0e, 0x63, 0x6f, 0x6d, 0x70, 0x6f, 0x6e, 0x65, 0x6e,
      0x74, 0x2d, 0x6e, 0x61, 0x6d, 0x65, 0x01, 0x05, 0x05, 0x01, 0x00, 0x01,
      0x69,
  };
  // clang-format on
  ASSERT_TRUE(VM.loadWasm(Vec));
...
// (component
//   (import "f" (func))
//   (alias export 0 "x" (func))  ;; instance index 0 does not exist
// )
TEST(ComponentValidatorTest, AliasExportInvalidInstanceIndex) {
  Configure Conf;
  Conf.addProposal(Proposal::Component);
  VM::VM VM(Conf);
  // clang-format off
  std::vector<uint8_t> Vec = {
      0x00, 0x61, 0x73, 0x6d, 0x0d, 0x00, 0x01, 0x00, 0x07, 0x05, 0x01, 0x40,
      0x00, 0x01, 0x00, 0x0a, 0x06, 0x01, 0x00, 0x01, 0x66, 0x01, 0x00, 0x06,
      0x06, 0x01, 0x01, 0x00, 0x00, 0x01, 0x78,
  };
  // clang-format on
  ASSERT_TRUE(VM.loadWasm(Vec));
  ASSERT_FALSE(VM.validate());
...
//   (import "f" (func))
//   (instance $i (export "hello" (func 0)))
//   (alias export $i "nonexistent" (func))  ;; export name does not exist
// )
TEST(ComponentValidatorTest, AliasExportNameNotFound) {
  Configure Conf;
  Conf.addProposal(Proposal::Component);
  VM::VM VM(Conf);
  // clang-format off
  std::vector<uint8_t> Vec = {
      ...
      0x00, 0x01, 0x00, 0x0a, 0x06, 0x01, 0x00, 0x01, 0x66, 0x01, 0x00, 0x05,
      0x0c, 0x01, 0x01, 0x01, 0x00, 0x05, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x01,
      0x00, 0x06, 0x10, 0x01, 0x01, 0x00, 0x00, 0x0b, 0x6e, 0x6f, 0x6e, 0x65,
      0x78, 0x69, 0x73, 0x74, 0x65, 0x6e, 0x74, 0x00, 0x16, 0x0e, 0x63, 0x6f,
      0x6d, 0x70, 0x6f, 0x6e, 0x65, 0x6e, 0x74, 0x2d, 0x6e, 0x61, 0x6d, 0x65,
      0x01, 0x05, 0x05, 0x01, 0x00, 0x01, 0x69,
  };
  // clang-format on
  ASSERT_TRUE(VM.loadWasm(Vec));
  ASSERT_FALSE(VM.validate());
...
//   (component $c
//     (import "t" (type (eq 0)))
//     (export "my-type" (type 0))
//   )
//   (instance $i (instantiate $c (with "t" (type 0))))
//   (alias export $i "my-type" (type))
// )
TEST(ComponentValidatorTest, AliasExportTypeSortMatch) {
  Configure Conf;
  Conf.addProposal(Proposal::Component);
  VM::VM VM(Conf);
  // clang-format off
  std::vector<uint8_t> Vec = {
      0x00, 0x61, 0x73, 0x6d, 0x0d, 0x00, 0x01, 0x00, 0x0a, 0x07, 0x01, 0x00,
      0x01, 0x74, 0x03, 0x00, 0x00, 0x04, 0x35, 0x00, 0x61, 0x73, 0x6d, 0x0d,
      0x00, 0x01, 0x00, 0x0a, 0x07, 0x01, 0x00, 0x01, 0x74, 0x03, 0x00, 0x00,
      0x0b, 0x0d, 0x01, 0x00, 0x07, 0x6d, 0x79, 0x2d, 0x74, 0x79, 0x70, 0x65,
      0x03, 0x00, 0x00, 0x00, 0x13, 0x0e, 0x63, 0x6f, 0x6d, 0x70, 0x6f, 0x6e,
      0x65, 0x6e, 0x74, 0x2d, 0x6e, 0x61, 0x6d, 0x65, 0x00, 0x02, 0x01, 0x63,
      0x05, 0x08, 0x01, 0x00, 0x00, 0x01, 0x01, 0x74, 0x03, 0x00, 0x06, 0x0c,
      0x01, 0x03, 0x00, 0x00, 0x07, 0x6d, 0x79, 0x2d, 0x74, 0x79, 0x70, 0x65,
      0x00, 0x1d, 0x0e, 0x63, 0x6f, 0x6d, 0x70, 0x6f, 0x6e, 0x65, 0x6e, 0x74,
      0x2d, 0x6e, 0x61, 0x6d, 0x65, 0x01, 0x05, 0x04, 0x01, 0x00, 0x01, 0x63,
      0x01, 0x05, 0x05, 0x01, 0x00, 0x01, 0x69,
  };
  // clang-format on
  ASSERT_TRUE(VM.loadWasm(Vec));
