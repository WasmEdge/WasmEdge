...
TEST(ComponentValidatorTest, EmptyVariantCaseName) {
  ...
  AST::Component::VariantTy::Case Case;
  Case.Label = "";
  Case.ValType = ValTy;
  Variant.Cases.push_back(Case);
...
TEST(ComponentValidatorTest, VariantCaseRefinesInvalidIndex) {
  AST::Component::Component Comp;
  Comp.getSections().emplace_back();
  Comp.getSections().back().emplace<AST::Component::TypeSection>();
  auto &TypeSec = std::get<AST::Component::TypeSection>(Comp.getSections()[0]);

  // Add a variant type with a case that refines an invalid index
  // (type $v (variant (case "x" string (refines 1))))
  TypeSec.getContent().emplace_back();
  AST::Component::DefValType DVT;
  AST::Component::VariantTy Variant;
  ...
  AST::Component::VariantTy::Case Case;
  Case.Label = "x";
  Case.ValType = ComponentValType(ComponentTypeCode::String);
  Case.Refines =
      1; // Invalid - refines case index 1, but this is the first case (index 0)
  Variant.Cases.push_back(Case);

  DVT.setVariant(std::move(Variant));
  TypeSec.getContent()[0].setDefValType(std::move(DVT));
...
TEST(ComponentValidatorTest, VariantCaseRefinesLaterCase) {
  AST::Component::Component Comp;
  Comp.getSections().emplace_back();
  Comp.getSections().back().emplace<AST::Component::TypeSection>();
  auto &TypeSec = std::get<AST::Component::TypeSection>(Comp.getSections()[0]);

  // Add a variant type with multiple cases
  // (type $v (variant (case "x" string) (case "y" u64 (refines 2)) (case "z"
  // string)))
  TypeSec.getContent().emplace_back();
  AST::Component::DefValType DVT;
  AST::Component::VariantTy Variant;

  // Case 0: "x" with type string
  AST::Component::VariantTy::Case Case1;
  Case1.Label = "x";
  Case1.ValType = ComponentValType(ComponentTypeCode::String);
  Variant.Cases.push_back(Case1);

  // Case 1: "y" with type u64, refines case 2 (invalid - can't refine a later
  // case)
  AST::Component::VariantTy::Case Case2;
  Case2.Label = "y";
  Case2.ValType = ComponentValType(ComponentTypeCode::U64);
  Case2.Refines =
      2; // Invalid - trying to refine case 2, which comes after this case
  Variant.Cases.push_back(Case2);

  // Case 2: "z" with type string
  AST::Component::VariantTy::Case Case3;
  Case3.Label = "z";
  Case3.ValType = ComponentValType(ComponentTypeCode::String);
  Variant.Cases.push_back(Case3);
...
TEST(ComponentValidatorTest, EmptyVariant) {
  ...
  // Create a variant with no cases (empty)
...
TEST(ComponentValidatorBinaryTest, ValidDefValTypes_FromWasmBinary) {
  ...
  std::vector<uint8_t> Binary = {
      0x00, 0x61, 0x73, 0x6d, 0x0d, 0x00, 0x01, 0x00, 0x07, 0x7a, 0x24, 0x7f,
      ...
      };
