...
//===-- wasmedge/test/component/spectest.cpp - Component spec tests -------===//
//
...
static const TestsuiteProposal TestsuiteProposals[] = {
    ...
    {"component-model"sv, WasmEdge::Standard::WASM_3, {Proposal::Component}},
};
...
std::map<std::string, ComponentModelSupport> ComponentModelFolders = {
    // | Folder | Test table: {load, validate, instantiate} |
    // ------------------------------------------------------
    // Folder: the directory name of tests.
    // Test table: the testing status of load, validate and instantiate.
    {"adapt",                   {true, false, false}},
    {"alias",                   {true, false, false}},
    {"big",                     {true, false, false}},
    {"definedtypes",            {true, false, false}},
    {"empty",                   {true, false, false}},
    {"example",                 {true, false, false}},
    {"export",                  {true, false, false}},
    {"export-ascription",       {true, false, false}},
    {"export-introduces-alias", {true, false, false}},
    {"func",                    {false, false, false}},
    {"import",                  {true, false, false}},
    {"imports-exports",         {true, false, false}},
    {"inline-exports",          {true, false, false}},
    {"instance-types",          {true, false, false}},
    {"instantiate",             {true, false, false}},
    {"invalid",                 {true, false, false}},
    {"link",                    {true, false, false}},
    {"lots-of-aliases",         {true, false, false}},
    {"lower",                   {true, false, false}},
    {"memory64",                {true, false, false}},
    {"module-link",             {true, false, false}},
    {"more-flags",              {true, false, false}},
    {"naming",                  {true, false, false}},
    {"nested-modules",          {true, false, false}},
    {"resources",               {true, false, false}},
    {"tags",                    {true, false, false}},
    {"type-export-restrictions",{true, false, false}},
    {"types",                   {true, false, false}},
    {"very-nested",             {true, false, false}},
    {"virtualize",              {true, false, false}},
    {"wrong-order",             {false, false, false}},
};
// clang-format on

bool isPhaseSupported(std::string_view Folder, WasmEdge::WasmPhase P) {
...
void SpecTest::run(std::string_view Proposal, std::string_view UnitName) {
  ...
  std::map<std::string, std::unique_ptr<AST::Component::Component>> ASTMap;
  std::string LastModName;

  // Helper function to check result of invocation.
  auto Invoke = [&](const simdjson::dom::object &, const simdjson::dom::array &,
                    uint64_t) {
    // TODO: This will be merged to the spec test once the component model
    // implementation is ready.
  };

  // Helper function to check one of matched results of invocation.
  auto InvokeEither = [&](const simdjson::dom::object &,
                          const simdjson::dom::array &, uint64_t) {
    // TODO: This will be merged to the spec test once the component model
    // implementation is ready.
  };

  // Helper function to get values.
  auto Get = [&](const simdjson::dom::object &, const simdjson::dom::array &,
                 uint64_t) {
    // TODO: This will be merged to the spec test once the component model
    // implementation is ready.
  };
  ...
  auto TrapLoad = [&](const std::string &FileName, const std::string &Text) {
    if (isPhaseSupported(UnitName, WasmPhase::Loading)) {
  ...
  auto TrapValidate = [&](const std::string &FileName,
    ...
    if (isPhaseSupported(UnitName, WasmPhase::Validation)) {
  ...
  auto TrapInstantiate = [&](const std::string &FileName,
                             const std::string &Text) {
    if (isPhaseSupported(UnitName, WasmPhase::Instantiation)) {
      if (auto Res = onInstantiate(FileName); Res) {
        EXPECT_TRUE(false);
      } else {
        EXPECT_TRUE(Res.error().getErrCodePhase() ==
                        WasmEdge::WasmPhase::Instantiation ||
                    Res.error().getErrCodePhase() ==
  ...
  auto TrapInvoke = [&](const simdjson::dom::object &, const std::string &,
                        uint64_t) {
    // TODO: This will be merged to the spec test once the component model
    // implementation is ready.
  };
  ...
  auto ExceptionInvoke = [&](const simdjson::dom::object &, uint64_t) {
    // TODO: This will be merged to the spec test once the component model
    // implementation is ready.
  };
  ...
  auto RunCommand = [&](const simdjson::dom::object &Cmd) {
    ...
    if (!Cmd["type"].get(TypeField)) {
      ...
      case SpecTest::CommandID::Module: {
        ...
        if (isPhaseSupported(UnitName, WasmPhase::Instantiation)) {
          // TODO: remove this condition once all component model instantiation
          // supported.
