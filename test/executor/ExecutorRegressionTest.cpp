...
TEST(ExecutorRegression, NullLocalAbstractType) {
  ...
  // The exports return 1 when the null local matches the ref.test target, 0
  // when it does not, and 7 when the ref.cast succeeds.
  struct Case {
    const char *Name;
    uint32_t Expected;
  };
  const Case Cases[] = {
      {"test_struct", 1U}, {"test_struct_nonnull", 0U},
      {"cast_struct", 7U}, {"test_any", 1U},
      {"test_i31", 1U},    {"test_array", 1U},
      {"test_func", 1U},   {"test_func_nonnull", 0U},
      {"cast_func", 7U},   {"test_extern", 1U},
  };
  for (const auto &[Name, Expected] : Cases) {
    auto Result = VM.execute(Name);
    ASSERT_TRUE(Result) << Name << " trapped";
    ASSERT_EQ(Result->size(), 1U) << Name;
    EXPECT_EQ((*Result)[0].first.get<uint32_t>(), Expected) << Name;
  }
}
