...
// Test fixture for validating subtyping when the GC proposal is DISABLED.
class ValidatorSubtypeNoGcTest : public testing::Test {
protected:
  void SetUp() override {
    Conf = std::make_unique<WasmEdge::Configure>();
    LoadEngine = std::make_unique<WasmEdge::Loader::Loader>(*Conf);
...
// Test a valid Wasm module with a single supertype relationship.
TEST_F(ValidatorSubtypeNoGcTest, ValidSingleSupertype) {
  std::array<WasmEdge::Byte, 22> Wasm = {
      0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x0c, 0x02,
      0x50, 0x00, 0x60, 0x00, 0x00, 0x50, 0x01, 0x00, 0x60, 0x00, 0x00};

  auto Result = LoadEngine->parseModule(Wasm);
  ASSERT_TRUE(Result);

  // The validation should succeed because the single supertype is valid.
  auto ValidationResult = ValidEngine->validate(**Result);
  EXPECT_TRUE(ValidationResult);
}

// Test that subtyping a 'final' type is rejected.
TEST_F(ValidatorSubtypeNoGcTest, InvalidFinalSupertype) {
  std::array<WasmEdge::Byte, 20> Wasm = {
      0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x0a,
      0x02, 0x60, 0x00, 0x00, 0x50, 0x01, 0x00, 0x60, 0x00, 0x00};
...
// Test that subtyping a non-existent type index is rejected.
TEST_F(ValidatorSubtypeNoGcTest, InvalidSupertypeIndex) {
  std::array<WasmEdge::Byte, 17> Wasm = {0x00, 0x61, 0x73, 0x6d, 0x01, 0x00,
                                         0x00, 0x00, 0x01, 0x07, 0x01, 0x50,
                                         0x01, 0x63, 0x60, 0x00, 0x00};
...
// Test that a structural mismatch between subtype and supertype is rejected.
TEST_F(ValidatorSubtypeNoGcTest, InvalidSupertypeMismatch) {
  std::array<WasmEdge::Byte, 23> Wasm = {
      0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x0d, 0x02, 0x50,
      0x00, 0x60, 0x01, 0x7f, 0x00, 0x50, 0x01, 0x00, 0x60, 0x00, 0x00};

  auto Result = LoadEngine->parseModule(Wasm);
  ASSERT_TRUE(Result);

  // Validation must fail because the types do not match structurally.
  auto ValidationResult = ValidEngine->validate(**Result);
  EXPECT_FALSE(ValidationResult);
  EXPECT_EQ(ValidationResult.error(), WasmEdge::ErrCode::Value::InvalidSubType);
}
