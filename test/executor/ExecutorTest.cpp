...
// num_total_types: The total number of types to define. Type k+1 subtypes type
// k. The maximum depth of the chain will be num_total_types - 1.
static std::vector<WasmEdge::Byte>
generateWasmWithSubtypeChain(int num_total_types) {
  std::vector<WasmEdge::Byte> WasmBytes;
  // Estimate capacity and Reserve a bit more to be safe. Max num_total_types
  // expected is small (e.g. < 100).
  WasmBytes.reserve(static_cast<size_t>(num_total_types) * 12 + 100);
  ...
  uint32_t const TypeSectionPayloadSize =
      static_cast<uint32_t>(6 * num_total_types);
  ...
  TypeNamesSubsectionContent.push_back(
      static_cast<WasmEdge::Byte>(num_total_types));
  for (int i = 0; i < num_total_types; ++i) {
