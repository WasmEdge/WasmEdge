...
std::vector<WasmEdge::Byte> WasiSchedYieldNoMemoryWasm{
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01,
    0x60, 0x00, 0x01, 0x7f, 0x02, 0x26, 0x01, 0x16, 0x77, 0x61, 0x73,
    0x69, 0x5f, 0x73, 0x6e, 0x61, 0x70, 0x73, 0x68, 0x6f, 0x74, 0x5f,
    0x70, 0x72, 0x65, 0x76, 0x69, 0x65, 0x77, 0x31, 0x0b, 0x73, 0x63,
    0x68, 0x65, 0x64, 0x5f, 0x79, 0x69, 0x65, 0x6c, 0x64, 0x00, 0x00,
    0x03, 0x02, 0x01, 0x00, 0x07, 0x0a, 0x01, 0x06, 0x5f, 0x73, 0x74,
    0x61, 0x72, 0x74, 0x00, 0x01, 0x0a, 0x06, 0x01, 0x04, 0x00, 0x10,
    0x00, 0x0b};

std::vector<WasmEdge::Byte> WasiArgsSizesGetExportedMemoryWasm{
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x0b, 0x02,
    0x60, 0x02, 0x7f, 0x7f, 0x01, 0x7f, 0x60, 0x00, 0x01, 0x7f, 0x02,
    0x29, 0x01, 0x16, 0x77, 0x61, 0x73, 0x69, 0x5f, 0x73, 0x6e, 0x61,
    0x70, 0x73, 0x68, 0x6f, 0x74, 0x5f, 0x70, 0x72, 0x65, 0x76, 0x69,
    0x65, 0x77, 0x31, 0x0e, 0x61, 0x72, 0x67, 0x73, 0x5f, 0x73, 0x69,
    0x7a, 0x65, 0x73, 0x5f, 0x67, 0x65, 0x74, 0x00, 0x00, 0x03, 0x02,
    0x01, 0x01, 0x05, 0x05,
    0x02, 0x00, 0x00, 0x00, 0x01, 0x07, 0x20, 0x03, 0x0a, 0x6e, 0x6f,
    0x74, 0x5f, 0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79, 0x02, 0x00, 0x06,
    0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79, 0x02, 0x01, 0x06, 0x5f, 0x73,
    0x74, 0x61, 0x72, 0x74, 0x00, 0x01, 0x0a, 0x0a, 0x01, 0x08, 0x00,
    0x41, 0x00, 0x41, 0x04, 0x10, 0x00, 0x0b};

std::vector<WasmEdge::Byte> WasiArgsSizesGetRenamedMemoryWasm{
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x0b, 0x02,
    0x60, 0x02, 0x7f, 0x7f, 0x01, 0x7f, 0x60, 0x00, 0x01, 0x7f, 0x02,
    0x29, 0x01, 0x16, 0x77, 0x61, 0x73, 0x69, 0x5f, 0x73, 0x6e, 0x61,
    0x70, 0x73, 0x68, 0x6f, 0x74, 0x5f, 0x70, 0x72, 0x65, 0x76, 0x69,
    0x65, 0x77, 0x31, 0x0e, 0x61, 0x72, 0x67, 0x73, 0x5f, 0x73, 0x69,
    0x7a, 0x65, 0x73, 0x5f, 0x67, 0x65, 0x74, 0x00, 0x00, 0x03, 0x02,
    0x01, 0x01, 0x05, 0x03, 0x01, 0x00, 0x01, 0x07, 0x15, 0x02, 0x08,
    0x6d, 0x65, 0x6d, 0x72, 0x6f, 0x74, 0x72, 0x79, 0x02, 0x00, 0x06,
    0x5f, 0x73, 0x74, 0x61, 0x72, 0x74, 0x00, 0x01, 0x0a, 0x0a, 0x01,
    0x08, 0x00, 0x41, 0x00, 0x41, 0x04, 0x10, 0x00, 0x0b};
...
TEST(VM, WASIAcceptsMemoryFreeFunction) {
  WasmEdge::Configure Conf;
  Conf.addHostRegistration(WasmEdge::HostRegistration::Wasi);
  WasmEdge::VM::VM VM(Conf);

  ASSERT_TRUE(VM.loadWasm(WasiSchedYieldNoMemoryWasm));
  ASSERT_TRUE(VM.validate());
  ASSERT_TRUE(VM.instantiate());
  auto Result = VM.execute("_start");
  ASSERT_TRUE(Result);
  ASSERT_EQ((*Result).size(), 1U);
  EXPECT_EQ((*Result)[0].first.get<uint32_t>(), 0U);
}

TEST(VM, WASIUsesExportedMemoryName) {
  WasmEdge::Configure Conf;
  Conf.addHostRegistration(WasmEdge::HostRegistration::Wasi);
  WasmEdge::VM::VM VM(Conf);

  ASSERT_TRUE(VM.loadWasm(WasiArgsSizesGetExportedMemoryWasm));
  ASSERT_TRUE(VM.validate());
  ASSERT_TRUE(VM.instantiate());
  auto Result = VM.execute("_start");
  ASSERT_TRUE(Result);
...
TEST(VM, WASIMissingMemoryExportFaultsOnAccess) {
  WasmEdge::Configure Conf;
  Conf.addHostRegistration(WasmEdge::HostRegistration::Wasi);
  WasmEdge::VM::VM VM(Conf);

  ASSERT_TRUE(VM.loadWasm(WasiArgsSizesGetRenamedMemoryWasm));
  ASSERT_TRUE(VM.validate());
  ASSERT_TRUE(VM.instantiate());
  auto Result = VM.execute("_start");
  ASSERT_TRUE(Result);
  ASSERT_EQ((*Result).size(), 1U);
  EXPECT_NE((*Result)[0].first.get<uint32_t>(), 0U);
}
