...
public:
  Expect<void> body(const WasmEdge::Runtime::CallingFrame &CF) {
    // Collect scanning only the registered roots (value stack, tables, globals,
    // host-roots) and intentionally NOT the native stack. This interpreter test
    // fully controls what is live on the value stack at each collection point,
    // so dropped arrays are deterministically reclaimed and the exact
    // memory-usage assertions below stay stable. Scanning the native stack here
    // (as the AOT path does) could conservatively retain a just-dropped object
    // and make those assertions flaky across compilers/optimization levels.
...
TEST(GC, gc) {
  ...
  // coll calls the wasm makes; otherwise a background collection firing between
  // a rec snapshot and the next would make the exact byte assertions flaky.
  ...
  // The 160-byte array is still live across this collection: a freshly
  // allocated object is born gray, so the first cycle after its allocation
  // always keeps it (it is only reclaimed once a later cycle finds it white).
