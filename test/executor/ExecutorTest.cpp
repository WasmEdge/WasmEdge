...
TEST_P(CoreTest, TestSuites) {
  ...
  struct TestContext {
    ...
    // does not support the Component Model yet. Therefore this structure owns
    // the instances, and VM.ActiveCompInst does not control their lifetime.
  ...
  T.onParse =
    ...
    if (Type == Wast::ModuleType::BinaryFile ||
      ...
      // The source is inline WAT text. The JSON mode does not use it, but
      // this code converts it for completeness.
      WasmEdge::Loader::Loader Ld(Conf);
      EXPECTED_TRY(auto ModPtr,
                   Ld.parseModule(Span<const uint8_t>(
                       reinterpret_cast<const uint8_t *>(Source.data()),
  ...
  T.onInstantiate = [](SpecTest::ContextHandle Ctx, const std::string &ModName,
    ...
    // possible. A ComponentSection owns a unique_ptr<Component>. Therefore
