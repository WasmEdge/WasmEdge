...
public:
  Expect<void> body(const WasmEdge::Runtime::CallingFrame &CF) {
    // Collect scans only the registered roots (value stack, tables, globals,
    // host-roots), NOT the native stack. The interpreter fully controls the
    // value stack at each collection point, so dropped arrays are reclaimed
    // deterministically; native-stack scanning (as AOT does) could retain a
    // just-dropped object and make the exact usage asserts flaky.
    EXPECT_TRUE(CF.getExecutor()->getAllocator().manualCollect());
    return {};
  }
};
...
TEST(GC, gc) {
  ...
  // cycle between rec snapshots makes the exact byte asserts flaky.
  ...
  // element arrays from that formula instead of hard-coding them.
  constexpr auto GCBytes = [](uint32_t N) constexpr {
    return sizeof(WasmEdge::GC::Allocator::Header) +
           sizeof(WasmEdge::Runtime::Instance::GCInstance::RawData) +
           static_cast<size_t>(N) * sizeof(WasmEdge::ValVariant);
  };
  const size_t Bytes4 = GCBytes(4);
  const size_t Bytes8 = GCBytes(8);
  const size_t Bytes12 = GCBytes(12);
  ...
  EXPECT_EQ(Result[6], Bytes8);
  // The 160-byte array survives: born-gray keeps an object through the first
  // cycle after allocation (reclaimed only once a later cycle finds it white).
  EXPECT_EQ(Result[7], Bytes8);
  EXPECT_EQ(Result[8], 0);
  EXPECT_EQ(Result[9], Bytes12);
  EXPECT_EQ(Result[10], Bytes12);
  EXPECT_EQ(Result[11], Bytes12);
