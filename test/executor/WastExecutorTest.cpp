...
TEST_P(WastCoreTest, TestSuites) {
  ...
  T.onParse =
    ...
    if (Type == Wast::ModuleType::Text) {
      ...
      // File path to a .wat file - read content and parse. Open in binary mode
      // so the exact bytes are preserved (no CRLF->LF translation on Windows),
      // keeping tree-sitter's byte offsets aligned with the on-disk file.
      std::ifstream Ifs{std::string(Source), std::ios::binary};
      if (!Ifs) {
        spdlog::error("Cannot open WAT file: {}"sv, Source);
