...
///
/// \file
/// Runs Wasm spec tests directly from .wast files via the tree-sitter WAST
/// parser.
///
//===----------------------------------------------------------------------===//
...
TEST_P(WastCoreTest, TestSuites) {
  ...
  struct TestContext {
    // SpecTestMod must precede VM: members are destroyed in reverse declaration
    // order, and VM (with every module importing "spectest") pins SpecTestMod
    // as a dependent. Destroying it while dependents remain trips the
    // assuming(!hasDependents()) check in ModuleInstance's destructor. See
