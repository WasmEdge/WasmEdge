...
TEST(GCThread, SharedReferencesAcrossThreads) {
  // Threads share one GC array via a global; each writes its own index.
  Configure Conf;
  Conf.addProposal(Proposal::GC);
  ...
  // Thread i writes (i+1)*10 to index i and reads it back after GC.
  std::vector<std::thread> Threads;
