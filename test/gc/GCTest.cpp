...
// R7-M3 capability gate test: manual Loader/Compiler/JIT path so a module's
...
// E4.1 load-bearing check: Executor::expandGCRefType
...
TEST(GC, ExpandGCRefTypeNormalizesConcreteSlotRef) {
  ...
  // ---- Concrete ref in an exported GLOBAL slot (populated by struct.new) ----
  {
    ...
    // was normalized on the way in) -- see the M1 fallback.
...
TEST(GCThread, TableCanHoldManagedResolvedAtConstruction) {
  // E2.0: the can-hold-managed bit is resolved ONCE, at
  // ModuleInstance::addTable's call site (spec 4.2) -- the one place that has
...
TEST(GCThread, SetAllocatorRejectsForeignGrowableTable) {
  // E2.1: TableInstance::setAllocator is the single, fallible, centralized
  // owner-attach check (spec 4.2). A SECOND, FOREIGN controller may not attach
...
// registerModule walk is what attaches it (spec 4.2, E2.2).
...
TEST(GCThread, RegisterModuleRejectsForeignGrowableTable) {
  // E2.2: registerModule walks a prebuilt module's owned tables/globals and
  // attaches them to the registering executor's allocator, validating ownership
  // (spec 4.2). A managed/growable table already owned by ANOTHER controller is
...
TEST(GCThread, RegisterModuleRollsBackClaimWhenPublicationFails) {
  // E2.2 transaction boundary: the ownership claim spans the WHOLE registration,
  ...
  // would make a later registration of the SAME module into a DIFFERENT executor
  // fail the E2.1 foreign-hazard check, turning a recoverable name conflict into
  // a permanent cross-executor rejection. Both registerModule overloads are
  // covered. Deterministic and single-threaded.
  Configure Conf;
  Conf.addProposal(Proposal::GC);
  ...
  // --- Default-name overload ----------------------------------------------
  {
    ...
    // registrable into a different GC executor. Without the rollback its managed
    // table would still be owned by A, so this would fail the E2.1
...
TEST(GCThread, ControllerOwnedTableRejectsRawCApiMutation) {
  // E2.2: the direct C-API table mutators (WasmEdge_TableInstanceSetData /
  // WasmEdge_TableInstanceGrow) refuse a raw mutation on a table a GC controller
  // manages -- the C-API cannot prove the caller's controller matches the owner.
  // The gate is TableInstance::isManagedByController(); the C-API wrappers do
  // exactly `if (isManagedByController()) reject; else mutate`. The GC suite
  // cannot link libwasmedge, so the gate decision AND the still-open paths are
  // exercised here directly against real TableInstances. Deterministic and
  // single-threaded.
  GC::Controller Ctrl;
  GC::Allocator &Alloc = Ctrl.getAllocator();
...
TEST(GCThread, LegacyManagedRefGettersReturnSentinel) {
  // E4.0: WasmEdge_GlobalInstanceGetValue / WasmEdge_TableInstanceGetData
  // (lib/api/wasmedge.cpp) restrict a managed-ref slot's legacy BORROWED
  // getter to a documented typed-null sentinel + spdlog warning instead of
  // handing out the live, unretained value -- a raw reference not added to
  // HostRoots that a concurrent collection could reclaim while the host
  // still holds it (see GlobalInstance::getValue's doc-comment). The gate is
  // `CanHoldManaged() && !RefType.isExternRefType()`
  // (isRestrictedManagedRefGetterType, shared between wasmedge.cpp and this
  // test via include/api/internal/managed_ref_getter.h), and the sentinel is
  // `RefVariant(RefType)` (genManagedRefGetterSentinel, wasmedge.cpp only --
  // it returns a C-API type this test does not need). externref is
  // deliberately excluded -- it is the host-facing reference primitive and
  // the standard C-API workflow round-trips a host pointer through it (see
  // APIUnitTest.cpp's externref table get/set round trip), which must keep
  // working, mirroring TableInstance::isManagedByController's (E2.2)
  // externref exclusion from the raw-mutation gate.
  //
  // The GC suite cannot link libwasmedge (no wasmedgeCAPI dependency; see
  // GCThread.ControllerOwnedTableRejectsRawCApiMutation above), so this test
  // cannot call the exported WasmEdge_GlobalInstanceGetValue /
  // WasmEdge_TableInstanceGetData symbols directly. Instead it calls the
  // REAL isRestrictedManagedRefGetterType predicate (not a hand-rederived
  // copy of the boolean) against real GlobalInstance/TableInstance objects,
  // so a mis-wiring of the gate in wasmedge.cpp (e.g. `||` for `&&`, a
  // dropped `!`) would fail this test too. It also confirms (a) the
  // underlying C++ getValue()/getRefAddr() this C-API wraps is UNCHANGED and
  // still returns the live borrowed value, and (b) the sentinel
  // RefVariant(RefType) it is replaced with is null and distinct from that
  // live value. test/api/APIUnitTest.cpp separately drives the actual
  // exported C symbols for the non-managed exclusion paths the C-API CAN
  // construct (numeric/funcref/externref) -- the C-API has no minting
  // function for internal GC refs, so it cannot construct the managed case
  // this test covers. Deterministic and single-threaded.
  ...
  // sentinel instead of this live value.
  {
    ...
    EXPECT_TRUE(isRestrictedManagedRefGetterType(
        ...
        << "gate condition: canHoldManaged() && !isExternRefType() -> "
           "restricted";
    // getValue() itself (E4.0 leaves it untouched) still returns the live,
  ...
  // working -- the gate must NOT fire despite CanHoldManaged().
  {
    ...
    EXPECT_FALSE(isRestrictedManagedRefGetterType(
        ...
        << "gate condition: canHoldManaged() && !isExternRefType() -> false "
           "-> externref getter unaffected (E2.2-style exclusion)";
...
TEST(GCThread, RetainedManagedRefGetterSurvivesCollection) {
  // E4.1 (spec 4.4): the producer-bearing RETAINED getters
  // (WasmEdge_GlobalInstanceGetValueRetained /
  ...
  // suite can construct (mirrors E4.0). Deterministic and single-threaded.
  using RawData = Runtime::Instance::GCInstance::RawData;
  Configure Conf;
  Conf.addProposal(Proposal::GC);
...
// controller (~Controller::beginClosing) while an in-flight multi-mutator
// collection still has a Running-but-unacked mutator must NOT hang. The
// coordinator spins in waitForAcks for that ack; a mutator that bailed its safe
// point on Closing never delivers it (and Registration::reset leaves its entry
// lingering Running during teardown), so pre-fix the wait spins forever, the
// coordinator never returns from collect(), never releases its launch lease, and
// the drain in beginClosing() (which VM.reset()/~VM ultimately invokes) hangs.
//
// This is modelled deterministically at the Controller level -- the exact
// machinery an async VM invocation uses (a launch lease + a registered value
// stack), driven directly so the interleaving is fixed rather than raced:
//
//   * Mutator M: holds a lease and registers a value stack (Running), then never
//     acknowledges -- it just waits. This is the mutator that, at real teardown,
//     bailed its safe point on Closing. When released it drops its Registration
//     (which during Closing lingers, NOT erased) and its lease.
//   * Coordinator C: holds a lease and registers a value stack, then calls the
//     real collect(), whose STW #1 waitForAcks blocks on M's missing ack.
//   * Teardown T: calls beginClosing() (what ~Controller does), which publishes
//     Closing and then drains until every lease is released.
//
// A VM-level asyncExecute reproduction is unreliable: after a mutator bails its
// safe point it re-enters the coll host call, which flips it NativeRunning for a
// window in which waitForAcks skips it and the coordinator escapes even WITHOUT
// the fix -- masking the bug. Driving the Controller directly keeps M strictly
// Running-unacked, so the deadlock is deterministic.
//
// Pre-fix: C's waitForAcks never observes Closing, so even after M releases its
// lease the coordinator stays wedged, its lease outstanding, and beginClosing
// never returns -> the watchdog converts the hang into a FAILURE + hard-exit.
// Post-fix: waitForAcks abandons on Closing, C finishes collect() and releases
// its lease, and beginClosing drains cleanly.
...
TEST(GCThread, GrowBlocksDuringCollectAndViceVersa) {
  // E1.1: Controller::collect() now owns the exclusive-operation token for the
  ...
  // arbiter until E1.2). The grower must PARK -- not acquire -- until the
...
// Phase-1 AOT shadow-root prototype: a managed ref held ONLY in a native-frame
...
// Phase-1 R7-M1: the stable shadow-head cell must keep a FIXED address across
// Entries-vector relocation. Registering many threads forces the vector to
// reallocate repeatedly; a head pointer taken before must still equal the head
// looked up after, with an unchanged incarnation.
...
// R7-M2 (Rev 9): an abnormal fault must reset the thread's shadow-root head to
...
// Fault (setjmp) + emitFault (longjmp); the published frame lives in a scope the
// longjmp abandons, so after recovery the head must equal the boundary
...
// Positive control for the truncation test: with the Fault NOT armed, emitFault's
// longjmp leaves the head pointing at the abandoned compiled frame -- exactly the
// dangling state R7-M2 fixes, and what the recovery C++ would then clobber. This
// asserts the dangling ADDRESS (never dereferences it), so it stays memory-safe.
...
// R7-M2 (Rev 9): nested compiled boundaries each truncate to THEIR OWN boundary
// value. An inner fault resets the head to the frame the OUTER boundary
// published, not all the way to null -- proving per-boundary O(1) truncation
// composes across compiled->host->compiled reentry (localHandler is a stack of
// Faults; emitFault truncates the innermost).
...
// R7-M2 (Rev 9) state restore: an abnormal fault's longjmp skips the NativeScope
...
// R7-M2 (Rev 9): when the boundary was itself NativeRunning (a host->guest
...
// Phase-1 AOT codegen thesis: a JIT-compiled function allocates a struct into a
...
// ONLY if the compiler-emitted shadow spill published the ref for the collector.
...
// A1 (round-2): a GC cooperative safepoint poll at loop back-edges lets a
...
// to release the loop. Without the poll this DEADLOCKS: the collect waits for an
// ack that never comes, and the release is gated on the collect returning. With
// it, the mutator parks+acks, the collect completes, the loop is released, and
// run returns. Shared memory + atomic accesses keep the signals race-free
...
// A2 (round-2): two mutators sharing one reference-typed global. In the multi-
// mutator model async invocations share the same GlobalInstance, so one thread's
// compiled global.set of a struct ref runs concurrently with another thread's
// global.get of the same 128-bit (type, pointer) slot. With A2's coherent store
// and coherent load (kCoherentRefStore/kCoherentRefLoad) the shared-slot access
// is race-free and can never form a torn pair; the pre-A2 codegen (a bare 128-
// bit load/store) tears the pair and TSan flags the data race. (The marker is
...
//   (func (export "get") (param $n) (result i32) <loop: acc += !ref.is_null(global.get $g)>)
...
// A2 tables: the table analog of CoherentRefGlobalConcurrentAccess. Two async
// mutators share one table element -- one compiled `set` loop (table.set
// element 0 = struct.new), one compiled `get` loop (ref.is_null(table.get 0)).
// Table elements are always managed refs; the compiled coherent store/load
// (kCoherentRefStore / kCoherentRefLoad, routed through TableInstance's coherent
// accessors in the interpreter) make the shared 128-bit slot access race-free. A
// bare 128-bit load/store (pre-A2) tears the (type, pointer) pair and TSan flags
// the race. Validated race-clean under TSan; the plain-access control reports it.
//   (table $t 1 (ref null $s))
//   (func (export "set") (param $n) <loop: table.set $t 0 = struct.new>)
//   (func (export "get") (param $n) (result i32) <loop: acc += !ref.is_null(table.get $t 0)>)
...
// A3 (round-2): a reallocating table.grow must not free the old Refs buffer
...
// the backing buffer), the other repeatedly table.get element 0 (reading through
// the table's DataPtr). A collector thread runs concurrent collects, which free
// the buffers grow retired -- at a stop-the-world root scan, when every mutator
// is parked so none holds an in-flight pointer into a retired buffer. Without the
// fix, grow's realloc frees the buffer out from under the reader (ASan reports
// heap-use-after-free); with it (retire + free-at-STW) the read always hits live
// memory.
//   (table $t 1 (ref null $s))
//   (func (export "grow") (param $n) <loop: table.grow $t (ref.null) 1>)
//   (func (export "read") (param $n) (result i32) <loop: acc += !ref.is_null(table.get $t 0)>)
...
// E1.2: an INTERPRETER table.grow reallocates the Refs vector object that a
...
// `Refs = std::move(New)`). The A3 fix made the AOT DataPtr load atomic, but the
// interpreter reads the vector object directly, so that path left a residual
// data race (spec 3.4). Making table growth an exclusive mutator-parking STW
...
//   (func (export "grow") (param $n)  <loop: drop (table.grow $t (ref.null) 1)>)
//   (func (export "getset") (param $n) (result i32)
...
TEST(GCThread, InterpreterGrowExcludesConcurrentReader) {
  ...
  // Confirm we exercise the interpreter path (the A3 atomic-DataPtr path is
...
TEST(GCThread, GrowCollectArbitration) {
  // E1.2: two growers + one collector contend for the per-controller exclusive
  ...
  // watchdog + _Exit turns a wedge into a failure), (2) the table's max limit is
  // respected exactly -- every grow reads the CURRENT min inside the token, so a
  // loser re-validates the limit a prior winner raised and total successful
  // grows == max (never over-grows), (3) collections keep completing throughout.
  GC::Controller Ctrl;
  GC::Allocator &Alloc = Ctrl.getAllocator();
...
TEST(GCThread, GrowOverLimitDoesNotStopTheWorld) {
  // E1.2 fix: a definitely-doomed (over-limit) grow is rejected BEFORE the
  // exclusive token / handshake, so a guest `table.grow <huge>` loop cannot force
  // a global stop-the-world every iteration. Prove it by holding a collection's
  // token OPEN (pinned sweep, as GrowBlocksDuringCollectAndViceVersa does): a
  // token-taking grow would PARK behind the collection, but an over-limit grow
  // must still return the failure value promptly, never queueing for the token.
  GC::Controller Ctrl;
  GC::Allocator &Alloc = Ctrl.getAllocator();
  Alloc.setManualGC(true);
...
// E1.3: the interpreter table.grow initializer must be rooted across the grow
// window. runTableGrowOp pops the init ref into a bare local BEFORE growTable
// acquires the exclusive token; since E1.2 a grow-loser can park (Blocked)
...
// collection whose root snapshot runs while we are parked would sweep it and the
// grow would broadcast a dangling ref into the new slots. The fix pins it as a
// scoped boundary root before growTable, released after.
//
...
// but BEFORE the Idle->MarkingRoot CAS; the grower pops+pins+parks in that quiet
// window, then the released collection's stop-the-world snapshot -- gated behind
// the grower's Blocked ack -- scans the scoped roots and must find the pinned
// ref. Remove the pin (tableInstr.cpp) and this test sweeps the init object: an
// ASan heap-use-after-free on the sentinel read below, or memory usage 0.
...
// --- R5-M1a: the consumed-argument window of a mediated call -----------------
//
// (module
//  (type $s (struct (field i32)))
...
// and codegen takes the mediated IsNullBB path -- the exact R5-M1a window,
// where the only copy of the ref lives in the packed native argument buffer.
...
// R5-M1a asked whether a ref consumed as a call argument -- popped off the
// compiler's operand stack into an SSA temporary, then packed into an untyped
// native buffer -- can be missed by a collection before the proxy roots it.
//
...
// setSelfBlocked), and Controller::waitForAcks blocks on every Running entry, so
// no remote root scan can COMPLETE while a mutator sits in the window. The test
// drives exactly that: a collector thread opens STW #1 while the mutator is held
// inside the window, and the mutator observes that the collection has not
// finished. Were the window ever made non-Running -- the regression this guards
...
// R7-M2 Phase-B (Rev 9): a REAL guard-page SIGSEGV inside JIT-compiled code,
// with a real codegen-published shadow frame live for a ref local held across
// the faulting call, must drive emitFault's signal-safe head truncation and
// leave the runtime consistent. $boom does a wasm OOB i32.store -- a hardware
// trap in JIT code (which is NOT sanitizer-instrumented, so the SIGSEGV reaches
// WasmEdge's own Fault handler, not ASan's). run holds $r across the call to
// $boom, so run's codegen publishes $r's shadow frame around it. Unlike the
// Fault* unit tests (hand-published frame), this exercises the real path
// end-to-end: helper.cpp arming + real codegen publish + real trap + emitFault
// truncation + longjmp recovery. Asserts the trap surfaces as MemoryOutOfBounds
// (no process crash) and the VM is reusable afterward (a clean function still
// runs), proving the unwind left arming/handler state balanced.
// (module (type $s (struct (field i32))) (import "host" "collect" (func))
//  (memory 1)
...
// --- R7-M3: executor-enforced GC capability gate ---
//
...
// never yield at a stop-the-world and would publish no native roots. R7-M3
// records that capability per module (AST::Module::getGCCompiled) and the
...
const std::vector<uint8_t> R7M3RefLocalWasm = {
...
bool r7m3RunIsCompiled(bool GCCapable, bool ExecutorGC) {
  ...
  auto Mod = compileWithConfig(CompileConf, R7M3RefLocalWasm);
...
TEST(GCThread, CapabilityGateDeoptsNonCapableModuleUnderGCExecutor) {
  ...
  // is observable.
  EXPECT_TRUE(r7m3RunIsCompiled(/*GCCapable=*/true, /*ExecutorGC=*/false));
  // THE GATE: non-capable + GC-enabled -> deopt to interpreter.
  EXPECT_FALSE(r7m3RunIsCompiled(/*GCCapable=*/false, /*ExecutorGC=*/true));
  ...
  // to endanger it).
  EXPECT_TRUE(r7m3RunIsCompiled(/*GCCapable=*/false, /*ExecutorGC=*/false));
  // Negative control: the deopt is gated on the module's capability, so a
  // capable module under a GC executor runs native.
  EXPECT_TRUE(r7m3RunIsCompiled(/*GCCapable=*/true, /*ExecutorGC=*/true));
...
r7m3InvokeAcrossExecutors(bool CompileGC, bool InstantiateGC, bool InvokeGC,
                          bool *OutWasCompiled) {
  Configure CompileConf;
  if (!CompileGC) {
    CompileConf.removeProposal(Proposal::GC);
...
TEST(GCThread, PerCallCapabilityCheckRefusesForeignNonCapableModule) {
  ...
  auto Refused = r7m3InvokeAcrossExecutors(
  ...
  auto Allowed = r7m3InvokeAcrossExecutors(
  ...
  auto Capable = r7m3InvokeAcrossExecutors(
...
// --- R7-M3 follow-up: durable AOT capability bit + loader admission ---
//
...
// mode skips parsing function bodies (Loader::loadSegment), so a module bound to
// non-capable native code has no instructions left to deopt to. The loader
...
std::unique_ptr<AST::Module> aotRoundTrip(bool GCCapable, bool LoadWithGC,
  ...
  auto ModOrErr = LoaderEngine.parseModule(R7M3RefLocalWasm);
  if (!ModOrErr) {
    return nullptr;
  }
  ...
  if (!CodeGen.codegen(R7M3RefLocalWasm, std::move(*Data), Out)) {
    return nullptr;
  }
...
bool r7m3AotRunIsCompiled(AST::Module &Mod) {
  Configure GCConf; // default: GC enabled
  Executor::Executor Exec(GCConf);
  Runtime::StoreManager Store;
  auto InstOrErr = Exec.instantiateModule(Store, Mod);
...
TEST(GCThread, DurableAOTCapabilityBitSurvivesRoundTrip) {
  ...
  const auto CapablePath = Dir / "r7m3-gc-capable.wasm";
  const auto OffPath = Dir / "r7m3-gc-off.wasm";
  std::error_code EC;
  ...
  // non-capable one, and BOTH must still execute correctly.
  {
    ...
    ASSERT_NE(Capable, nullptr);
    EXPECT_TRUE(!!Capable->getSymbol());
    EXPECT_TRUE(r7m3AotRunIsCompiled(*Capable));
    ...
    // Runs interpreted -- and r7m3AotRunIsCompiled asserts it returns 42, which
    // is what proves the bodies came back rather than being empty.
    EXPECT_FALSE(r7m3AotRunIsCompiled(*NonCapable));
  }
...
// --- E3.0: component core-module capability hole ---
//
...
TEST(GCThread, ComponentCoreModuleCapabilityGated) {
  ...
  auto Mod2 = compileWithConfig(CompileConf, R7M3RefLocalWasm);
