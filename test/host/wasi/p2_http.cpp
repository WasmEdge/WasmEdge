...
std::string getText(const std::vector<uint8_t> &B) {
...
protected:
  void SetUp() override {
    ...
    Insts = newHttpInstances(*Http);
    // The interfaces of one host form one tree, as the VM builds them.
    for (auto &Inst : IoInsts) {
      Inst->setParent(Family);
    }
    for (auto &Inst : Insts) {
      Inst->setParent(Family);
    }
  ...
  // One executor for the test, so a spawned exchange outlives its call.
  ...
  call(Runtime::Instance::ComponentInstance &Inst, std::string_view Name,
    ...
    return HostTest::call(Exec, Inst, Name, std::move(Args));
  }
  ...
  Runtime::Instance::ComponentInstance Family{"wasi"};
  ...
  // Declared last: its tasks go before the hosts they name.
  Configure Conf;
...
TEST_F(WasiP2Http, FieldsFollowTheHeaderRules) {
  ...
  EXPECT_EQ(getText(std::get<1>(List[1])), "*/*");
...
TEST_F(WasiP2Http, PostsOverTheLoopbackAndReadsChunks) {
  ...
  EXPECT_TRUE((Runtime::Component::Wit<
               Expected<Runtime::Component::WitUnit,
                        Runtime::Component::WitUnit>>::from((*SetMethod)[0])));
  Scheme Http;
  auto SetScheme =
  ...
  EXPECT_TRUE(
      (Runtime::Component::Wit<
          Expected<Runtime::Component::WitUnit,
                   Runtime::Component::WitUnit>>::from((*SetAuthority)[0])));
  auto SetPath =
  ...
  EXPECT_FALSE(
      (Runtime::Component::Wit<
          Expected<Runtime::Component::WitUnit,
                   Runtime::Component::WitUnit>>::from((*BadAuthority)[0])));
  ...
  auto Out = Runtime::Component::Wit<
      Expected<Runtime::Component::Own<OutputStream>,
               Runtime::Component::WitUnit>>::from((*Write)[0]);
  ASSERT_TRUE(Out);
  ...
  auto InBody = Runtime::Component::Wit<
      Expected<Runtime::Component::Own<IncomingBody>,
               Runtime::Component::WitUnit>>::from((*Consumed)[0]);
  ASSERT_TRUE(InBody);
  ...
  auto In = Runtime::Component::Wit<
      Expected<Runtime::Component::Own<InputStream>,
               Runtime::Component::WitUnit>>::from((*Stream)[0]);
  ASSERT_TRUE(In);
...
TEST_F(WasiP2Http, ReportsRefusedAndUnsupported) {
  ...
  ASSERT_TRUE(Early);
  EXPECT_FALSE(
      Runtime::Component::Wit<FutureResponseGet::Result>::from((*Early)[0]));
  auto Sub =
      call(*Types, "[method]future-incoming-response.subscribe", {FutureSelf});
  ASSERT_TRUE(Sub);
  ASSERT_TRUE(call(
...
TEST_F(WasiP2Http, IncomingSideServesAHandler) {
  ...
  auto R = std::make_unique<IncomingRequest>();
  R->M.Case = Method::Put;
  ...
  R->Body = std::make_unique<IncomingBody>();
  R->Body->Data = bytes("payload");
  const uint64_t ReqRep = Http->IncomingRequests.add(std::move(R));
  const uint64_t ParamRep =
      Http->Outparams.add(std::make_unique<ResponseOutparam>());
  ...
  EXPECT_FALSE((Runtime::Component::Wit<
                Expected<Runtime::Component::Own<IncomingBody>,
                         Runtime::Component::WitUnit>>::from((*Twice)[0])));
  auto Stream = call(*Types, "[method]incoming-body.stream",
                     {Runtime::Component::Wit<IncomingBodyBorrow>::into(
                         IncomingBodyBorrow{InBody->Rep})});
  ASSERT_TRUE(Stream);
  ...
  EXPECT_TRUE((Runtime::Component::Wit<
               Expected<Runtime::Component::WitUnit,
                        Runtime::Component::WitUnit>>::from((*SetStatus)[0])));
  ...
  ASSERT_TRUE(Set);
  auto Param = Http->Outparams.remove(ParamRep);
  ASSERT_TRUE(Param);
  ...
  auto Sent = Http->OutgoingBodies.remove(Response.BodyRep);
  ASSERT_TRUE(Sent);
  EXPECT_EQ(getText(Sent->Data), "pong");
  EXPECT_TRUE(Sent->Finished);
  ASSERT_TRUE(Sent->Trailers);
  EXPECT_EQ(Sent->Trailers->first("x-checksum"), "abc");
  EXPECT_EQ(Http->Outparams.size(), 0u);
  EXPECT_EQ(Http->OutgoingBodies.size(), 0u);
}
