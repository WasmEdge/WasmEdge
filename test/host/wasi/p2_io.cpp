...
TEST_F(WasiP2Io, StandardStreamsOverPipes) {
  auto Stdin = std::make_unique<NodeInputStream>(*Io, Env.stdioNode(0),
                                                 NodeInputStream::Mode::Fd);
  const uint64_t InRep = Io->inputs().add(std::move(Stdin));
  auto Stdout = std::make_unique<NodeOutputStream>(*Io, Env.stdioNode(1),
                                                   NodeOutputStream::Mode::Fd);
  const uint64_t OutRep = Io->outputs().add(std::move(Stdout));
...
TEST_F(WasiP2Io, PollWaitsForTheEarliestDeadline) {
  auto Clocks = newClocksInstances(*Io);
  ...
  // The deadline counts from the instant read here, so the clock of the test
  // starts before it.
  ...
  auto Soon = call(*Clocks[1], "subscribe-instant",
                   {ComponentValVariant{getU64(Now) + 20000000}});
  ASSERT_TRUE(Later);
  ASSERT_TRUE(Soon);
...
TEST_F(WasiP2Io, ErrorsCarryTheirMessage) {
  const uint64_t Rep = Io->errors().add(
      std::make_unique<IoError>(IoError{__WASI_ERRNO_IO, "boom"}));
  auto Res =
