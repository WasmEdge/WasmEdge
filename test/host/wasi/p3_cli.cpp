...
TEST(WasiP3Cli, StdioDeclareTheirStreamShapes) {
  ...
  ASSERT_TRUE(Read->getFuncType().getResult().has_value());
  const auto *Tuple =
      *In.getType(Read->getFuncType().getResult()->getTypeIndex());
...
TEST(WasiP3Cli, TerminalInterfacesShareTheResourceTypes) {
  ...
  const auto *Opt =
      *Insts[8]->getType(Get->getFuncType().getResult()->getTypeIndex());
