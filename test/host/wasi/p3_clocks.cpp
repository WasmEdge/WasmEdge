...
TEST(WasiP3Clocks, MonotonicClockDeclaresItsInterface) {
  ...
  ASSERT_TRUE(Now->getFuncType().getResult().has_value());
  EXPECT_EQ(*Now->getFuncType().getResult(),
  ...
  EXPECT_FALSE(WaitFor->getFuncType().getResult().has_value());
...
TEST(WasiP3Clocks, MonotonicNowIsMonotone) {
  ...
  ASSERT_TRUE(First);
  ASSERT_TRUE(Second);
  EXPECT_LE(getU64(First), getU64(Second));
}
...
TEST(WasiP3Clocks, WaitUntilInThePastReturnsAtOnce) {
  ...
  ASSERT_TRUE(Now);
  const auto T0 = std::chrono::steady_clock::now();
  auto Res = call(Clock, "wait-until", {ComponentValVariant{getU64(Now) - 1}});
  ASSERT_TRUE(Res);
...
TEST(WasiP3Clocks, SystemNowIsARecentInstant) {
  ...
  ASSERT_TRUE(Now->getFuncType().getResult().has_value());
  const auto *Ty =
      *Clock.getType(Now->getFuncType().getResult()->getTypeIndex());
