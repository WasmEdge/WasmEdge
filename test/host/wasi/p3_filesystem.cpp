...
// A fresh directory preopened as the guest's root; it is relative, since a
// drive letter would split the `guest:host` preopen.
class WasiP3Filesystem : public testing::Test {
protected:
  void SetUp() override {
    Root = fs::path("wasmedge_p3_filesystem_test");
    fs::remove_all(Root);
    fs::create_directories(Root);
    const std::vector<std::string> Dirs{"/:" + Root.string()};
    Env.init(Dirs, "prog", {}, {});
    Insts = newFilesystemInstances(Env);
...
TEST_F(WasiP3Filesystem, MissingFilesReportNoEntry) {
  ...
  ASSERT_TRUE(Res);
  const auto &Payload =
      std::get<ResultVal>(Runtime::Component::valComp((*Res)[0]).V).Payload;
  ASSERT_TRUE(Payload.has_value());
  // no-entry is the 20th case of the 0.3 variant.
  const auto &Val = Runtime::Component::WitVariant::value(*Payload);
