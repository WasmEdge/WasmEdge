...
TEST_F(WasiP3Http, RequestAndResponseFields) {
  ...
  const uint64_t Rep = Http->Requests.add(std::move(R));
  ...
  EXPECT_FALSE((Runtime::Component::Wit<
                Expected<Runtime::Component::WitUnit,
                         Runtime::Component::WitUnit>>::from((*SetBad)[0])));
  auto SetA =
  ...
  const uint64_t RespRep = Http->Responses.add(std::make_unique<Response>());
  auto RespSelf =
  ...
  EXPECT_FALSE((Runtime::Component::Wit<
                Expected<Runtime::Component::WitUnit,
                         Runtime::Component::WitUnit>>::from((*BadStatus)[0])));
}
