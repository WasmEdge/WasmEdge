...
TEST_F(WasiP3Sockets, TcpBindConnectAndOptions) {
  ...
  // Windows reports the hop limit and the keep-alive timing as not supported.
  ...
  const uint64_t Refusing = createTcp();
  // Windows cannot report a refused connect yet.
#if !WASMEDGE_OS_WINDOWS
  auto Refused =
      status("[method]tcp-socket.connect",
             {tcp(Refusing),
...
TEST_F(WasiP3Sockets, UdpDatagramsRoundTrip) {
  ...
  // Windows reports the hop limit as not supported.
#if !WASMEDGE_OS_WINDOWS
