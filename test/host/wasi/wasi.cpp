...
TEST(WasiTest, SocketUDP) {
  ...
  // Open and Close udp socket
  {
    ...
    auto *AddrBufSend =
        MemInst.getPointer<sockaddr_in *>(AddrBufPtr, sizeof(sockaddr_in));

    AddrBufSend->sin_family = AF_INET;
    AddrBufSend->sin_port = htons(Port);
    AddrBufSend->sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    Addr->buf_len = sizeof(sockaddr_in);

    WasiSockSendTo.run(
        &MemInst,
        std::array<WasmEdge::ValVariant, 6>{FdClient, MsgInPackPtr, UINT32_C(1),
                                            AddrPtr, UINT32_C(0), SendtoRetPtr},
        Errno);
