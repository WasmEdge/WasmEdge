...
TEST_F(WasiCryptoTest, ChachaPoly) {
  {
    writeString("CHACHA20-POLY1305", 0);
    writeOptOptions(std::nullopt, 17);
    EXPECT_EQ(testRun<Symmetric::KeyGenerate>({0, 17, 17, 25}),
              __WASI_CRYPTO_ERRNO_SUCCESS);
  }
  auto KeyHandle = *MemInst.getPointer<__wasi_symmetric_key_t *>(25);
  ...
  {
    writeString("CHACHA20-POLY1305", 0);
    writeOptKey(KeyHandle, 17);
    writeOptOptions(OptionsHandle, 25);
    EXPECT_EQ(testRun<Symmetric::StateOpen>({0, 17, 17, 25, 33}),
              __WASI_CRYPTO_ERRNO_SUCCESS);
  }
  auto State1Handle = *MemInst.getPointer<__wasi_symmetric_state_t *>(33);
...

  {
    ...
    EXPECT_EQ(
        testRun<Symmetric::StateEncrypt>({State1Handle, 0, 20, 20, 4, 24}),
        __WASI_CRYPTO_ERRNO_SUCCESS);
  }
  ...
  auto State2Handle = *MemInst.getPointer<__wasi_symmetric_state_t *>(33);
  ...
  auto State3Handle = *MemInst.getPointer<__wasi_symmetric_state_t *>(33);
  ...
  auto State4Handle = *MemInst.getPointer<__wasi_symmetric_state_t *>(33);
