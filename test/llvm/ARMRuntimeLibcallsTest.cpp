...
RuntimeLibcallExecutionEngine makeRuntimeLibcallExecutionEngine() {
  ...
  LLModule->setModuleInlineAsm(llvm::StringRef());
  for (auto &Func : *LLModule) {
...
TEST(ARMRuntimeLibcallsCoreTest, RoundsIEEEBitsExactly) {
  ...
  const std::array<F32Case, 19> F32Cases{{
      ...
      {0x4b000001, 0x4b000001, 0x4b000001, 0x4b000001, 0x4b000001},
      {0xcb000001, 0xcb000001, 0xcb000001, 0xcb000001, 0xcb000001},
      {0x7f7fffff, 0x7f7fffff, 0x7f7fffff, 0x7f7fffff, 0x7f7fffff},
  }};
  ...
  const std::array<F64Case, 15> F64Cases{{
      ...
      {0x4330000000000001, 0x4330000000000001, 0x4330000000000001,
       0x4330000000000001, 0x4330000000000001},
      {0xc330000000000001, 0xc330000000000001, 0xc330000000000001,
       0xc330000000000001, 0xc330000000000001},
...
bool containsARMCallInstruction(llvm::StringRef Code, bool Thumb) {
  ...
  if (!Thumb) {
    for (size_t Offset = 0; Offset + 4 <= Code.size(); Offset += 4) {
      const uint32_t Word = static_cast<uint32_t>(U8(Offset)) |
                            (static_cast<uint32_t>(U8(Offset + 1)) << 8) |
                            (static_cast<uint32_t>(U8(Offset + 2)) << 16) |
                            (static_cast<uint32_t>(U8(Offset + 3)) << 24);
  ...
  for (size_t Offset = 0; Offset + 2 <= Code.size();) {
    const uint16_t First =
        static_cast<uint16_t>(U8(Offset) | (U8(Offset + 1) << 8));
    ...
    const uint16_t Second =
        static_cast<uint16_t>(U8(Offset + 2) | (U8(Offset + 3) << 8));
...
.size opposite_mode_caller, . - opposite_mode_caller
    ...
    Module.appendModuleInlineAsm(llvm::StringRef(Caller));
  }
  RuntimeModule.release();
...
const std::set<std::string> &expectedARMPublicABIHelpers() {
  static const std::set<std::string> Symbols = [] {
    std::set<std::string> Result = expectedARMComputationalSymbols();
    Result.insert("__aeabi_lasr");
    Result.insert("__aeabi_llsl");
    Result.insert("__aeabi_llsr");
    return Result;
  }();
  return Symbols;
}
...
TEST(ARMRuntimeLibcallsInventoryTest,
  ...
  const auto &ReviewedPublicABIHelpers = expectedARMPublicABIHelpers();
    ...
    for (const bool Thumb : {false, true})
      for (const bool HardFloat : {false, true}) {
        ...
        ASSERT_TRUE(Graph) << describe(Config) << ": " << Graph.error().Message
                           << " symbol name='" << Graph.error().SymbolName
                           << "'";
  ...
  for (const bool HardFloat : {false, true})
    for (const bool Thumb : {false, true}) {
      ...
      const auto ExpectedHelper = std::vector<Byte>(
          Graph->sections()[HelperSection].Content.begin() +
              static_cast<std::ptrdiff_t>(HelperOffset),
          Graph->sections()[HelperSection].Content.begin() +
              static_cast<std::ptrdiff_t>(HelperOffset + HelperSize));
      std::vector<Byte> ELF;
      Writer Output(ELF);
      ...
      EXPECT_TRUE(std::any_of(
          ...
          [&](const auto &Section) {
            ...
            if (Address > HelperImageAddress ||
              ...
              return false;
            return std::equal(
                HelperBytes.begin(), HelperBytes.end(),
                Content.begin() +
                    static_cast<std::ptrdiff_t>(HelperImageAddress - Address));
          }))
          << describe(Config);
    }
}
