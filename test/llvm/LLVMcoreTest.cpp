...
TEST_P(NativeCoreTest, TestSuites) {
  ...
  T.onParse =
    ...
    if (Type == Wast::ModuleType::BinaryFile ||
      ...
      // Compilation failed. Only malformed/invalid inputs (assert_malformed /
      // assert_invalid tests) fail in the loading or validation phase; fall
      // back to loading the original file so the harness observes that error in
      // the expected phase. A genuine compiler failure on a well-formed module
      // must not be masked as an interpreter pass, so propagate it.
