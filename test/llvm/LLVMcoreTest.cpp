...
TEST_P(NativeCoreTest, TestSuites) {
  ...
  T.onParse =
    ...
    if (Type == Wast::ModuleType::BinaryFile ||
      ...
      // The compilation failed. Only a malformed input or an input that is
      // not valid fails in the load phase or in the validation phase. An
      // assert_malformed test and an assert_invalid test give such an input.
      // Load the original file again, so that the harness sees that error in
      // the expected phase. A real compiler failure on a well-formed module
      // must not look like a pass of the interpreter, so return it.
...
TEST_P(JITCoreTest, TestSuites) {
  ...
  T.onInstantiate = [](SpecTest::ContextHandle Ctx, const std::string &ModName,
    ...
    if (std::holds_alternative<std::unique_ptr<AST::Module>>(Unit)) {
      ...
      // This is a component. The JIT mode does not use it, but this code
...
TEST_P(LazyJITCoreTest, TestSuites) {
  ...
  T.onInstantiate = [](SpecTest::ContextHandle Ctx, const std::string &ModName,
    ...
    if (std::holds_alternative<std::unique_ptr<AST::Module>>(Unit)) {
      ...
      // This is a component. The LazyJIT mode does not use it, but this code
      // returns without an error.
      return {};
    }
  };
...
TEST(AOTNullLocal, AbstractRefTypeIsBottomTyped) {
  ...
  Configure Conf;
  auto Mod = compileToJIT(Conf, NullLocalAbstractWasm);
