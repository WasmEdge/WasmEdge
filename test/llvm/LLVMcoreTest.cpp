...
TEST(GC, gc) {
  ...
  // coll calls (manualCollect(true)) the wasm makes; a background native-stack
  // collection firing between rec snapshots would widen the nondeterminism the
  // range assertions below already account for.
  ...
  // DLL stays loaded (and the file locked) until cleanup, matching the other
  // AOT tests in this file.
  VM.cleanup();
  EXPECT_NO_THROW(std::filesystem::remove(Path));
}
