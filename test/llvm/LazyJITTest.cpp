...
// // Simple module with 6 functions: add, mul, sub, const42, unused1,
// unused2
// // Compiled from:
// std::vector<uint8_t> SimpleWasm = {
//     0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x17, 0x04,
//     0x60, 0x02, 0x7f, 0x7f, 0x01, 0x7f, 0x60, 0x00, 0x01, 0x7f, 0x60,
//     0x01, 0x7f, 0x01, 0x7f, 0x60, 0x03, 0x7f, 0x7f, 0x7f, 0x01, 0x7f,
//     0x03, 0x07, 0x06, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x07, 0x31,
//     0x06, 0x03, 0x61, 0x64, 0x64, 0x00, 0x00, 0x03, 0x6d, 0x75, 0x6c,
//     0x00, 0x01, 0x03, 0x73, 0x75, 0x62, 0x00, 0x02, 0x07, 0x63, 0x6f,
//     0x6e, 0x73, 0x74, 0x34, 0x32, 0x00, 0x03, 0x07, 0x75, 0x6e, 0x75,
//     0x73, 0x65, 0x64, 0x31, 0x00, 0x04, 0x07, 0x75, 0x6e, 0x75, 0x73,
//     0x65, 0x64, 0x32, 0x00, 0x05, 0x0a, 0x32, 0x06, 0x07, 0x00, 0x20,
//     0x00, 0x20, 0x01, 0x6a, 0x0b, 0x07, 0x00, 0x20, 0x00, 0x20, 0x01,
//     0x6c, 0x0b, 0x07, 0x00, 0x20, 0x00, 0x20, 0x01, 0x6b, 0x0b, 0x04,
//     0x00, 0x41, 0x2a, 0x0b, 0x08, 0x00, 0x20, 0x00, 0x41, 0xe4, 0x00,
//     0x6a, 0x0b, 0x0a, 0x00, 0x20, 0x00, 0x20, 0x01, 0x6a, 0x20, 0x02,
//     0x6c, 0x0b};
...
//   // fib(0) = 1, fib(1) = 1, fib(2) = 2, fib(3) = 3, fib(4) = 5, fib(5) =
//   8 std::vector<std::pair<uint32_t, uint32_t>> TestCases = {
...
//   size_t InitialCompiled =
//   VM->getLazyJITState().LazyCompiledFuncs.size();
...
//   size_t AfterAdd = VM->getLazyJITState().LazyCompiledFuncs.size();
...
//   size_t AfterMul = VM->getLazyJITState().LazyCompiledFuncs.size();
//   EXPECT_EQ(AfterMul, InitialCompiled + 2)
//       << "After calling 'mul', compiled count should increase by 1 more";

//   // Call 'add' again - should NOT increase compiled count (already
//   compiled) std::vector<ValVariant> AddP2 = {5U, 6U}; auto AddResult2 =
//   VM->execute("add", AddP2, Types); ASSERT_TRUE(AddResult2);
//   EXPECT_EQ((*AddResult2)[0].first.get<uint32_t>(), 11U);

//   size_t AfterAddAgain = VM->getLazyJITState().LazyCompiledFuncs.size();
...
//   EXPECT_EQ(VM->getLazyJITState().TotalFuncCount, 6U);
//   EXPECT_LT(VM->getLazyJITState().LazyCompiledFuncs.size(),
//             VM->getLazyJITState().TotalFuncCount)
...
//   size_t Compiled = VM->getLazyJITState().LazyCompiledFuncs.size();
//   std::vector<ValType> Types = {ValType(TypeCode::I32),
//   ValType(TypeCode::I32)};
...
//     size_t NewCompiled = VM->getLazyJITState().LazyCompiledFuncs.size();
...
//   auto ConstResult = VM->execute("const42", EmptyP, EmptyT);
//   ASSERT_TRUE(ConstResult);
//   EXPECT_EQ(VM->getLazyJITState().LazyCompiledFuncs.size(), Compiled +
//   1);

//   // We called 4 functions (add, mul, sub, const42) but not unused1,
//   unused2
//   // So 4 functions should be compiled
//   EXPECT_EQ(VM->getLazyJITState().LazyCompiledFuncs.size(), 4U);
...
//   EXPECT_EQ(VM->getLazyJITState().LazyCompiledFuncs.size(), 0U);
//   EXPECT_EQ(VM->getLazyJITState().TotalFuncCount, 0U);
