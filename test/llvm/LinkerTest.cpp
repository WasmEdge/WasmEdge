...
TEST(NativeLinkerTest, AcceptsMatchingARMFloatABI) {
  EXPECT_TRUE(Internal::armFloatABICompatible(
      ObjectFormat::ELF,
      llvm::ELF::EF_ARM_EABI_VER5 | llvm::ELF::EF_ARM_ABI_FLOAT_SOFT, false));
  EXPECT_TRUE(Internal::armFloatABICompatible(
      ObjectFormat::ELF,
      llvm::ELF::EF_ARM_EABI_VER5 | llvm::ELF::EF_ARM_ABI_FLOAT_HARD, true));
}

TEST(NativeLinkerTest, RejectsMismatchedARMFloatABI) {
  EXPECT_FALSE(Internal::armFloatABICompatible(
      ObjectFormat::ELF, llvm::ELF::EF_ARM_ABI_FLOAT_HARD, false));
  EXPECT_FALSE(Internal::armFloatABICompatible(
      ObjectFormat::ELF, llvm::ELF::EF_ARM_ABI_FLOAT_SOFT, true));
}

TEST(NativeLinkerTest, RejectsMissingOrInvalidARMFloatABI) {
  EXPECT_FALSE(Internal::armFloatABICompatible(ObjectFormat::ELF, 0, false));
  EXPECT_FALSE(Internal::armFloatABICompatible(ObjectFormat::ELF, 0, true));
  constexpr uint32_t Both =
      llvm::ELF::EF_ARM_ABI_FLOAT_SOFT | llvm::ELF::EF_ARM_ABI_FLOAT_HARD;
  EXPECT_FALSE(Internal::armFloatABICompatible(ObjectFormat::ELF, Both, false));
  EXPECT_FALSE(Internal::armFloatABICompatible(ObjectFormat::ELF, Both, true));
}

TEST(NativeLinkerTest, BypassesARMFloatABIForNonELFFormats) {
  for (const auto Format : {ObjectFormat::MachO, ObjectFormat::COFF}) {
    EXPECT_TRUE(Internal::armFloatABICompatible(Format, 0, false));
    EXPECT_TRUE(Internal::armFloatABICompatible(
        Format, llvm::ELF::EF_ARM_ABI_FLOAT_HARD, false));
  }
}
...
void append32(std::vector<WasmEdge::Byte> &Bytes, uint32_t Value,
              Endianness Endian = Endianness::Little) {
  for (size_t I = 0; I < 4; ++I) {
    const size_t Shift = Endian == Endianness::Little ? I * 8 : (3 - I) * 8;
    Bytes.push_back(static_cast<WasmEdge::Byte>(Value >> Shift));
  }
}

std::vector<WasmEdge::Byte>
makeMachOBuildVersionCommand(uint32_t Platform, uint32_t MinimumOS,
                             uint32_t SDK,
                             std::initializer_list<MachOBuildToolVersion> Tools,
                             Endianness Endian = Endianness::Little) {
  std::vector<WasmEdge::Byte> Bytes;
  append32(Bytes, llvm::MachO::LC_BUILD_VERSION, Endian);
  append32(Bytes, 24 + static_cast<uint32_t>(Tools.size()) * 8, Endian);
  append32(Bytes, Platform, Endian);
  append32(Bytes, MinimumOS, Endian);
  append32(Bytes, SDK, Endian);
  append32(Bytes, static_cast<uint32_t>(Tools.size()), Endian);
  for (const auto &Tool : Tools) {
    append32(Bytes, Tool.Tool, Endian);
    append32(Bytes, Tool.Version, Endian);
  }
  return Bytes;
}

void expectMachOBuildVersionsEqual(
    const std::vector<MachOBuildVersion> &Actual,
    const std::vector<MachOBuildVersion> &Expected) {
  ASSERT_EQ(Actual.size(), Expected.size());
  for (size_t I = 0; I < Expected.size(); ++I) {
    EXPECT_EQ(Actual[I].Platform, Expected[I].Platform);
    EXPECT_EQ(Actual[I].MinimumOS, Expected[I].MinimumOS);
    EXPECT_EQ(Actual[I].SDK, Expected[I].SDK);
    ASSERT_EQ(Actual[I].Tools.size(), Expected[I].Tools.size());
    for (size_t J = 0; J < Expected[I].Tools.size(); ++J) {
      EXPECT_EQ(Actual[I].Tools[J].Tool, Expected[I].Tools[J].Tool);
      EXPECT_EQ(Actual[I].Tools[J].Version, Expected[I].Tools[J].Version);
    }
  }
}

std::vector<WasmEdge::Byte> makeLoadCommandOnlyMachO(
    std::initializer_list<std::vector<WasmEdge::Byte>> Commands) {
  uint32_t CommandBytes = 0;
  for (const auto &Command : Commands)
    CommandBytes += static_cast<uint32_t>(Command.size());
  std::vector<WasmEdge::Byte> Bytes;
  append32(Bytes, llvm::MachO::MH_MAGIC_64);
  append32(Bytes, llvm::MachO::CPU_TYPE_X86_64);
  append32(Bytes, llvm::MachO::CPU_SUBTYPE_X86_64_ALL);
  append32(Bytes, llvm::MachO::MH_OBJECT);
  append32(Bytes, static_cast<uint32_t>(Commands.size()));
  append32(Bytes, CommandBytes);
  append32(Bytes, 0);
  append32(Bytes, 0);
  for (const auto &Command : Commands)
...
size_t elf64SymbolTableSectionHeader(const std::vector<WasmEdge::Byte> &Bytes) {
  const auto Table = static_cast<size_t>(read64le(Bytes, 40));
  const auto Count = static_cast<size_t>(read32le(Bytes, 60) & 0xFFFF);
  for (size_t Index = 1; Index < Count; ++Index) {
    const auto Header = Table + Index * 64;
    if (read32le(Bytes, Header + 4) == llvm::ELF::SHT_SYMTAB)
      return Header;
  }
  return 0;
}
...
std::vector<WasmEdge::Byte>
makeARMAttributeObject(std::optional<unsigned> VFPArgs,
                       std::string ExtraAssembly = {},
                       std::string Features = {}) {
  std::string Assembly = ".syntax unified\n.text\n.globl f0\nf0:\n bx lr\n";
  if (VFPArgs)
    Assembly += ".eabi_attribute 28, " + std::to_string(*VFPArgs) + "\n";
  Assembly += std::move(ExtraAssembly);
  return makeAssemblyObject(llvm::Triple("armv7-unknown-linux-gnueabi"),
                            std::move(Assembly), std::move(Features));
}

std::vector<std::pair<size_t, size_t>>
armAttributeSections(const std::vector<WasmEdge::Byte> &Bytes) {
  ...
  for (const auto &Section : (*Object)->sections()) {
    ...
    if (ELFSection.getType() == llvm::ELF::SHT_ARM_ATTRIBUTES)
      Result.emplace_back(static_cast<size_t>(ELFSection.getOffset()),
                          static_cast<size_t>(Section.getSize()));
  }
  return Result;
}

void markARMAttributeSection(std::vector<WasmEdge::Byte> &Bytes,
                             llvm::StringRef SectionName, uint32_t Flags = 0) {
  auto Object =
      llvm::object::ObjectFile::createObjectFile(llvm::MemoryBufferRef(
          llvm::StringRef(reinterpret_cast<const char *>(Bytes.data()),
                          Bytes.size()),
  ...
  for (const auto &Section : (*Object)->sections()) {
    ...
    if (Name && *Name == SectionName) {
      const size_t Header = read32le(Bytes, 32) + Section.getIndex() * 40;
      write32le(Bytes, Header + 4, llvm::ELF::SHT_ARM_ATTRIBUTES);
      write32le(Bytes, Header + 8, Flags);
      return;
    }
  }
  FAIL() << "missing section " << SectionName.str();
}
...
#define REQUIRE_NATIVE_TARGET(TARGET)                                          \
  do {                                                                         \
    if (nativeTarget() != (TARGET)) {                                          \
      GTEST_SKIP() << architectureName(TARGET) << " is not the native target"; \
    }                                                                          \
  } while (false)
  ...
  const auto Exidx = std::find_if(
      ...
      [](const auto &Value) {
        ...
        return Address <= 0x2000 && 0x2004 <= Address + Content.size() &&
               std::equal(Content.begin() +
                              static_cast<std::ptrdiff_t>(0x2000 - Address),
                          Content.begin() +
                              static_cast<std::ptrdiff_t>(0x2004 - Address),
  ...
  const auto Generic = std::find_if(
      ...
      [](const auto &Value) {
        ...
        return Address <= 0x4000 && 0x4004 <= Address + Content.size() &&
               std::equal(Content.begin() +
                              static_cast<std::ptrdiff_t>(0x4000 - Address),
                          Content.begin() +
                              static_cast<std::ptrdiff_t>(0x4004 - Address),
...
TEST_F(LinkerOutputTest,
       UniversalWasmWriterKeepsCOFFPDataOutsideMergedSections) {
  REQUIRE_RELOCATION_HANDLER(Target::X86_64);
  REQUIRE_NATIVE_TARGET(Target::X86_64);
  constexpr std::array<WasmEdge::Byte, 34> TinyWasm{
      0x00, 0x61, 0x73, 0x6D, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60,
      0x00, 0x01, 0x7F, 0x03, 0x02, 0x01, 0x00, 0x07, 0x05, 0x01, 0x01, 0x66,
      0x00, 0x00, 0x0A, 0x06, 0x01, 0x04, 0x00, 0x41, 0x07, 0x0B};
  LinkGraph Graph(Target::X86_64, Endianness::Little, ObjectFormat::COFF);
  ASSERT_TRUE(Graph.beginInput("straddled-pdata.obj"));
  auto Text =
      Graph.addSection(Section{".text", SectionKind::Text, 16, 64, 0x100, 0,
                               std::vector<WasmEdge::Byte>(64)});
  auto Data = Graph.addSection(
      Section{".data", SectionKind::Data, 8, 2, 0x200, 0, {0, 0}});
  ASSERT_TRUE(Graph.addSection(
      Section{".pdata", SectionKind::Unwind, 4, 12, 0x400, 0,
              std::vector<WasmEdge::Byte>(12), SectionPurpose::PData}));
  ASSERT_TRUE(Graph.addSection(Section{".xdata", SectionKind::Unwind, 4, 8,
                                       0x600, 0, std::vector<WasmEdge::Byte>(8),
                                       SectionPurpose::XData}));
  ASSERT_TRUE(Text && Data);
  ASSERT_TRUE(Graph.addSymbol(Symbol{"version", *Data, 0, 1, true}));
  ASSERT_TRUE(Graph.addSymbol(Symbol{"intrinsics", *Data, 1, 1, true}));
  ASSERT_TRUE(Graph.addSymbol(Symbol{"f0", *Text, 0, 1, true}));
  write32le(const_cast<Section &>(Graph.sections()[2]).Content, 0, 0x100);
  write32le(const_cast<Section &>(Graph.sections()[2]).Content, 4, 0x110);
  write32le(const_cast<Section &>(Graph.sections()[2]).Content, 8, 0x600);
  ASSERT_TRUE(applyRelocations(Graph));
  const auto Output = Directory / "straddled-pdata.wasm";
  ...
  EXPECT_TRUE(std::none_of(Metadata.Sections.begin(), Metadata.Sections.end(),
                           [](const auto &Value) {
                             return std::get<0>(Value) != 4 &&
                                    std::get<1>(Value) <= 0x400 &&
                                    0x400 <
...
TEST_F(LinkerOutputTest, ReportsRealObjectDiagnosticOnceAtBoundary) {
  ...
  const std::string UndefinedName = "_value";
#else
  const std::string UndefinedName = "value";
#endif
  ...
  EXPECT_EQ(ReadResult.error().SymbolName, UndefinedName);
  ...
  EXPECT_NE(Logs[0].find("symbol name='" + UndefinedName + "'"),
            std::string::npos);
}
...
TEST_F(LinkerOutputTest, NativeMachOWriterSupportsEmptyModule) {
  constexpr std::array<WasmEdge::Byte, 8> EmptyWasm{0x00, 0x61, 0x73, 0x6D,
                                                    0x01, 0x00, 0x00, 0x00};
  const auto Output = Directory / "empty.dylib";
  WasmEdge::Configure Conf;
  Conf.getCompilerConfigure().setOutputFormat(
  ...
  WasmEdge::LLVM::Compiler Compiler(Conf);
  WasmEdge::LLVM::CodeGen CodeGen(Conf);
  auto Module = Loader.parseModule(EmptyWasm);
  ASSERT_TRUE(Module);
  ASSERT_TRUE(Validator.validate(**Module));
  auto Data = Compiler.compile(**Module);
  ASSERT_TRUE(Data);
  ASSERT_TRUE(CodeGen.codegen(EmptyWasm, std::move(*Data), Output));
...
TEST(ObjectReaderTest, ParsesCompleteMachOBuildVersionCommand) {
  const auto Bytes = makeMachOBuildVersionCommand(
      7, UINT32_C(0x01020304), UINT32_C(0xA0B0C0D0),
      {{3, UINT32_C(0x11223344)}, {9, UINT32_C(0xFFEEDDCC)}});

  auto Parsed = Internal::parseMachOBuildVersion(Bytes, Endianness::Little);
  ASSERT_TRUE(Parsed);
  EXPECT_EQ(Parsed->Platform, 7U);
  EXPECT_EQ(Parsed->MinimumOS, UINT32_C(0x01020304));
  EXPECT_EQ(Parsed->SDK, UINT32_C(0xA0B0C0D0));
  ASSERT_EQ(Parsed->Tools.size(), 2U);
  EXPECT_EQ(Parsed->Tools[0].Tool, 3U);
  EXPECT_EQ(Parsed->Tools[0].Version, UINT32_C(0x11223344));
  EXPECT_EQ(Parsed->Tools[1].Tool, 9U);
  EXPECT_EQ(Parsed->Tools[1].Version, UINT32_C(0xFFEEDDCC));
}

TEST(ObjectReaderTest, ParsesBigEndianMachOBuildVersionCommand) {
  const auto Bytes = makeMachOBuildVersionCommand(
      UINT32_C(0x10203040), UINT32_C(0x50607080), UINT32_C(0x90A0B0C0),
      {{UINT32_C(0xD0E0F001), UINT32_C(0x12345678)}}, Endianness::Big);

  auto Parsed = Internal::parseMachOBuildVersion(Bytes, Endianness::Big);
  ASSERT_TRUE(Parsed);
  EXPECT_EQ(Parsed->Platform, UINT32_C(0x10203040));
  EXPECT_EQ(Parsed->MinimumOS, UINT32_C(0x50607080));
  EXPECT_EQ(Parsed->SDK, UINT32_C(0x90A0B0C0));
  ASSERT_EQ(Parsed->Tools.size(), 1U);
  EXPECT_EQ(Parsed->Tools[0].Tool, UINT32_C(0xD0E0F001));
  EXPECT_EQ(Parsed->Tools[0].Version, UINT32_C(0x12345678));
}

TEST(ObjectReaderTest, RejectsMalformedMachOBuildVersionCommands) {
  const auto Valid = makeMachOBuildVersionCommand(1, 2, 3, {{4, 5}});
  std::vector<std::vector<WasmEdge::Byte>> Invalid;
  Invalid.emplace_back(Valid.begin(), Valid.begin() + 23);
  Invalid.push_back(Valid);
  Invalid.back().push_back(0);
  Invalid.push_back(Valid);
  write32le(Invalid.back(), 4, 24);
  Invalid.push_back(Valid);
  write32le(Invalid.back(), 20, 2);
  Invalid.push_back(Valid);
  write32le(Invalid.back(), 20, UINT32_MAX);
  Invalid.push_back(Valid);
  write32le(Invalid.back(), 0, llvm::MachO::LC_UUID);

  for (const auto &Bytes : Invalid) {
    auto Parsed = Internal::parseMachOBuildVersion(Bytes, Endianness::Little);
    EXPECT_FALSE(Parsed);
    if (!Parsed) {
      EXPECT_EQ(Parsed.error().Kind, DiagnosticKind::Malformed);
    }
  }
}

TEST(ObjectReaderTest, PreservesRepeatedMachOBuildVersionsInCommandOrder) {
  REQUIRE_RELOCATION_HANDLER(Target::X86_64);
  const auto First = makeMachOBuildVersionCommand(
      1, UINT32_C(0x000D0000), UINT32_C(0x000E0000), {{3, 10}});
  const auto Second = makeMachOBuildVersionCommand(
      1, UINT32_C(0x000F0100), UINT32_C(0x00100000), {{1, 20}, {4, 30}});
  const auto Bytes = makeLoadCommandOnlyMachO({First, Second});

  auto Graph = ObjectReader::read(Bytes, Target::X86_64);
  ASSERT_TRUE(Graph) << (Graph ? "" : Graph.error().Message);
  ASSERT_EQ(Graph->machOBuildVersions().size(), 2U);
  EXPECT_EQ(Graph->machOBuildVersions()[0].MinimumOS, UINT32_C(0x000D0000));
  EXPECT_EQ(Graph->machOBuildVersions()[0].Tools[0].Version, 10U);
  EXPECT_EQ(Graph->machOBuildVersions()[1].MinimumOS, UINT32_C(0x000F0100));
  ASSERT_EQ(Graph->machOBuildVersions()[1].Tools.size(), 2U);
  EXPECT_EQ(Graph->machOBuildVersions()[1].Tools[0].Version, 20U);
  EXPECT_EQ(Graph->machOBuildVersions()[1].Tools[1].Version, 30U);
}

TEST(ObjectReaderTest, LeavesMachOBuildVersionsEmptyWhenCommandIsAbsent) {
  REQUIRE_RELOCATION_HANDLER(Target::X86_64);
  const auto Bytes = makeLoadCommandOnlyMachO({});

  auto Graph = ObjectReader::read(Bytes, Target::X86_64);
  ASSERT_TRUE(Graph) << (Graph ? "" : Graph.error().Message);
  EXPECT_TRUE(Graph->machOBuildVersions().empty());
}

TEST(ObjectReaderTest, ExtractsBuildVersionFromLLVMGeneratedMacOSObject) {
  REQUIRE_LLVM_TARGET("x86_64-apple-macosx10.15.0");
  REQUIRE_RELOCATION_HANDLER(Target::X86_64);
  const auto Bytes = makeObject(llvm::Triple("x86_64-apple-macosx10.15.0"));

  auto Graph = ObjectReader::read(Bytes, Target::X86_64);
  ASSERT_TRUE(Graph) << (Graph ? "" : Graph.error().Message);
  ASSERT_FALSE(Graph->machOBuildVersions().empty());
  EXPECT_EQ(Graph->machOBuildVersions()[0].Platform, 1U);
  EXPECT_EQ(Graph->machOBuildVersions()[0].MinimumOS, UINT32_C(0x000A0F00));
}

TEST(ObjectReaderTest, PropagatesMalformedMachOLoadCommandDiagnostics) {
  auto Bytes =
      makeLoadCommandOnlyMachO({makeMachOBuildVersionCommand(1, 2, 3, {})});
  write32le(Bytes, 32 + 4, 32);

  auto Graph = ObjectReader::read(Bytes, Target::X86_64);
  ASSERT_FALSE(Graph);
  EXPECT_EQ(Graph.error().Kind, DiagnosticKind::Malformed);
  EXPECT_NE(Graph.error().Message.find("object file parse error"),
            std::string::npos);
}

TEST(LinkGraphTest, StoresMachOBuildVersionsInInsertionOrder) {
  LinkGraph Graph(Target::AArch64, Endianness::Little, ObjectFormat::MachO);
  ASSERT_TRUE(Graph.addMachOBuildVersion(
      MachOBuildVersion{1, 0x000D0000, 0x000E0000, {{3, 0x004A0000}}}));
  ASSERT_TRUE(Graph.addMachOBuildVersion(MachOBuildVersion{
      2, 0x000F0100, 0x00100000, {{1, 0x000F0000}, {4, 0x00100000}}}));

  ASSERT_EQ(Graph.machOBuildVersions().size(), 2U);
  const auto &First = Graph.machOBuildVersions()[0];
  EXPECT_EQ(First.Platform, 1U);
  EXPECT_EQ(First.MinimumOS, 0x000D0000U);
  EXPECT_EQ(First.SDK, 0x000E0000U);
  ASSERT_EQ(First.Tools.size(), 1U);
  EXPECT_EQ(First.Tools[0].Tool, 3U);
  EXPECT_EQ(First.Tools[0].Version, 0x004A0000U);
  const auto &Second = Graph.machOBuildVersions()[1];
  EXPECT_EQ(Second.Platform, 2U);
  ASSERT_EQ(Second.Tools.size(), 2U);
  EXPECT_EQ(Second.Tools[0].Tool, 1U);
  EXPECT_EQ(Second.Tools[1].Tool, 4U);

  LinkGraph ELFGraph(Target::X86_64, Endianness::Little, ObjectFormat::ELF);
  auto Rejected = ELFGraph.addMachOBuildVersion(MachOBuildVersion{});
  ASSERT_FALSE(Rejected);
  EXPECT_EQ(Rejected.error().Message,
            "Mach-O build version requires a Mach-O link graph");
  EXPECT_TRUE(ELFGraph.machOBuildVersions().empty());
}

TEST(LinkGraphTest, RejectsMachOBuildVersionAfterRelocations) {
  REQUIRE_RELOCATION_HANDLER(Target::X86_64);
  LinkGraph Graph(Target::X86_64, Endianness::Little, ObjectFormat::MachO);
  ASSERT_TRUE(Graph.beginInput("input.o"));
  ASSERT_TRUE(applyRelocations(Graph));

  auto Result = Graph.addMachOBuildVersion(
      MachOBuildVersion{1, 0x000D0000, 0x000E0000, {}});
  ASSERT_FALSE(Result);
  EXPECT_EQ(Result.error().Message, "link graph relocations already applied");
  EXPECT_TRUE(Graph.machOBuildVersions().empty());
}

TEST(LinkGraphTest, PreservesMachOBuildVersionWhenPruningEHFrame) {
  LinkGraph Graph(Target::X86_64, Endianness::Little, ObjectFormat::MachO);
  ASSERT_TRUE(Graph.beginInput("input.o"));
  auto Text =
      Graph.addSection(Section{"__text", SectionKind::Text, 1, 1, 0, 0, {0}});
  ASSERT_TRUE(Text);
  auto Function =
      Graph.addSymbol(Symbol{"_f", *Text, 0, 1, false, std::nullopt, true});
  ...
  ASSERT_TRUE(Graph.addCompactUnwind(CompactUnwindRecord{
      *Function, 1, 0, std::nullopt, std::nullopt, std::nullopt}));
  ASSERT_TRUE(Graph.addMachOBuildVersion(
      MachOBuildVersion{1, 0x000D0000, 0x000E0000, {{3, 0x004A0000}}}));

  ASSERT_TRUE(Graph.pruneUnreferencedMachOEHFrame());
  ASSERT_EQ(Graph.sections().size(), 1U);
  ASSERT_EQ(Graph.machOBuildVersions().size(), 1U);
  EXPECT_EQ(Graph.machOBuildVersions()[0].Platform, 1U);
  ASSERT_EQ(Graph.machOBuildVersions()[0].Tools.size(), 1U);
  EXPECT_EQ(Graph.machOBuildVersions()[0].Tools[0].Version, 0x004A0000U);
}

TEST(LinkGraphTest, CopiesMovesAndSwapsMachOBuildVersions) {
  const MachOBuildVersion Version{
      1, 0x000D0000, 0x000E0000, {{3, 0x004A0000}, {4, 0x000E0100}}};
  LinkGraph Source(Target::AArch64, Endianness::Little, ObjectFormat::MachO);
  ASSERT_TRUE(Source.addMachOBuildVersion(Version));

  LinkGraph Copied(Source);
  ASSERT_EQ(Copied.machOBuildVersions().size(), 1U);
  EXPECT_EQ(Copied.machOBuildVersions()[0].Tools[1].Version, 0x000E0100U);
  LinkGraph CopyAssigned(Target::X86_64, Endianness::Little,
                         ObjectFormat::MachO);
  CopyAssigned = Source;
  ASSERT_EQ(CopyAssigned.machOBuildVersions().size(), 1U);
  EXPECT_EQ(CopyAssigned.machOBuildVersions()[0].MinimumOS, 0x000D0000U);

  LinkGraph Moved(std::move(Source));
  ASSERT_EQ(Moved.machOBuildVersions().size(), 1U);
  EXPECT_TRUE(Source.machOBuildVersions().empty());
  EXPECT_TRUE(Source.addMachOBuildVersion(
      MachOBuildVersion{2, 0x000F0000, 0x00100000, {}}));

  LinkGraph MoveAssigned(Target::X86_64, Endianness::Little,
                         ObjectFormat::MachO);
  MoveAssigned = std::move(Source);
  ASSERT_EQ(MoveAssigned.machOBuildVersions().size(), 1U);
  EXPECT_EQ(MoveAssigned.machOBuildVersions()[0].Platform, 2U);
  EXPECT_TRUE(Source.machOBuildVersions().empty());
  EXPECT_TRUE(Source.addMachOBuildVersion(Version));

  Moved.swap(MoveAssigned);
  EXPECT_EQ(Moved.machOBuildVersions()[0].Platform, 2U);
  EXPECT_EQ(MoveAssigned.machOBuildVersions()[0].Platform, 1U);
}
...
TEST(ARMRelocationTest, DecodesImplicitThumbJumpAddends) {
  ...
  for (const auto &Test : Cases) {
    ...
    uint32_t Displacement =
        (Sign << 24) | (I1 << 23) | (I2 << 22) |
        (static_cast<uint32_t>(First & UINT16_C(0x03FF)) << 12) |
        (static_cast<uint32_t>(Second & UINT16_C(0x07FF)) << 1);
    if (Sign != 0)
...
TEST(ARMRelocationTest, AcceptsGeneratedPersonalityMarker) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  const auto ObjectBytes =
  ...
  for (const auto &Relocation : Graph->relocations())
    EXPECT_NE(Relocation.Type, llvm::ELF::R_ARM_NONE);
  ASSERT_TRUE(layout(*Graph, 0x1000));
  EXPECT_TRUE(applyRelocations(*Graph));
}

TEST(ARMRelocationTest, RejectsGeneratedPersonalityReference) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  const auto ObjectBytes = makeAssemblyObject(
...
.align 2
  .word __aeabi_unwind_cpp_pr0(prel31)
)",
      "+thumb-mode");
...
TEST(ARMRelocationTest, RelocatesSelfCallAndPackedDataAtZeroImageBase) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  const auto ObjectBytes = makeAssemblyObject(
      llvm::Triple("armv7-unknown-linux-gnueabihf"), R"(.syntax unified
.arm
...
self:
  bl self
.section .data.packed,"aw",%progbits
.byte 0
.long self
.long self-.
)");
...
TEST(ARMRelocationTest, RelocatesThumbSelfCallAtZeroImageBase) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  const auto ObjectBytes = makeAssemblyObject(
      llvm::Triple("armv7-unknown-linux-gnueabihf"), R"(.syntax unified
.thumb
...
TEST(AArch64RelocationTest, AppliesLoadStoreLow12ToHAndSRegisters) {
  REQUIRE_RELOCATION_HANDLER(Target::AArch64);
  struct Case {
    uint32_t Type;
    uint32_t Instruction;
  ...
  const std::array<Case, 2> Cases{{
      {llvm::ELF::R_AARCH64_LDST16_ABS_LO12_NC, UINT32_C(0x7D400000),
       UINT64_C(0x1AB8), UINT32_C(0x7D557000)},
      {llvm::ELF::R_AARCH64_LDST32_ABS_LO12_NC, UINT32_C(0xBD400000),
       UINT64_C(0x1ABC), UINT32_C(0xBD4ABC00)},
  }};
  for (const auto &Test : Cases) {
    std::vector<WasmEdge::Byte> Bytes(8);
    ASSERT_TRUE(Internal::writeUnsigned(Bytes, 0, 4, Endianness::Little,
                                        Test.Instruction));
    auto Graph = makeELFRelocationGraph(Target::AArch64, Endianness::Little,
                                        Test.Type, 4, Test.Target, 0x1000, 0, 0,
                                        false, std::move(Bytes));
...
TEST(AArch64RelocationTest, RejectsInvalidLoadStoreLow12Atomically) {
  REQUIRE_RELOCATION_HANDLER(Target::AArch64);
  struct Case {
    uint32_t Type;
    uint32_t Instruction;
  ...
  const std::array<Case, 4> Cases{{
      {llvm::ELF::R_AARCH64_LDST16_ABS_LO12_NC, UINT32_C(0xB9400000)},
      {llvm::ELF::R_AARCH64_LDST32_ABS_LO12_NC, UINT32_C(0x79400000)},
      {llvm::ELF::R_AARCH64_LDST16_ABS_LO12_NC, UINT32_C(0x78400000)},
      {llvm::ELF::R_AARCH64_LDST32_ABS_LO12_NC, UINT32_C(0xB8400000)},
  }};
  for (const auto &Test : Cases) {
    std::vector<WasmEdge::Byte> Bytes(8);
    ASSERT_TRUE(Internal::writeUnsigned(Bytes, 0, 4, Endianness::Little,
...
TEST(AArch64RelocationTest, RelocatesGeneratedFPSIMDLow12Loads) {
  REQUIRE_RELOCATION_HANDLER(Target::AArch64);
  REQUIRE_LLVM_TARGET("aarch64-unknown-linux-gnu");
  auto Graph = ObjectReader::read(
      makeAssemblyObject(llvm::Triple("aarch64-unknown-linux-gnu"),
...
f:
  adrp x0, hdata
  ldr h0, [x0, :lo12:hdata]
  adrp x1, sdata
  ldr s0, [x1, :lo12:sdata]
  ret
.data
.p2align 1
hdata:
...
sdata:
  ...
  ASSERT_TRUE(Graph);
  ASSERT_EQ(Graph->relocations().size(), 4U);
  EXPECT_EQ(Graph->relocations()[1].Type,
            llvm::ELF::R_AARCH64_LDST16_ABS_LO12_NC);
  EXPECT_EQ(Graph->relocations()[3].Type,
...
TEST(RISCVRelocationTest, RelocatesCompressedMisalignedSites) {
  REQUIRE_LLVM_TARGET("riscv64-unknown-linux-gnu");
  const auto ObjectBytes =
      makeAssemblyObject(llvm::Triple("riscv64-unknown-linux-gnu"), R"(
.text
.globl caller
caller:
  c.nop
...
gvar:
  .word 0
                         ...
                         "+c");
  auto Graph = ObjectReader::read(ObjectBytes, Target::RISCV64);
  ASSERT_TRUE(Graph);
  std::set<uint64_t> Offsets;
  for (const auto &Relocation : Graph->relocations())
    Offsets.insert(Relocation.Offset);
  EXPECT_NE(Offsets.find(2), Offsets.end());
  ASSERT_TRUE(layout(*Graph, 0x1000));
  EXPECT_TRUE(applyRelocations(*Graph));
}

TEST(RISCVRelocationTest, RelocatesFloatingPointLow12LoadsAndStores) {
  REQUIRE_LLVM_TARGET("riscv64-unknown-linux-gnu");
  const auto ObjectBytes =
      makeAssemblyObject(llvm::Triple("riscv64-unknown-linux-gnu"), R"(
.text
...
.Lstore:
  auipc a0, %pcrel_hi(gvar)
  fsd fa0, %pcrel_lo(.Lstore)(a0)
.Lload:
  auipc a1, %pcrel_hi(gvar)
  fld fa1, %pcrel_lo(.Lload)(a1)
  ret
.data
.globl gvar
gvar:
    ...
    for (const auto &Test : Cases) {
      ...
      const int64_t LowDelta =
          static_cast<int32_t>(static_cast<uint32_t>(*Low) &
                               UINT32_C(0xFFF00000)) >>
          20;
      EXPECT_EQ(PatchAddress + static_cast<uint64_t>(HighDelta) +
                    static_cast<uint64_t>(LowDelta),
                TargetAddress);
    }
  }
...
constexpr uint32_t AArch64COFFPageOffset =
    llvm::COFF::IMAGE_REL_ARM64_PAGEOFFSET_12L;
#else
constexpr uint32_t AArch64COFFPageOffset =
    llvm::COFF::IMAGE_REL_ARM64_PAGEOFFSET_12A;
...
TEST(MachOEHFrameTest, RelocationOwnsFDEAddressAndAppliesAddend) {
  ...
  const auto ReferencedSymbol = RelocationValue.Symbol;
  ...
  const auto &Symbol = Graph->symbols()[ReferencedSymbol];
...
TEST_P(GeneratedMachOLinkerTest, LinksGeneratedMacOSObjects) {
  ...
  auto Graph = ObjectReader::read(Object, Architecture);
  ASSERT_TRUE(Graph) << Triple;
  const auto InputBuildVersions = Graph->machOBuildVersions();
  ASSERT_FALSE(InputBuildVersions.empty()) << Triple;
  ...
  const auto *MachO = llvm::dyn_cast<llvm::object::MachOObjectFile>(&**Image);
  ASSERT_NE(MachO, nullptr) << Triple;
  std::vector<MachOBuildVersion> OutputBuildVersions;
  for (const auto &Command : MachO->load_commands()) {
    if (Command.C.cmd != llvm::MachO::LC_BUILD_VERSION)
      continue;
    const WasmEdge::Span<const WasmEdge::Byte> CommandBytes(
        reinterpret_cast<const WasmEdge::Byte *>(Command.Ptr),
        Command.C.cmdsize);
    auto Version =
        Internal::parseMachOBuildVersion(CommandBytes, Endianness::Little);
    ASSERT_TRUE(Version) << Triple;
    OutputBuildVersions.push_back(std::move(*Version));
  }
  expectMachOBuildVersionsEqual(OutputBuildVersions, InputBuildVersions);
  bool HasEHFrame = false;
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ObjectArchitectureCase{"x86_64-apple-macosx10.15.0", Target::X86_64},
        ObjectArchitectureCase{"arm64-apple-macosx10.15.0", Target::AArch64}),
    objectArchitectureCaseName);
...
TEST(ObjectReaderTest, RejectsRelocationFreeELFWithZeroSymbolEntrySize) {
  REQUIRE_LLVM_TARGET("x86_64-unknown-linux-gnu");
  auto Bytes = makeAssemblyObject(llvm::Triple("x86_64-unknown-linux-gnu"),
                                  R"(
.text
.globl leaf
leaf:
  ...
  const auto Header = elf64SymbolTableSectionHeader(Bytes);
  ASSERT_NE(Header, 0U);
  write64le(Bytes, Header + 56, 0);
  EXPECT_FALSE(ObjectReader::read(Bytes, Target::X86_64));
}
...
TEST(ObjectReaderTest, ReadsARMVFPArgumentAttributes) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  constexpr std::array<std::pair<unsigned, uint32_t>, 4> Cases{{
      {llvm::ARMBuildAttrs::BaseAAPCS, llvm::ELF::EF_ARM_ABI_FLOAT_SOFT},
      {llvm::ARMBuildAttrs::HardFPAAPCS, llvm::ELF::EF_ARM_ABI_FLOAT_HARD},
      {llvm::ARMBuildAttrs::CompatibleFPAAPCS,
       llvm::ELF::EF_ARM_ABI_FLOAT_SOFT},
      {llvm::ARMBuildAttrs::ToolChainFPPCS, 0},
  }};
  for (const auto &[Attribute, FloatFlags] : Cases) {
    auto Bytes = makeARMAttributeObject(Attribute);
    ASSERT_FALSE(armAttributeSections(Bytes).empty());
    auto Result = ObjectReader::read(Bytes, Target::ARM);
    ASSERT_TRUE(Result) << Attribute << ": "
                        << (Result ? "" : Result.error().Message);
    EXPECT_EQ(Result->elfFlags() & llvm::ELF::EF_ARM_EABIMASK,
              llvm::ELF::EF_ARM_EABI_VER5)
        << Attribute;
    EXPECT_EQ(Result->elfFlags() & (llvm::ELF::EF_ARM_ABI_FLOAT_SOFT |
                                    llvm::ELF::EF_ARM_ABI_FLOAT_HARD),
              FloatFlags)
        << Attribute;
    EXPECT_TRUE(std::none_of(Result->sections().begin(),
                             Result->sections().end(), [](const auto &Section) {
                               return Section.Name.find("attributes") !=
                                      std::string::npos;
                             }));
  }
}

TEST(ObjectReaderTest, TreatsARMSoftFPAsBaseAAPCS) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Result = ObjectReader::read(
      makeARMAttributeObject(llvm::ARMBuildAttrs::BaseAAPCS,
                             ".text\n vadd.f32 s0, s0, s0\n", "+vfp3"),
      Target::ARM);
  ASSERT_TRUE(Result);
  EXPECT_EQ(Result->elfFlags() & llvm::ELF::EF_ARM_ABI_FLOAT_SOFT,
            llvm::ELF::EF_ARM_ABI_FLOAT_SOFT);
  EXPECT_EQ(Result->elfFlags() & llvm::ELF::EF_ARM_ABI_FLOAT_HARD, 0U);
}

TEST(ObjectReaderTest, DefaultsMissingARMVFPArgumentAttributeToSoft) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Result =
      ObjectReader::read(makeARMAttributeObject(std::nullopt), Target::ARM);
  ASSERT_TRUE(Result);
  EXPECT_EQ(Result->elfFlags() & llvm::ELF::EF_ARM_ABI_FLOAT_SOFT,
            llvm::ELF::EF_ARM_ABI_FLOAT_SOFT);
}

TEST(ObjectReaderTest, RejectsUnsupportedARMHeaderFlags) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(llvm::ARMBuildAttrs::BaseAAPCS);
  constexpr uint32_t Unsupported = UINT32_C(0x00800000);
  write32le(Bytes, 36,
            llvm::ELF::EF_ARM_EABI_VER4 | Unsupported |
                llvm::ELF::EF_ARM_ABI_FLOAT_HARD);
  auto Result = ObjectReader::read(Bytes, Target::ARM);
  ASSERT_FALSE(Result);
  EXPECT_EQ(Result.error().Message, "unsupported ARM ELF header flags");
  EXPECT_EQ(Result.error().Kind, DiagnosticKind::Unsupported);
}

TEST(ObjectReaderTest, WritesAttributeNormalizedARMFlags) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(llvm::ARMBuildAttrs::BaseAAPCS);
  write32le(Bytes, 36,
            llvm::ELF::EF_ARM_EABI_VER4 | llvm::ELF::EF_ARM_ABI_FLOAT_HARD);
  auto Result = ObjectReader::read(Bytes, Target::ARM);
  ASSERT_TRUE(Result);
  constexpr uint32_t Expected =
      llvm::ELF::EF_ARM_EABI_VER5 | llvm::ELF::EF_ARM_ABI_FLOAT_SOFT;
  ASSERT_EQ(Result->elfFlags(), Expected);
  ASSERT_TRUE(ELFWriter::layout(*Result));
  ASSERT_TRUE(applyRelocations(*Result));
  std::vector<WasmEdge::Byte> OutputBytes;
  Writer Output(OutputBytes);
  ASSERT_TRUE(ELFWriter::write(*Result, Output));
  ASSERT_GE(OutputBytes.size(), 40U);
  EXPECT_EQ(read32le(OutputBytes, 36), Expected);
}

TEST(ObjectReaderTest, RejectsMalformedARMAttributes) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(llvm::ARMBuildAttrs::HardFPAAPCS);
  const auto Sections = armAttributeSections(Bytes);
  ASSERT_FALSE(Sections.empty());
  ASSERT_NE(Sections[0].second, 0U);
  Bytes[Sections[0].first] = 'B';
  EXPECT_FALSE(ObjectReader::read(Bytes, Target::ARM));
}

TEST(ObjectReaderTest, RejectsUnknownARMVFPArgumentAttribute) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  EXPECT_FALSE(ObjectReader::read(makeARMAttributeObject(4), Target::ARM));
}

TEST(ObjectReaderTest, RejectsConflictingARMAttributeSections) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(llvm::ARMBuildAttrs::BaseAAPCS,
                                      R"(
.section .ARM.attributes.extra,"",%progbits
.byte 0x41
.long 0x11
.asciz "aeabi"
.byte 0x01
...
.byte 0x1c, 0x01
  ...
  markARMAttributeSection(Bytes, ".ARM.attributes.extra");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  EXPECT_FALSE(ObjectReader::read(Bytes, Target::ARM));
}

TEST(ObjectReaderTest, RejectsConflictingARMAttributesWithinSection) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(llvm::ARMBuildAttrs::BaseAAPCS,
                                      R"(
.section .ARM.attributes.duplicates,"",%progbits
.byte 0x41
.long 0x13
.asciz "aeabi"
.byte 0x01
.long 0x09
.byte 0x1c, 0x00, 0x1c, 0x01
)");
  markARMAttributeSection(Bytes, ".ARM.attributes.duplicates");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  EXPECT_FALSE(ObjectReader::read(Bytes, Target::ARM));
}

TEST(ObjectReaderTest, ReadsSectionScopedARMVFPArgumentAttribute) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(std::nullopt, R"(
.section arm_section_attributes,"",%progbits
.byte 0x41
.long 0x13
.asciz "aeabi"
.byte 0x02
...
.byte 0x1c, 0x01
  ...
  markARMAttributeSection(Bytes, "arm_section_attributes");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  auto Result = ObjectReader::read(Bytes, Target::ARM);
...
TEST(ObjectReaderTest, ReadsSymbolScopedARMVFPArgumentAttribute) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(std::nullopt, R"(
.section arm_symbol_attributes,"",%progbits
.byte 0x41
.long 0x13
.asciz "aeabi"
.byte 0x03
...
.byte 0x1c, 0x02
  ...
  markARMAttributeSection(Bytes, "arm_symbol_attributes");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  auto Result = ObjectReader::read(Bytes, Target::ARM);
  ASSERT_TRUE(Result) << (Result ? "" : Result.error().Message);
  EXPECT_EQ(Result->elfFlags() & (llvm::ELF::EF_ARM_ABI_FLOAT_SOFT |
...
TEST(ObjectReaderTest, RejectsConflictingARMAttributeScopes) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(llvm::ARMBuildAttrs::BaseAAPCS, R"(
.section arm_scoped_attributes,"",%progbits
.byte 0x41
.long 0x1c
.asciz "aeabi"
.byte 0x02
...
.byte 0x1c, 0x02
  ...
  markARMAttributeSection(Bytes, "arm_scoped_attributes");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  EXPECT_FALSE(ObjectReader::read(Bytes, Target::ARM));
}

TEST(ObjectReaderTest, RejectsARMScopedAttributeCrossingSubsectionBoundary) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(std::nullopt, R"(
.section arm_scoped_boundary,"",%progbits
.byte 0x41
.long 0x1a
.asciz "aeabi"
.byte 0x02
...
.byte 0x1c, 0x01
  ...
  markARMAttributeSection(Bytes, "arm_scoped_boundary");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  EXPECT_FALSE(ObjectReader::read(Bytes, Target::ARM));
}

TEST(ObjectReaderTest, IgnoresPrivateARMAttributeVendor) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(std::nullopt, R"(
.section arm_private_attributes,"",%progbits
.byte 0x41
.long 0x0f
.asciz "private"
.byte 0xff, 0x80, 0x7f
...
.byte 0x1c, 0x01
  ...
  markARMAttributeSection(Bytes, "arm_private_attributes");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
...
TEST(ObjectReaderTest, IgnoresMixedCasePrivateARMAttributeVendors) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  for (const std::string Vendor : {"AEABI", "Aeabi"}) {
    auto Bytes = makeARMAttributeObject(
        std::nullopt, ".section arm_mixed_case_attributes,\"\",%progbits\n"
                      ".byte 0x41\n"
                      ".long 0x0d\n"
                      ".asciz \"" +
                          Vendor +
                          ...
                          ".long 0x07\n"
                          ".byte 0x1c, 0x01\n");
    markARMAttributeSection(Bytes, "arm_mixed_case_attributes");
    EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
    const auto Original = Bytes;
    auto Result = ObjectReader::read(Bytes, Target::ARM);
    ...
    EXPECT_EQ(Result->elfFlags() & (llvm::ELF::EF_ARM_ABI_FLOAT_SOFT |
              ...
              llvm::ELF::EF_ARM_ABI_FLOAT_HARD)
        << Vendor;
    EXPECT_EQ(Bytes, Original) << Vendor;
  }
}

TEST(ObjectReaderTest, IgnoresManyPrivateARMAttributeVendors) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  std::string Assembly =
      ".section arm_many_private_attributes,\"\",%progbits\n.byte 0x41\n";
  for (unsigned I = 0; I < 256; ++I)
    Assembly += ".long 0x0a\n.asciz \"p" + std::to_string(I % 10) +
                "\"\n.byte 0xde, 0xad, 0xbe\n";
  Assembly += R"(
.long 0x11
.asciz "aeabi"
...
.byte 0x1c, 0x01
  ...
  auto Bytes = makeARMAttributeObject(std::nullopt, std::move(Assembly));
  markARMAttributeSection(Bytes, "arm_many_private_attributes");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  const auto Original = Bytes;
  auto Result = ObjectReader::read(Bytes, Target::ARM);
  ASSERT_TRUE(Result) << (Result ? "" : Result.error().Message);
...
TEST(ObjectReaderTest, RejectsUnterminatedPrivateARMAttributeVendor) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(std::nullopt, R"(
.section arm_private_attributes,"",%progbits
.byte 0x41
...
TEST(ObjectReaderTest, RejectsTrailingByteInARMAttributes) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(std::nullopt, R"(
.section arm_trailing_attribute_byte,"",%progbits
.byte 0x41
.long 0x11
.asciz "aeabi"
.byte 0x01
...
.byte 0x00
  ...
  markARMAttributeSection(Bytes, "arm_trailing_attribute_byte");
  EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
  EXPECT_FALSE(ObjectReader::read(Bytes, Target::ARM));
}

TEST(ObjectReaderTest, RejectsTrailingBytesInARMAttributes) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  for (const std::string Tail :
       {".byte 0x00, 0x00, 0x00\n", ".byte 0x00, 0x01, 0x00\n"}) {
    auto Bytes = makeARMAttributeObject(
        std::nullopt, ".section arm_trailing_attribute_bytes,\"\",%progbits\n"
                      ".byte 0x41\n"
                      ".long 0x11\n"
                      ".asciz \"aeabi\"\n"
                      ".byte 0x01\n"
                      ...
                      ".byte 0x1c, 0x01\n" +
                          Tail);
    markARMAttributeSection(Bytes, "arm_trailing_attribute_bytes");
    EXPECT_EQ(armAttributeSections(Bytes).size(), 2U);
    EXPECT_FALSE(ObjectReader::read(Bytes, Target::ARM)) << Tail;
  }
}

TEST(ObjectReaderTest, OmitsAllocatableARMAttributeSections) {
  REQUIRE_LLVM_TARGET("armv7-unknown-linux-gnueabihf");
  auto Bytes = makeARMAttributeObject(llvm::ARMBuildAttrs::HardFPAAPCS);
  markARMAttributeSection(Bytes, ".ARM.attributes", llvm::ELF::SHF_ALLOC);
  auto Object =
      llvm::object::ObjectFile::createObjectFile(llvm::MemoryBufferRef(
          llvm::StringRef(reinterpret_cast<const char *>(Bytes.data()),
                          Bytes.size()),
          "allocatable-arm-attributes.o"));
  ASSERT_TRUE(static_cast<bool>(Object));
  bool FoundAllocatableAttributes = false;
  for (const auto &Section : (*Object)->sections()) {
    const llvm::object::ELFSectionRef ELFSection(Section);
    FoundAllocatableAttributes |=
        ELFSection.getType() == llvm::ELF::SHT_ARM_ATTRIBUTES &&
        (ELFSection.getFlags() & llvm::ELF::SHF_ALLOC) != 0;
  }
  ASSERT_TRUE(FoundAllocatableAttributes);
  auto Result = ObjectReader::read(Bytes, Target::ARM);
  ASSERT_TRUE(Result) << (Result ? "" : Result.error().Message);
  EXPECT_TRUE(std::none_of(
      Result->sections().begin(), Result->sections().end(),
      [](const auto &Section) { return Section.Name == ".ARM.attributes"; }));
}
