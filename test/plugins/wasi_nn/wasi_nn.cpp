...
using WasmEdge::Host::WASINN::ErrNo;
    ...
    defined(WASMEDGE_PLUGIN_WASI_NN_BACKEND_TF)
namespace {
...
TEST(WasiNNTest, TFBackend) {
  ...
  writeFatPointer(MemInst, StorePtr, WeightRead.size(), BuilderPtr);
  writeFatPointer(MemInst, StorePtr + WeightRead.size(), TagSet.size(),
                  BuilderPtr);
  writeFatPointer(MemInst, StorePtr + WeightRead.size() + TagSet.size(),
  ...
  StorePtr += (WeightRead.size() + TagSet.size() + SigMapVector.size());
  {
    EXPECT_TRUE(HostFuncLoad.run(CallFrame,
                                 std::initializer_list<WasmEdge::ValVariant>{
                                     LoadEntryPtr, UINT32_C(2),
  ...
  //     EXPECT_TRUE(HostFuncInit.run(
  //         CallFrame,
  //         std::initializer_list<WasmEdge::ValVariant>{UINT32_C(2),
  //         BuilderPtr}, Errno));
  //     EXPECT_EQ(Errno[0].get<int32_t>(),
  //               static_cast<uint32_t>(ErrNo::InvalidArgument));
  //   }
  ...
  //   // Swap to the tmp. env.
  //   // Test: init_execution_context -- graph id exceeds.
  //   NNGraphTmp.emplace_back(Backend::Tensorflow);
  //   NNGraphTmp.swap(NNMod->getEnv().NNGraph);
  //   NNContextTmp.swap(NNMod->getEnv().NNContext);
  //   {
  //     EXPECT_TRUE(HostFuncInit.run(
  ...
  //   // WASI-NN set_input tests.
  //   SetInputEntryPtr = BuilderPtr;
  //   writeFatPointer(MemInst, StorePtr, TensorDim.size(), BuilderPtr);
  //   writeUInt32(MemInst, UINT32_C(1), BuilderPtr);
  //   writeFatPointer(MemInst, StorePtr + TensorDim.size() * 4,
  //   TensorData.size(),
  //                   BuilderPtr);
  //   writeBinaries<uint32_t>(MemInst, TensorDim, StorePtr);
  //   writeBinaries<uint8_t>(MemInst, TensorData, StorePtr + TensorDim.size() *
  //   4);
  ...
  //     EXPECT_TRUE(HostFuncCompute.run(
  //         CallFrame,
  //         std::initializer_list<WasmEdge::ValVariant>{UINT32_C(3)}, Errno));
  //     EXPECT_EQ(Errno[0].get<int32_t>(),
  //               static_cast<uint32_t>(ErrNo::InvalidArgument));
  //   }
  ...
  //     EXPECT_TRUE(HostFuncCompute.run(
  //         CallFrame,
  //         std::initializer_list<WasmEdge::ValVariant>{UINT32_C(0)}, Errno));
  //     EXPECT_EQ(Errno[0].get<int32_t>(),
  //               static_cast<uint32_t>(ErrNo::MissingMemory));
  //   }
  //   // Swap back.
  ...
  //     EXPECT_TRUE(HostFuncCompute.run(
  //         CallFrame,
  //         std::initializer_list<WasmEdge::ValVariant>{UINT32_C(1)}, Errno));
  //     EXPECT_EQ(Errno[0].get<int32_t>(),
  //     static_cast<uint32_t>(ErrNo::Success));
  //   }
  //   // WASI-NN get_output tests.
  ...
  //         MemInst.getPointer<uint8_t *>(StorePtr, 965),
  //         MemInst.getPointer<uint8_t *>(StorePtr, 965) + 965);
  //     std::vector<size_t> SortedIndex, CorrectClasses{166, 158, 34, 778,
  //     819};
  //     // FIXME: classSort causing segmentation fault
  //     SortedIndex = classSort<uint8_t>(OutputClassification);
...
#endif // WASMEDGE_PLUGIN_WASI_NN_BACKEND_TF