...
TEST(WasiNNTest, TFBackend) {
  ...
  // Test: get_output -- get output successfully.
  {
    EXPECT_TRUE(HostFuncGetOutput.run(
        ...
        Errno));
    EXPECT_EQ(Errno[0].get<int32_t>(), static_cast<uint32_t>(ErrNo::Success));
    EXPECT_EQ(*MemInst.getPointer<uint32_t *>(BuilderPtr), UINT32_C(965 * 4));
    std::vector<float_t> OutputClassification(
        MemInst.getPointer<float_t *>(StorePtr, 965),
        MemInst.getPointer<float_t *>(StorePtr, 965) + 965 * 4);
    std::vector<size_t> SortedIndex, CorrectClasses{576, 427, 331, 798, 743};
    // FIXME: classSort causing segmentation fault
    SortedIndex = classSort<float_t>(OutputClassification);
