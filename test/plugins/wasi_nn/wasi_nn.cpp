...
TEST(WasiNNTest, GGMLBackendBitNetModel) {
  auto NNMod = createModule();
  ASSERT_TRUE(NNMod);
  ...
  const std::string ModelPath = "./wasinn_ggml_fixtures/ggml-model-i2_s.gguf";
...

  {
    ...
    ASSERT_TRUE(HostFuncLoad.run(
        CallFrame,
        std::initializer_list<WasmEdge::ValVariant>{
            LoadEntryPtr, 2, static_cast<uint32_t>(Backend::GGML),
            static_cast<uint32_t>(Device::CPU), BuilderPtr},
        Errno))
        << "Load call failed. Ensure model file exists at: " << ModelPath;
    if (Errno[0].get<int32_t>() !=
        static_cast<uint32_t>(ErrNo::Success)) {
      EXPECT_NE(Errno[0].get<int32_t>(),
                static_cast<uint32_t>(ErrNo::Success));
      return;
    }
