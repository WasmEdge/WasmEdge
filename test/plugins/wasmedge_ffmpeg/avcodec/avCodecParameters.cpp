...
TEST_F(FFmpegTest, BorrowedHandlesSurviveGuestFree) {
  ...
  // Closing the context frees codecpar and metadata exactly once. Before the
  // ownership fix the guest frees above had already released them, so this
  // close double-freed.
...
TEST_F(FFmpegTest, ClosingContextInvalidatesBorrowedChildIds) {
  ...
  // child ids must no longer resolve; otherwise a later use is a
  // use-after-free.
