...
TEST(WasmEdgeAVCodecTest, AVFrameFunc) {
  ...
  uint32_t AvCodecPtr = UINT32_C(0); // To separate memory address. Same number
                                     // leading to Collision of addresses.

  {
    writeUInt32(MemInst, UINT32_C(0), AvCodecPtr);
    EXPECT_TRUE(HostFuncAVCodecAllocContext3.run(
        CallFrame, std::initializer_list<WasmEdge::ValVariant>{0, AvCodecPtr},
        Result));
