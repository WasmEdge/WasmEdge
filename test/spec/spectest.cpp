...
bool SpecTest::compare(const std::pair<std::string, std::string> &Expected,
  ...
  if (!IsV128 && ValStr.substr(0, 4) == "nan:"sv) {
    ...
    if (LaneType == "f32") {
      float VF[4];
      uint32_t VI[4];
      std::memcpy(VF, &Got.first.get<float>(), 16);
      std::memcpy(VI, &Got.first.get<uint32_t>(), 16);
#if !WASMEDGE_ENDIAN_LITTLE_BYTE
      std::reverse(VI, VI + 4);
      std::reverse(VF, VF + 4);
#endif
      ...
      double VF[2];
      uint64_t VI[2];
      std::memcpy(VF, &Got.first.get<double>(), 16);
      std::memcpy(VI, &Got.first.get<uint64_t>(), 16);
#if !WASMEDGE_ENDIAN_LITTLE_BYTE
      std::reverse(VI, VI + 2);
      std::reverse(VF, VF + 2);
#endif
