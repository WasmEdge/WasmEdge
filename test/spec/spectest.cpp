...
bool equalComponentValue(const WasmEdge::ComponentValVariant &A,
  ...
  if (std::holds_alternative<std::shared_ptr<ValComp>>(A)) {
    ...
    if (std::holds_alternative<FlagsVal>(CA->V)) {
      // A lifted flags value carries both forms; compare the set labels.
      auto SetOf = [](const FlagsVal &F) {
        std::vector<std::string> S = F.SetLabels;
        std::sort(S.begin(), S.end());
        return S;
      };
      return SetOf(std::get<FlagsVal>(CA->V)) ==
             SetOf(std::get<FlagsVal>(CB->V));
    }
