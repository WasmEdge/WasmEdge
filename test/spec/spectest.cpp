...
bool SpecTest::compareResult(const Wast::Result &Expected,
  ...
  // getCode() is only Ref/RefNull for reference types; the heap kind lives in
  // getHeapTypeCode(), so dispatch on that.
  ...
  // Reference types: dispatch on the heap type code (see above).
  if (Expected.Type.isRefType()) {
    ...
    case TypeCode::I31Ref:
      ...
      return IsRefMatch(Got.first.get<RefVariant>());
    case TypeCode::NullRef:
      // "nullref" (none), bottom of internal refs: matches any internal ref.
      if (!Got.second.isRefType() || Got.second.isExternRefType()) {
        return false;
      }
      return IsRefMatch(Got.first.get<RefVariant>());
    ...
    case TypeCode::NullFuncRef:
      // "nullfuncref" (nofunc): bottom type for func refs.
      if (!Got.second.isFuncRefType()) {
        return false;
      }
      return IsRefMatch(Got.first.get<RefVariant>());
    ...
    case TypeCode::NullExternRef:
      // "nullexternref" (noextern): bottom type for extern refs.
      if (!Got.second.isExternRefType()) {
        return false;
      }
      return IsRefMatch(Got.first.get<RefVariant>());
    ...
    case TypeCode::NullExnRef:
      // "nullexnref" (noexn): bottom type for exn refs.
      if (!Got.second.isRefType() ||
          (Got.second.getHeapTypeCode() != TypeCode::ExnRef &&
           Got.second.getHeapTypeCode() != TypeCode::NullExnRef)) {
        return false;
  ...
  if (Code == TypeCode::V128) {
    ...
    // Lane type from shape string "<type>x<count>" (e.g. "f32x4" -> "f32").
...
void SpecTest::executeCommands(Span<const Wast::ScriptCommand> Commands,
  ...
  // googletest assertion macros race when called from spawned threads (on
  // Windows). Each thread intercepts its assertions into a thread-local
  // reporter and appends them here under the mutex; the parent replays them
  // after join().
  ...
  // Unit name for checkComponentSupported(). Use stem(), not filename():
  // ComponentModelFolders keys are bare folder names without the extension
  // (e.g. "empty" from ".../empty/empty.json").
  std::string UnitName;
  if (IsComponent) {
    std::filesystem::path PF{TestFile};
    UnitName = PF.stem().u8string();
  }

  // Resolve source for onParse. File-based modules get their full path
  // resolved and stored (ResolvedPaths) to back the returned view.
  ...
  for (const auto &Cmd : Commands) {
    ...
    case Wast::CommandType::AssertExhaustion: {
      // Runtime has no stack-exhaustion detection; log as skipped rather than
      // silently passing so the missing coverage stays visible.
      // TODO: Require a stack-exhaustion error once the runtime reports one.
      spdlog::debug("{}:{}: assert_exhaustion skipped (unsupported)"sv,
                    TestFile, Cmd.Line);
      break;
    }
    case Wast::CommandType::AssertInvalid: {
      ...
      // Text/WAT modules may surface validation errors during parsing, so
      // accept either phase; binary modules keep the strict single-phase
      // (Validation) check to catch loader/validator misclassification.
      const bool IsText = Cmd.ModType == Wast::ModuleType::Text ||
                          Cmd.ModType == Wast::ModuleType::Quote ||
                          Cmd.ModType == Wast::ModuleType::TextFile;

      // onParse
      auto ParseRes = onParse(RootCtx, Source, Type, Conf);
      if (!ParseRes) {
        const auto Phase = ParseRes.error().getErrCodePhase();
        const bool PhaseOk = Phase == WasmEdge::WasmPhase::Validation ||
                             (IsText && Phase == WasmEdge::WasmPhase::Loading);
        EXPECT_TRUE(PhaseOk)
    ...
    case Wast::CommandType::AssertMalformed: {
      ...
      // Text/WAT modules may surface validation errors during parsing, so
      // accept either phase; binary modules keep the strict single-phase
      // (Loading) check to catch loader/validator misclassification.
      const bool IsText = Cmd.ModType == Wast::ModuleType::Text ||
                          Cmd.ModType == Wast::ModuleType::Quote ||
                          Cmd.ModType == Wast::ModuleType::TextFile;
      ...
      if (ParseRes) {
        ...
        const bool PhaseOk =
            ...
            (IsText && Phase == WasmEdge::WasmPhase::Validation);
        EXPECT_TRUE(PhaseOk)
  ...
  // All spawned threads have joined; replay their intercepted assertion
