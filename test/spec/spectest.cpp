...
bool SpecTest::compareResult(const Wast::Result &Expected,
  ...
  if (Expected.Type.isRefType()) {
    ...
    case TypeCode::I31Ref:
      ...
      return IsRefMatch(Got.first.get<RefVariant>());
    case TypeCode::NullRef:
      // The "nullref" type, which is none, is the bottom type of the internal
      // references. It matches every internal reference.
      if (!Got.second.isRefType() || Got.second.isExternRefType()) {
        return false;
      }
      return IsRefMatch(Got.first.get<RefVariant>());
    ...
    case TypeCode::NullFuncRef:
      // The "nullfuncref" type, which is nofunc, is the bottom type of the
      // func references.
      if (!Got.second.isFuncRefType()) {
        return false;
      }
    ...
    case TypeCode::NullExternRef:
      // The "nullexternref" type, which is noextern, is the bottom type of the
      // extern references.
      if (!Got.second.isExternRefType()) {
        return false;
      }
    ...
    case TypeCode::NullExnRef:
      // The "nullexnref" type, which is noexn, is the bottom type of the exn
      // references.
      if (!Got.second.isRefType() ||
          (Got.second.getHeapTypeCode() != TypeCode::ExnRef &&
           Got.second.getHeapTypeCode() != TypeCode::NullExnRef)) {
  ...
  if (Code == TypeCode::V128) {
    ...
    // Get the lane type from the shape string "<type>x<count>". For example,
    // "f32x4" gives "f32".
    ...
    case V128Shape::I64x2:
      ...
      break;
    default:
      // The shape is empty, or the code does not know it. Compare the raw 128
      // bits.
...
void SpecTest::executeCommands(Span<const Wast::ScriptCommand> Commands,
  ...
  // Some units hold only commands that this runner cannot run, so they run
  // no assertion. Such a unit must not report as a passing test. A thread
  // body adds to the counts of its unit.
  CommandCounts OwnCounts;
  const bool IsUnitRoot = (UnitCounts == nullptr);
  CommandCounts &Counts = IsUnitRoot ? OwnCounts : *UnitCounts;
  ...
  for (const auto &Cmd : Commands) {
    ...
    case Wast::CommandType::AssertExhaustion: {
      // The runtime does not report a stack exhaustion, so this command does
      // not run. Count it, and report the count at the end of the unit. If
      // the runner skips it silently, the unit passes but covers less than
      // the suite states.
      // TODO: Run this command after the runtime reports a stack exhaustion.
      ++Counts.SkippedExhaustion;
      break;
    }
    ...
    case Wast::CommandType::Thread: {
      ...
      ThreadMap.emplace(
          ...
          std::thread([this, ChildCtx, SubCmds = Cmd.SubCommands, Conf,
                       IsComponent, TestFile, &Counts, &ThreadResultMutex,
  ...
  // Report the commands that this runner does not run, so that the result of
  // the unit shows the missing coverage. A unit whose assertions are all
  // unsupported asserts nothing, so it reports as skipped and not as passed.
  // This report comes last, because GTEST_SKIP returns from this function.
  const uint32_t SkippedExhaustion = Counts.SkippedExhaustion.load();
  if (IsUnitRoot && SkippedExhaustion > 0) {
    if (Counts.RanAssertions.load() == 0) {
      GTEST_SKIP() << TestFile << ": every assertion of this unit is an "
                   << "assert_exhaustion command, and the runtime does not "
                      "report a stack exhaustion. "
                   << SkippedExhaustion << " command(s) did not run.";
    }
    // GTEST_LOG_ writes to the test output whatever the logging level of the
    // runtime is.
    GTEST_LOG_(WARNING) << TestFile << ": skipped " << SkippedExhaustion
                        << " assert_exhaustion command(s); the runtime does "
                           "not report a stack exhaustion";
  }
}
