...
void SpecTest::executeCommands(Span<const Wast::ScriptCommand> Commands,
  ...
  for (const auto &Cmd : Commands) {
    ...
    case Wast::CommandType::AssertExhaustion: {
      // The runtime does not report a stack exhaustion. Log this command as
      // skipped, and do not pass it without a message. The missing coverage
      // then stays visible.
      // TODO: Need a stack-exhaustion error after the runtime reports one.
    ...
    case Wast::CommandType::AssertInvalid: {
      ...
      // A text module or a WAT module can report a validation error in the
      // parse step, so accept the two phases. A binary module keeps the strict
      // test on the validation phase. This test finds a wrong class of error
      // from the loader or from the validator.
    ...
    case Wast::CommandType::AssertMalformed: {
      ...
      // A text module or a WAT module can report a validation error in the
      // parse step, so accept the two phases. A binary module keeps the strict
      // test on the load phase. This test finds a wrong class of error from
      // the loader or from the validator.
      const bool IsText = Cmd.ModType == Wast::ModuleType::Text ||
                          Cmd.ModType == Wast::ModuleType::Quote ||
                          Cmd.ModType == Wast::ModuleType::TextFile;
