...
void SpecTest::executeCommands(Span<const Wast::ScriptCommand> Commands,
  ...
  // For component-model tests, extract the unit name (the test folder name,
  // e.g. "empty" from ".../empty/empty.json") for checkComponentSupported().
  // Use stem(), not filename(): the lookup keys in ComponentModelFolders are
  // bare folder names without the ".json"/".wast" extension.
  std::string UnitName;
  if (IsComponent) {
