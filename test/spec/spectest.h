...
public:
  ...
  // The parser mode selects the test files that enumerate() lists, so it is
  // a part of the construction. A static SpecTest enumerates at the static
  // initialization, before any test body runs.
  explicit SpecTest(std::filesystem::path Root,
                    ParserMode ParseMode = ParserMode::Json)
      : Mode(ParseMode), TestsuiteRoot(std::move(Root)) {}
...
private:
  // The counts that decide the report at the end of a unit. The body of a
  // (thread ...) command runs on its own thread and adds to the counts of the
  // unit, so the counts are atomic.
  struct CommandCounts {
    std::atomic<uint32_t> SkippedExhaustion{0};
    std::atomic<uint32_t> RanAssertions{0};
  };

  // Run the WAST commands or JSON commands of the parser on a context. The
  // body of a (thread ...) command gets the counts of its unit as UnitCounts.
  // Only the call without UnitCounts reports the counts, after it joins all
  // of its threads.
  void executeCommands(Span<const Wast::ScriptCommand> Commands,
                       ...
                       CommandCounts *UnitCounts = nullptr);
