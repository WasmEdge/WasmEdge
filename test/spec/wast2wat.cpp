...
int main(int Argc, char *Argv[]) {
  ...
  for (auto &Entry : fs::recursive_directory_iterator(InputDir)) {
    ...
    for (auto It = RelPath.begin(); It != RelPath.end(); ++It) {
      ...
      if (Next == RelPath.end()) {
        // Filename component: use its stem only if it differs from the parent
        // dir. A .wast directly under InputDir has no parent to compare, so
        // just use the stem. (We track the parent during forward iteration
        // because path iterators do not portably support std::prev.)
