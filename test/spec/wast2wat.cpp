  ...
  for (auto &Entry : fs::recursive_directory_iterator(InputDir)) {
    ...
    for (auto It = RelPath.begin(); It != RelPath.end(); ++It) {
      ...
      if (Next == RelPath.end()) {
        ...
        // there is nothing to compare against — just use the stem. (Tracking
        // the parent during forward iteration avoids std::prev, which path
        // iterators do not portably support.)
