...
/// Node type for the WAT grammar. Mirrors the enum in converter.h, redefined
/// locally to avoid depending on WAT converter internals.
enum class NodeType {
  Sexpr,
  Keyword,
  U,
...
// --- WastConverter ---
// Walks a WAT grammar parse tree (sexpr/keyword nodes) into a WastScript,
// interpreting semantics via keyword-string matching (like WAT Converter).
...
private:
  ...
  static size_t findNextString(std::string_view Src, size_t &Pos) {
    while (Pos < Src.size()) {
      ...
      if (Pos + 1 < Src.size() && Src[Pos] == '(' && Src[Pos + 1] == ';') {
        // WAT block comments nest: (; a (; b ;) c ;). Track depth so a quote
        // sitting in the comment tail after an inner ;) is not mistaken for a
        // real string token and spliced into the reconstructed module bytes.
        size_t Depth = 1;
        Pos += 2;
  ...
  // Decode a quoted token (register name / trap message), resolving escapes so
  // it compares equal to the runtime's decoded strings rather than the raw
  // backslash-escaped lexeme (e.g. \"/\5c). Owned by OwnedStrings_ to keep the
  // view valid; on failure, fall back to the lexeme with quotes stripped.
...
// --- WastConverter::convert (out-of-line, after helpers) ---
