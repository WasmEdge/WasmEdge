...
// the values for 3b are not derived from testing but instead chosen
// conservatively static const std::map<e_model, size_t> &
// VRAM_REQ_SCRATCH_BASE()
// {
//     static std::map<e_model, size_t> k_sizes = {
//         { MODEL_3B,   512ull * kB },
//         { MODEL_7B,   512ull * kB },
...
struct llama_file_loader {
  ...
  void read_magic() {
    ...
    throw std::runtime_error(format("unknown (magic, version) combination: "
                                    "%08x, %08x; is this really a GGML file?",
                                    magic, version));
  }
  void read_hparams() {
...
struct llama_file_saver {
  ...
  void write_vocab() {
    if (any_file_loader->file_version == LLAMA_FILE_VERSION_GGML) {
      LLAMA_LOG_WARN("llama.cpp: WARNING: input is an old file that doesn't "
...
#define LLAMA_BACKEND_OFFLOAD_SPLIT GGML_BACKEND_CPU
  ...
  // print memory requirements
  {
    ...
    if (low_vram) {
      ...
      if (n_gpu_layers > 0) {
        LLAMA_LOG_INFO("%s: allocating batch_size x (%zd kB + n_ctx x %zd B) = "
                       "%zd MB VRAM for the scratch buffer\n",
                       __func__, vram_scratch_base / kB,
                       vram_scratch_per_context,
    ...
    LLAMA_LOG_INFO("%s: total VRAM used: %zu MB\n", __func__,
                   (vram_weights + vram_scratch + vram_kv_cache + MB - 1) /
                       MB); // round up
#else
    (void)n_gpu_layers;
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    // self-attention
    {
      ...
      struct ggml_tensor *K = ggml_permute(
          ...
          ggml_reshape_3d(
              ctx0,
              ggml_view_1d(ctx0, kv_self.k, (n_past + N) * n_embd_gqa,
...
llama_model_quantize_internal(const std::string &fname_inp,
  ...
  for (llama_load_tensor &tensor : model_loader->tensors_map.tensors) {
    ...
    LLAMA_LOG_INFO("[%4zu/%4zu] %36s - %16s, type = %6s, ", ++idx,
                   model_loader->tensors_map.tensors.size(),
                   tensor.name.c_str(),
...
llama_new_context_with_model(struct llama_model *model,
  ...
  if (!params.vocab_only) {
...
#ifdef LLAMA_USE_ALLOCATOR
    {
      ...
      // LLAMA_LOG_INFO("%s: (debug) equivalent with scratch buffer = %7.2f
